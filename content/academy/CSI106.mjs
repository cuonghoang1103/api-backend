...
export default {
  ...
  course: {
    ...
    shortDescription: 'The big picture of CS — computers & the von Neumann model, data as bits (binary/hex/two\'s complement/Unicode), Boolean logic & gates, CPU & memory, operating systems, networks & the web, algorithms & Big-O, data structures & databases, security, ethics & AI. Bilingual, with examples, tables & quizzes.|||Bức tranh lớn của KHMT — máy tính & mô hình von Neumann, dữ liệu là bit (nhị phân/hex/bù 2/Unicode), logic Boole & cổng, CPU & bộ nhớ, hệ điều hành, mạng & web, thuật toán & Big-O, cấu trúc dữ liệu & CSDL, an toàn, đạo đức & AI. Song ngữ, có ví dụ, bảng & quiz.',
