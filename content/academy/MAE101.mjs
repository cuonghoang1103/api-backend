...
`,
        ...
        {
          title: "1.3 — Indeterminate 0/0: factor &amp; rationalize|||1.3 — Khử dạng vô định 0/0: phân tích &amp; nhân liên hợp",
          slug: "mae101-1-3-khu-vo-dinh",
          type: 'DOCUMENT',
          description: "Phân tích thành nhân tử, nhân liên hợp, quy đồng và rút gọn để khử 0/0.",
          content: "<div class=\"ml-en\"><span class=\"eyebrow\">Chapter 1 · Lesson 1.3</span><h2>When substitution gives 0/0</h2><p class=\"lead\">If plugging in a produces 0/0, that is an <strong>indeterminate form</strong> — the answer could be anything until you simplify. The zero on top and the zero on the bottom share a common factor; cancel it, then substitute.</p><div class=\"formula\"><span class=\"lbl\">The indeterminate forms (must simplify first)</span>0/0    ∞/∞    ∞ − ∞    0·∞    1<sup>∞</sup>    0⁰    ∞⁰</div><h3>Three tools</h3><div class=\"lz-stack\"><div class=\"lz-layer\"><div class=\"lz-lt\">Factor &amp; cancel</div><div class=\"lz-ld\">For polynomials: factor top and bottom, cancel the shared (x − a) that causes the zero.</div></div><div class=\"lz-layer\"><div class=\"lz-lt\">Rationalize (multiply by the conjugate)</div><div class=\"lz-ld\">When a square root causes the zero, multiply top and bottom by the conjugate so √ ( ) turns into a plain difference.</div></div><div class=\"lz-layer\"><div class=\"lz-lt\">Combine fractions</div><div class=\"lz-ld\">For a difference of fractions, put over a common denominator first, then cancel.</div></div></div><h3>Worked examples (step by step)</h3><div class=\"out\"><b>Example 1 — factor.</b> lim<sub>x→3</sub> (x² − 9)/(x − 3).<br>Substitute → 0/0. Factor top: x² − 9 = (x − 3)(x + 3).<br>= lim<sub>x→3</sub> [(x − 3)(x + 3)]/(x − 3) = lim<sub>x→3</sub> (x + 3) = 3 + 3 = <b>6</b>.</div><div class=\"out\"><b>Example 2 — rationalize.</b> lim<sub>x→0</sub> (√(x + 1) − 1)/x.<br>Substitute → 0/0. Multiply top and bottom by the conjugate (√(x + 1) + 1):<br>= lim<sub>x→0</sub> [(x + 1) − 1] / [x(√(x + 1) + 1)] = lim<sub>x→0</sub> x/[x(√(x + 1) + 1)]<br>= lim<sub>x→0</sub> 1/(√(x + 1) + 1) = 1/(1 + 1) = <b>1/2</b>.</div><div class=\"out\"><b>Example 3 — combine fractions.</b> lim<sub>x→0</sub> (1/(x + 2) − 1/2)/x.<br>Top = [2 − (x + 2)] / [2(x + 2)] = −x / [2(x + 2)].<br>Divide by x: = lim<sub>x→0</sub> −1/[2(x + 2)] = −1/[2(2)] = <b>−1/4</b>.</div><div class=\"callout\"><b>Common mistakes.</b> (1) Cancelling (x − a) top and bottom and forgetting the result is still a limit — you must still substitute afterward. (2) Multiplying by the wrong conjugate (sign flipped) so nothing simplifies. (3) &ldquo;Cancelling&rdquo; terms that are added, not multiplied — you may only cancel common <em>factors</em>, never across a + or −.</div><h3>Practice</h3><p><b>P1.</b> lim<sub>x→4</sub> (x² − 16)/(x − 4).  <b>P2.</b> lim<sub>x→4</sub> (√x − 2)/(x − 4).  <b>P3.</b> lim<sub>x→2</sub> (x² + x − 6)/(x − 2).</p><details><summary>Answers</summary><p>P1: (x + 4) → <b>8</b>. P2: rationalize → 1/(√x + 2) → <b>1/4</b>. P3: (x + 3)(x − 2)/(x − 2) → x + 3 → <b>5</b>.</p></details></div>\n<div class=\"ml-vi\"><span class=\"eyebrow\">Chương 1 · Bài 1.3</span><h2>Khi thay số ra 0/0</h2><p class=\"lead\">Nếu thay a vào mà ra 0/0, đó là <strong>dạng vô định</strong> — đáp án có thể là bất cứ gì cho tới khi bạn rút gọn. Số 0 ở tử và số 0 ở mẫu cùng chung một nhân tử; triệt tiêu nó, rồi thay số.</p><div class=\"formula\"><span class=\"lbl\">Các dạng vô định (phải rút gọn trước)</span>0/0    ∞/∞    ∞ − ∞    0·∞    1<sup>∞</sup>    0⁰    ∞⁰</div><h3>Ba công cụ</h3><div class=\"lz-stack\"><div class=\"lz-layer\"><div class=\"lz-lt\">Phân tích &amp; triệt tiêu</div><div class=\"lz-ld\">Với đa thức: phân tích tử và mẫu, triệt tiêu nhân tử chung (x − a) gây ra số 0.</div></div><div class=\"lz-layer\"><div class=\"lz-lt\">Nhân liên hợp</div><div class=\"lz-ld\">Khi căn gây ra số 0, nhân tử và mẫu với liên hợp để √ ( ) biến thành một hiệu bình thường.</div></div><div class=\"lz-layer\"><div class=\"lz-lt\">Quy đồng phân số</div><div class=\"lz-ld\">Với hiệu của các phân số, quy về mẫu chung trước, rồi triệt tiêu.</div></div></div><h3>Ví dụ có lời giải (từng bước)</h3><div class=\"out\"><b>Ví dụ 1 — phân tích.</b> lim<sub>x→3</sub> (x² − 9)/(x − 3).<br>Thay số → 0/0. Phân tích tử: x² − 9 = (x − 3)(x + 3).<br>= lim<sub>x→3</sub> [(x − 3)(x + 3)]/(x − 3) = lim<sub>x→3</sub> (x + 3) = 3 + 3 = <b>6</b>.</div><div class=\"out\"><b>Ví dụ 2 — nhân liên hợp.</b> lim<sub>x→0</sub> (√(x + 1) − 1)/x.<br>Thay số → 0/0. Nhân tử và mẫu với liên hợp (√(x + 1) + 1):<br>= lim<sub>x→0</sub> [(x + 1) − 1] / [x(√(x + 1) + 1)] = lim<sub>x→0</sub> x/[x(√(x + 1) + 1)]<br>= lim<sub>x→0</sub> 1/(√(x + 1) + 1) = 1/(1 + 1) = <b>1/2</b>.</div><div class=\"out\"><b>Ví dụ 3 — quy đồng phân số.</b> lim<sub>x→0</sub> (1/(x + 2) − 1/2)/x.<br>Tử = [2 − (x + 2)] / [2(x + 2)] = −x / [2(x + 2)].<br>Chia cho x: = lim<sub>x→0</sub> −1/[2(x + 2)] = −1/[2(2)] = <b>−1/4</b>.</div><div class=\"callout\"><b>Lỗi hay gặp.</b> (1) Triệt tiêu (x − a) ở tử và mẫu rồi quên rằng kết quả vẫn là một giới hạn — vẫn phải thay số sau đó. (2) Nhân sai liên hợp (đổi dấu sai) nên không rút gọn được gì. (3) &ldquo;Triệt tiêu&rdquo; các số hạng đang cộng, không phải nhân — chỉ được triệt tiêu <em>nhân tử</em> chung, không bao giờ qua dấu + hay −.</div><h3>Bài tự luyện</h3><p><b>P1.</b> lim<sub>x→4</sub> (x² − 16)/(x − 4).  <b>P2.</b> lim<sub>x→4</sub> (√x − 2)/(x − 4).  <b>P3.</b> lim<sub>x→2</sub> (x² + x − 6)/(x − 2).</p><details><summary>Đáp án</summary><p>P1: (x + 4) → <b>8</b>. P2: nhân liên hợp → 1/(√x + 2) → <b>1/4</b>. P3: (x + 3)(x − 2)/(x − 2) → x + 3 → <b>5</b>.</p></details></div>",
        },
        {
          title: "1.4 — Squeeze theorem &amp; trig limits|||1.4 — Định lý kẹp &amp; giới hạn lượng giác",
          slug: "mae101-1-4-dinh-ly-kep",
          type: 'DOCUMENT',
          description: "Định lý kẹp, lim(sin x)/x = 1, lim(1−cos x)/x = 0 và ví dụ x²sin(1/x).",
          content: "<div class=\"ml-en\"><span class=\"eyebrow\">Chapter 1 · Lesson 1.4</span><h2>The squeeze theorem &amp; trig limits</h2><p class=\"lead\">Some functions oscillate too wildly for factoring. The <strong>squeeze theorem</strong> (also called the sandwich theorem) traps such a function between two others that meet at the same limit — so it has no choice but to go there too.</p><div class=\"formula\"><span class=\"lbl\">Squeeze theorem</span>If g(x) ≤ f(x) ≤ h(x) near a (except possibly at a)<br>and lim<sub>x→a</sub> g(x) = lim<sub>x→a</sub> h(x) = L,   then lim<sub>x→a</sub> f(x) = L</div><h3>The fundamental trig limits</h3><div class=\"formula\"><span class=\"lbl\">Memorise these</span>lim<sub>x→0</sub> (sin x)/x = 1    lim<sub>x→0</sub> (1 − cos x)/x = 0    lim<sub>x→0</sub> (1 − cos x)/x² = 1/2</div><p>The first is proved with the squeeze theorem (cos x ≤ (sin x)/x ≤ 1 for small x, and both ends → 1). Here x is in <strong>radians</strong> — the identity fails in degrees.</p><h3>Worked examples</h3><div class=\"out\"><b>Example 1 — reduce to (sin u)/u.</b> lim<sub>x→0</sub> (sin 3x)/x = lim<sub>x→0</sub> 3 · (sin 3x)/(3x). Let u = 3x → u → 0.<br>= 3 · lim<sub>u→0</sub> (sin u)/u = 3 · 1 = <b>3</b>.</div><div class=\"out\"><b>Example 2 — tan over x.</b> lim<sub>x→0</sub> (tan x)/x = lim<sub>x→0</sub> (sin x)/x · 1/cos x = 1 · 1/cos 0 = 1 · 1 = <b>1</b>.</div><div class=\"out\"><b>Example 3 — squeeze an oscillation.</b> lim<sub>x→0</sub> x² sin(1/x).<br>Since −1 ≤ sin(1/x) ≤ 1, multiply by x² ≥ 0: −x² ≤ x² sin(1/x) ≤ x².<br>lim<sub>x→0</sub> (−x²) = 0 and lim<sub>x→0</sub> x² = 0, so by squeeze the middle → <b>0</b>.</div><div class=\"callout\"><b>Common mistakes.</b> (1) Using (sin x)/x = 1 with x in degrees — it is only 1 in radians. (2) Applying squeeze without checking the bounding functions have the <em>same</em> limit. (3) Splitting the multiplier out wrong: to use (sin 3x)/(3x) you must also multiply by 3 to keep the expression equal.</div><h3>Practice</h3><p><b>P1.</b> lim<sub>x→0</sub> (sin 5x)/(sin 2x).  <b>P2.</b> lim<sub>x→0</sub> (1 − cos x)/x².  <b>P3.</b> Use squeeze to find lim<sub>x→0</sub> x cos(1/x).</p><details><summary>Answers</summary><p>P1: (5x/5x)·(2x/2x) trick → (5/2)·[(sin5x)/(5x)]·[(2x)/(sin2x)] → <b>5/2</b>. P2: <b>1/2</b> (standard limit). P3: −|x| ≤ x cos(1/x) ≤ |x|, both → 0 → <b>0</b>.</p></details></div>\n<div class=\"ml-vi\"><span class=\"eyebrow\">Chương 1 · Bài 1.4</span><h2>Định lý kẹp &amp; giới hạn lượng giác</h2><p class=\"lead\">Vài hàm dao động quá dữ để phân tích. <strong>Định lý kẹp</strong> (còn gọi định lý kẹp bánh mì) nhốt một hàm như vậy giữa hai hàm khác gặp nhau tại cùng một giới hạn — nên nó không còn lựa chọn nào ngoài đi tới đó.</p><div class=\"formula\"><span class=\"lbl\">Định lý kẹp</span>Nếu g(x) ≤ f(x) ≤ h(x) gần a (có thể trừ tại a)<br>và lim<sub>x→a</sub> g(x) = lim<sub>x→a</sub> h(x) = L,   thì lim<sub>x→a</sub> f(x) = L</div><h3>Các giới hạn lượng giác cơ bản</h3><div class=\"formula\"><span class=\"lbl\">Cần nhớ</span>lim<sub>x→0</sub> (sin x)/x = 1    lim<sub>x→0</sub> (1 − cos x)/x = 0    lim<sub>x→0</sub> (1 − cos x)/x² = 1/2</div><p>Giới hạn đầu được chứng minh bằng định lý kẹp (cos x ≤ (sin x)/x ≤ 1 với x nhỏ, và cả hai đầu → 1). Ở đây x tính bằng <strong>radian</strong> — đẳng thức sai nếu dùng độ.</p><h3>Ví dụ có lời giải</h3><div class=\"out\"><b>Ví dụ 1 — đưa về (sin u)/u.</b> lim<sub>x→0</sub> (sin 3x)/x = lim<sub>x→0</sub> 3 · (sin 3x)/(3x). Đặt u = 3x → u → 0.<br>= 3 · lim<sub>u→0</sub> (sin u)/u = 3 · 1 = <b>3</b>.</div><div class=\"out\"><b>Ví dụ 2 — tan chia x.</b> lim<sub>x→0</sub> (tan x)/x = lim<sub>x→0</sub> (sin x)/x · 1/cos x = 1 · 1/cos 0 = 1 · 1 = <b>1</b>.</div><div class=\"out\"><b>Ví dụ 3 — kẹp một dao động.</b> lim<sub>x→0</sub> x² sin(1/x).<br>Vì −1 ≤ sin(1/x) ≤ 1, nhân với x² ≥ 0: −x² ≤ x² sin(1/x) ≤ x².<br>lim<sub>x→0</sub> (−x²) = 0 và lim<sub>x→0</sub> x² = 0, nên theo định lý kẹp phần giữa → <b>0</b>.</div><div class=\"callout\"><b>Lỗi hay gặp.</b> (1) Dùng (sin x)/x = 1 với x tính bằng độ — chỉ bằng 1 khi tính bằng radian. (2) Áp dụng định lý kẹp mà không kiểm hai hàm chặn có <em>cùng</em> giới hạn. (3) Tách hệ số sai: để dùng (sin 3x)/(3x) phải nhân bù thêm 3 để biểu thức không đổi.</div><h3>Bài tự luyện</h3><p><b>P1.</b> lim<sub>x→0</sub> (sin 5x)/(sin 2x).  <b>P2.</b> lim<sub>x→0</sub> (1 − cos x)/x².  <b>P3.</b> Dùng định lý kẹp tìm lim<sub>x→0</sub> x cos(1/x).</p><details><summary>Đáp án</summary><p>P1: mẹo tách → (5/2)·[(sin5x)/(5x)]·[(2x)/(sin2x)] → <b>5/2</b>. P2: <b>1/2</b> (giới hạn chuẩn). P3: −|x| ≤ x cos(1/x) ≤ |x|, cả hai → 0 → <b>0</b>.</p></details></div>",
        },
        {
          title: "1.5 — Limits at infinity &amp; asymptotes|||1.5 — Giới hạn tại vô cực &amp; tiệm cận",
          slug: "mae101-1-5-gioi-han-vo-cuc",
          type: 'DOCUMENT',
          description: "lim x→±∞, tiệm cận ngang/đứng, dạng ∞/∞ và quy tắc so sánh bậc hàm hữu tỉ.",
          content: "<div class=\"ml-en\"><span class=\"eyebrow\">Chapter 1 · Lesson 1.5</span><h2>Limits at infinity &amp; asymptotes</h2><p class=\"lead\">Now let x run off to ∞ (or −∞) and ask where f(x) settles. The answer describes the graph&#39;s long-run behaviour — its <strong>horizontal asymptotes</strong> — while blow-ups to ±∞ at finite points give <strong>vertical asymptotes</strong>.</p><div class=\"formula\"><span class=\"lbl\">Asymptotes from limits</span>Horizontal: if lim<sub>x→±∞</sub> f(x) = L (finite), the line y = L is a horizontal asymptote<br>Vertical: if lim<sub>x→a⁻</sub> f(x) = ±∞ or lim<sub>x→a⁺</sub> f(x) = ±∞, the line x = a is a vertical asymptote</div><h3>The key idea: highest power wins</h3><p>As x → ∞ the biggest-degree term dwarfs the rest. For a rational function, divide top and bottom by the highest power of x in the denominator (a ∞/∞ form), because 1/x → 0, 1/x² → 0, and so on.</p><div class=\"formula\"><span class=\"lbl\">Degree rule for a rational function as x→∞</span>deg(top) &lt; deg(bottom) → 0    deg(top) = deg(bottom) → ratio of leading coefficients    deg(top) &gt; deg(bottom) → ±∞</div><h3>Worked examples</h3><div class=\"out\"><b>Example 1 — equal degrees.</b> lim<sub>x→∞</sub> (3x² + 1)/(x² − 5).<br>Divide top and bottom by x²: = lim<sub>x→∞</sub> (3 + 1/x²)/(1 − 5/x²) = (3 + 0)/(1 − 0) = <b>3</b>. So y = 3 is a horizontal asymptote.</div><div class=\"out\"><b>Example 2 — bottom heavier.</b> lim<sub>x→∞</sub> (2x + 5)/(x² − 1): top degree 1 &lt; bottom degree 2 → <b>0</b>. And lim<sub>x→∞</sub> x³/(x² + 1): top wins → <b>+∞</b>.</div><div class=\"out\"><b>Example 3 — vertical asymptote.</b> f(x) = 1/(x − 2). As x → 2⁺, denominator → 0⁺ so f → +∞; as x → 2⁻, f → −∞. The line <b>x = 2</b> is a vertical asymptote; and lim<sub>x→∞</sub> 1/(x − 2) = 0 gives the horizontal asymptote <b>y = 0</b>.</div><div class=\"callout\"><b>Common mistakes.</b> (1) Writing ∞/∞ = 1 — it is indeterminate, you must divide by the highest power. (2) Ignoring sign: near a vertical asymptote the two sides may go to +∞ and −∞. (3) With a root, remember √(x²) = |x| = −x when x → −∞, which flips a sign.</div><h3>Practice</h3><p><b>P1.</b> lim<sub>x→∞</sub> (5x² − 3x)/(2x² + 7).  <b>P2.</b> lim<sub>x→∞</sub> (x + 1)/(x² + 1).  <b>P3.</b> Find every asymptote of f(x) = x/(x − 3).</p><details><summary>Answers</summary><p>P1: equal degrees → <b>5/2</b>. P2: bottom heavier → <b>0</b>. P3: vertical <b>x = 3</b>; horizontal lim<sub>x→∞</sub> x/(x−3) = <b>y = 1</b>.</p></details></div>\n<div class=\"ml-vi\"><span class=\"eyebrow\">Chương 1 · Bài 1.5</span><h2>Giới hạn tại vô cực &amp; tiệm cận</h2><p class=\"lead\">Giờ cho x chạy ra ∞ (hoặc −∞) và hỏi f(x) ổn định ở đâu. Đáp án mô tả hành vi đường dài của đồ thị — các <strong>tiệm cận ngang</strong> — còn chỗ bùng lên ±∞ tại điểm hữu hạn cho <strong>tiệm cận đứng</strong>.</p><div class=\"formula\"><span class=\"lbl\">Tiệm cận từ giới hạn</span>Ngang: nếu lim<sub>x→±∞</sub> f(x) = L (hữu hạn), đường y = L là tiệm cận ngang<br>Đứng: nếu lim<sub>x→a⁻</sub> f(x) = ±∞ hoặc lim<sub>x→a⁺</sub> f(x) = ±∞, đường x = a là tiệm cận đứng</div><h3>Ý tưởng cốt lõi: bậc cao nhất thắng</h3><p>Khi x → ∞ số hạng bậc cao nhất áp đảo phần còn lại. Với hàm hữu tỉ, chia tử và mẫu cho luỹ thừa cao nhất của x ở mẫu (một dạng ∞/∞), vì 1/x → 0, 1/x² → 0, và cứ thế.</p><div class=\"formula\"><span class=\"lbl\">Quy tắc bậc cho hàm hữu tỉ khi x→∞</span>bậc(tử) &lt; bậc(mẫu) → 0    bậc(tử) = bậc(mẫu) → tỉ số hệ số dẫn đầu    bậc(tử) &gt; bậc(mẫu) → ±∞</div><h3>Ví dụ có lời giải</h3><div class=\"out\"><b>Ví dụ 1 — cùng bậc.</b> lim<sub>x→∞</sub> (3x² + 1)/(x² − 5).<br>Chia tử và mẫu cho x²: = lim<sub>x→∞</sub> (3 + 1/x²)/(1 − 5/x²) = (3 + 0)/(1 − 0) = <b>3</b>. Vậy y = 3 là tiệm cận ngang.</div><div class=\"out\"><b>Ví dụ 2 — mẫu nặng hơn.</b> lim<sub>x→∞</sub> (2x + 5)/(x² − 1): bậc tử 1 &lt; bậc mẫu 2 → <b>0</b>. Và lim<sub>x→∞</sub> x³/(x² + 1): tử thắng → <b>+∞</b>.</div><div class=\"out\"><b>Ví dụ 3 — tiệm cận đứng.</b> f(x) = 1/(x − 2). Khi x → 2⁺, mẫu → 0⁺ nên f → +∞; khi x → 2⁻, f → −∞. Đường <b>x = 2</b> là tiệm cận đứng; và lim<sub>x→∞</sub> 1/(x − 2) = 0 cho tiệm cận ngang <b>y = 0</b>.</div><div class=\"callout\"><b>Lỗi hay gặp.</b> (1) Viết ∞/∞ = 1 — đó là dạng vô định, phải chia cho luỹ thừa cao nhất. (2) Bỏ qua dấu: gần tiệm cận đứng hai phía có thể đi tới +∞ và −∞. (3) Với căn, nhớ √(x²) = |x| = −x khi x → −∞, điều này đổi dấu.</div><h3>Bài tự luyện</h3><p><b>P1.</b> lim<sub>x→∞</sub> (5x² − 3x)/(2x² + 7).  <b>P2.</b> lim<sub>x→∞</sub> (x + 1)/(x² + 1).  <b>P3.</b> Tìm mọi tiệm cận của f(x) = x/(x − 3).</p><details><summary>Đáp án</summary><p>P1: cùng bậc → <b>5/2</b>. P2: mẫu nặng hơn → <b>0</b>. P3: đứng <b>x = 3</b>; ngang lim<sub>x→∞</sub> x/(x−3) = <b>y = 1</b>.</p></details></div>",
        },
        {
          title: "1.6 — Continuity &amp; the Intermediate Value Theorem|||1.6 — Tính liên tục &amp; Định lý giá trị trung gian (IVT)",
          slug: "mae101-1-6-lien-tuc-ivt",
          type: 'DOCUMENT',
          description: "Liên tục tại điểm/khoảng, ba loại gián đoạn, và IVT chứng minh phương trình có nghiệm.",
          content: "<div class=\"ml-en\"><span class=\"eyebrow\">Chapter 1 · Lesson 1.6</span><h2>Continuity &amp; the Intermediate Value Theorem</h2><p class=\"lead\">A function is <strong>continuous</strong> at a if its graph passes through (a, f(a)) with no hole, jump or blow-up — you could draw it there without lifting your pen. Continuity is defined entirely by a limit, so it is the natural payoff of this chapter.</p><div class=\"formula\"><span class=\"lbl\">Continuity at a point — three conditions, all required</span>1) f(a) is defined    2) lim<sub>x→a</sub> f(x) exists    3) lim<sub>x→a</sub> f(x) = f(a)</div><p>f is continuous on an interval if it is continuous at every point of it. Polynomials are continuous everywhere; rational, root, trig, exponential and log functions are continuous on their domains.</p><h3>Three kinds of discontinuity</h3><div class=\"lz-stack\"><div class=\"lz-layer\"><div class=\"lz-lt\">Removable</div><div class=\"lz-ld\">The limit exists but f(a) is missing or wrong — a single hole. Example: (x² − 1)/(x − 1) at x = 1 (limit 2, value undefined). &ldquo;Fixable&rdquo; by defining the point.</div></div><div class=\"lz-layer\"><div class=\"lz-lt\">Jump</div><div class=\"lz-ld\">Left and right limits both exist but differ — a step. Example: a piecewise function whose two pieces do not meet.</div></div><div class=\"lz-layer\"><div class=\"lz-lt\">Infinite</div><div class=\"lz-ld\">A one-sided limit is ±∞ — a vertical asymptote. Example: 1/x at x = 0.</div></div></div><div class=\"formula\"><span class=\"lbl\">Intermediate Value Theorem (IVT)</span>If f is continuous on [a, b] and N is any value between f(a) and f(b),<br>then there exists c ∈ (a, b) with f(c) = N.</div><h3>Worked examples</h3><div class=\"out\"><b>Example 1 — make it continuous.</b> f(x) = { (x² − 1)/(x − 1) if x ≠ 1 ; k if x = 1 }. For continuity we need k = lim<sub>x→1</sub> (x² − 1)/(x − 1) = lim (x + 1) = 2. So <b>k = 2</b>.</div><div class=\"out\"><b>Example 2 — classify.</b> (a) (x² − 4)/(x − 2) at x = 2: limit = 4 exists but value undefined → <b>removable</b>. (b) f(x) = { x, x &lt; 0 ; x + 1, x ≥ 0 } at 0: left 0, right 1 → <b>jump</b>. (c) 1/x² at 0 → +∞ → <b>infinite</b>.</div><div class=\"out\"><b>Example 3 — IVT proves a root.</b> Show x³ + x − 1 = 0 has a solution in (0, 1). Let f(x) = x³ + x − 1 (continuous). f(0) = −1 &lt; 0, f(1) = 1 &gt; 0. Since 0 lies between −1 and 1, by IVT there is c ∈ (0, 1) with f(c) = 0. <b>A root exists.</b></div><div class=\"callout\"><b>Common mistakes.</b> (1) Checking only that f(a) is defined — continuity needs all three conditions, including the limit equalling the value. (2) Applying IVT without confirming f is continuous on the whole closed interval. (3) Thinking IVT tells you <em>where</em> the root is — it only guarantees one <em>exists</em>.</div><h3>Practice</h3><p><b>P1.</b> Find k so f(x) = { (x² − 9)/(x − 3), x ≠ 3 ; k, x = 3 } is continuous.<br><b>P2.</b> Classify the discontinuity of f(x) = |x|/x at x = 0.<br><b>P3.</b> Use IVT to show cos x = x has a solution in (0, 1).</p><details><summary>Answers / hints</summary><p>P1: limit = x + 3 → <b>k = 6</b>. P2: left −1, right +1, both finite but unequal → <b>jump</b>. P3: g(x) = cos x − x is continuous; g(0) = 1 &gt; 0, g(1) = cos 1 − 1 ≈ −0.46 &lt; 0 → a root in (0, 1).</p></details></div>\n<div class=\"ml-vi\"><span class=\"eyebrow\">Chương 1 · Bài 1.6</span><h2>Tính liên tục &amp; Định lý giá trị trung gian (IVT)</h2><p class=\"lead\">Một hàm <strong>liên tục</strong> tại a nếu đồ thị đi qua điểm (a, f(a)) mà không có lỗ, chỗ nhảy hay chỗ bùng nổ — bạn vẽ được ở đó mà không nhấc bút. Tính liên tục được định nghĩa hoàn toàn bằng giới hạn, nên nó là thành quả tự nhiên của chương này.</p><div class=\"formula\"><span class=\"lbl\">Liên tục tại một điểm — ba điều kiện, đủ cả ba</span>1) f(a) xác định    2) lim<sub>x→a</sub> f(x) tồn tại    3) lim<sub>x→a</sub> f(x) = f(a)</div><p>f liên tục trên một khoảng nếu nó liên tục tại mọi điểm của khoảng đó. Đa thức liên tục ở mọi nơi; hàm hữu tỉ, căn, lượng giác, mũ và logarit liên tục trên miền xác định của chúng.</p><h3>Ba loại gián đoạn</h3><div class=\"lz-stack\"><div class=\"lz-layer\"><div class=\"lz-lt\">Khử được (removable)</div><div class=\"lz-ld\">Giới hạn tồn tại nhưng f(a) khuyết hoặc sai — một cái lỗ đơn. Ví dụ: (x² − 1)/(x − 1) tại x = 1 (giới hạn 2, giá trị không xác định). &ldquo;Vá được&rdquo; bằng cách định nghĩa lại điểm đó.</div></div><div class=\"lz-layer\"><div class=\"lz-lt\">Nhảy (jump)</div><div class=\"lz-ld\">Giới hạn trái và phải đều tồn tại nhưng khác nhau — một bậc thang. Ví dụ: hàm từng khúc mà hai khúc không gặp nhau.</div></div><div class=\"lz-layer\"><div class=\"lz-lt\">Vô hạn (infinite)</div><div class=\"lz-ld\">Một giới hạn một phía là ±∞ — một tiệm cận đứng. Ví dụ: 1/x tại x = 0.</div></div></div><div class=\"formula\"><span class=\"lbl\">Định lý giá trị trung gian (IVT)</span>Nếu f liên tục trên [a, b] và N là giá trị bất kỳ nằm giữa f(a) và f(b),<br>thì tồn tại c ∈ (a, b) sao cho f(c) = N.</div><h3>Ví dụ có lời giải</h3><div class=\"out\"><b>Ví dụ 1 — làm cho liên tục.</b> f(x) = { (x² − 1)/(x − 1) nếu x ≠ 1 ; k nếu x = 1 }. Để liên tục cần k = lim<sub>x→1</sub> (x² − 1)/(x − 1) = lim (x + 1) = 2. Vậy <b>k = 2</b>.</div><div class=\"out\"><b>Ví dụ 2 — phân loại.</b> (a) (x² − 4)/(x − 2) tại x = 2: giới hạn = 4 tồn tại nhưng giá trị không xác định → <b>khử được</b>. (b) f(x) = { x, x &lt; 0 ; x + 1, x ≥ 0 } tại 0: trái 0, phải 1 → <b>nhảy</b>. (c) 1/x² tại 0 → +∞ → <b>vô hạn</b>.</div><div class=\"out\"><b>Ví dụ 3 — IVT chứng minh có nghiệm.</b> Chứng minh x³ + x − 1 = 0 có nghiệm trong (0, 1). Đặt f(x) = x³ + x − 1 (liên tục). f(0) = −1 &lt; 0, f(1) = 1 &gt; 0. Vì 0 nằm giữa −1 và 1, theo IVT tồn tại c ∈ (0, 1) với f(c) = 0. <b>Nghiệm tồn tại.</b></div><div class=\"callout\"><b>Lỗi hay gặp.</b> (1) Chỉ kiểm f(a) có xác định — liên tục cần đủ ba điều kiện, gồm cả giới hạn bằng giá trị. (2) Dùng IVT mà không xác nhận f liên tục trên cả đoạn đóng. (3) Tưởng IVT cho biết nghiệm <em>ở đâu</em> — nó chỉ bảo đảm nghiệm <em>tồn tại</em>.</div><h3>Bài tự luyện</h3><p><b>P1.</b> Tìm k để f(x) = { (x² − 9)/(x − 3), x ≠ 3 ; k, x = 3 } liên tục.<br><b>P2.</b> Phân loại điểm gián đoạn của f(x) = |x|/x tại x = 0.<br><b>P3.</b> Dùng IVT chứng minh cos x = x có nghiệm trong (0, 1).</p><details><summary>Đáp án / gợi ý</summary><p>P1: giới hạn = x + 3 → <b>k = 6</b>. P2: trái −1, phải +1, đều hữu hạn nhưng khác nhau → <b>nhảy</b>. P3: g(x) = cos x − x liên tục; g(0) = 1 &gt; 0, g(1) = cos 1 − 1 ≈ −0,46 &lt; 0 → có nghiệm trong (0, 1).</p></details></div>",
        },
        {
