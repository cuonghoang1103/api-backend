...
export default {
  ...
  course: {
    ...
    shortDescription: 'Communication & teamwork for software engineers — the communication model, active listening, writing, presenting, team roles (Belbin) & stages (Tuckman), conflict (Thomas-Kilmann), leadership, effective meetings, and cross-cultural, ethical communication with constructive feedback. Bilingual, with real situations & quizzes.|||Kỹ năng giao tiếp & làm việc nhóm cho kỹ sư phần mềm — mô hình giao tiếp, lắng nghe chủ động, viết, thuyết trình, vai trò nhóm (Belbin) & giai đoạn (Tuckman), xung đột (Thomas-Kilmann), lãnh đạo, họp hiệu quả, giao tiếp đa văn hoá & đạo đức kèm phản hồi xây dựng. Song ngữ, có tình huống thực & quiz.',
