...
const QUIZ3 = {
  ...
  quiz: {
    ...
    questions: [
      qx('Which TWO statements about static testing are MOST true? A cheap way to detect and remove defects · B makes dynamic testing less challenging · C early validation of user requirements · D finds run-time problems early · E has less value for safety-critical systems. (SWT3 s.15)|||Hai câu nào về kiểm thử tĩnh ĐÚNG NHẤT? A cách rẻ để tìm và gỡ defect · B làm kiểm thử động bớt thách thức · C validate yêu cầu người dùng từ sớm · D tìm sớm vấn đề lúc chạy · E kém giá trị với hệ thống an toàn-sống-còn. (SWT3 s.15)', ['A and C|||A và C', 'A and D|||A và D', 'B and C|||B và C', 'D and E|||D và E'], 0, 'A is true because a defect found in a document is cheap to find and fix, and C is true because reviewing requirements with users and testers validates them before anything is built (SWT3 slide 15). D is the tempting one, but run-time problems need the code to execute, which static testing never does.|||A đúng vì defect tìm thấy trong tài liệu thì rẻ để phát hiện và sửa, C đúng vì review yêu cầu cùng user và tester giúp validate chúng trước khi xây gì cả (SWT3 slide 15). D dễ bị chọn nhầm, nhưng vấn đề run-time cần chạy code, điều static testing không bao giờ làm.'),
      qx('Which is a correct definition of a review? (s.16)|||Định nghĩa đúng của review? (s.16)', ['An automated activity reviewing requirements, code and design to find defects|||Hoạt động tự động review yêu cầu, code, thiết kế để tìm defect', 'A mostly manual activity in which work products like requirements, code and design are reviewed to find defects|||Hoạt động chủ yếu thủ công xem xét các sản phẩm như yêu cầu, code, thiết kế để tìm defect', 'An automated activity reviewing code to find warnings and errors|||Hoạt động tự động xem code để tìm cảnh báo và lỗi', 'A manual activity examining work products to prove there is no defect|||Hoạt động thủ công xem xét sản phẩm để chứng minh không có defect'], 1, 'A review is a mostly manual examination of work products such as requirements, code and design, to find defects in them. A and C describe static analysis, which is tool-driven; D fails because no testing, static or dynamic, can prove that there are no defects (Principle 1).|||Review là việc chủ yếu thủ công xem xét các work product như yêu cầu, code, thiết kế để tìm defect. A và C mô tả static analysis (dùng công cụ); D sai vì không loại testing nào, tĩnh hay động, chứng minh được là không còn defect (Principle 1).'),
      qx('Which of the following is correct? (s.17)|||Câu nào đúng? (s.17)', ['Both reviews and static analysis are tool-driven|||Cả review và static analysis đều dùng công cụ', 'Both can be done either manually or by tools|||Cả hai đều làm được thủ công hoặc bằng công cụ', 'Reviews rely on manual examination while static analysis is tool-driven|||Review dựa vào con người xem xét, static analysis dùng công cụ', 'Static analysis is manual while reviews are tool-driven|||Static analysis thủ công, review dùng công cụ'], 2, 'Reviews rely on people examining the work product, while static analysis is done by tools. Review tools do exist, but they only help manage comments and logs; the examination itself stays manual, so B (both can be either) is wrong.|||Review dựa vào con người xem xét work product, còn static analysis do công cụ thực hiện. Có công cụ hỗ trợ review, nhưng chúng chỉ giúp quản lý góp ý và log; việc xem xét vẫn là thủ công, nên B (cả hai đều làm được theo hai cách) sai.'),
      qx('A defect found in requirements or design costs (…) the same defect found in production. (s.18)|||Defect tìm ở yêu cầu hay thiết kế tốn (…) cùng defect đó tìm ở production. (s.18)', ['more than|||nhiều hơn', 'less than|||ít hơn', 'the same as|||bằng'], 1, 'The earlier a defect is found, the cheaper it is to fix: about 1x in requirements or design versus up to 100x after release (SWT3 slide 8). This cost difference is the economic core of static testing and of the early-testing principle.|||Defect được tìm càng sớm thì sửa càng rẻ: khoảng 1x ở giai đoạn yêu cầu hay thiết kế so với tới 100x sau khi phát hành (SWT3 slide 8). Chênh lệch chi phí này là cốt lõi kinh tế của static testing và của nguyên tắc early testing.'),
      qx('Which technique is a form of static testing? (s.19)|||Kỹ thuật nào là kiểm thử tĩnh? (s.19)', ['Error guessing', 'Automated regression testing|||Regression test tự động', 'Providing inputs and examining outputs|||Đưa đầu vào và xem đầu ra', 'Code review|||Review code'], 3, 'Reading code without executing it is static testing, so a code review is the static technique. Error guessing is an experience-based technique for designing dynamic tests, and automated regression testing and giving inputs to check outputs both run the code.|||Đọc code mà không chạy là static testing, nên code review là kỹ thuật tĩnh. Error guessing là kỹ thuật dựa kinh nghiệm để thiết kế test động, còn regression test tự động và đưa input để xem output đều phải chạy code.'),
      qx('Which is a benefit of static analysis? (s.20)|||Đâu là lợi ích của static analysis? (s.20)', ['Early defect identification requires less documentation|||Tìm defect sớm cần ít tài liệu hơn', 'Defects can be identified that might not be caught by dynamic testing|||Tìm được defect mà kiểm thử động có thể bỏ sót', 'Early execution of the code gauges code quality|||Chạy code sớm để đo chất lượng', 'Tools are not needed because reviews replace execution|||Không cần công cụ vì review thay cho việc chạy code'], 1, 'Static analysis examines code without running it, so it finds defects such as unreachable code, buffer overflows or variables used before they are defined, which dynamic tests may never trigger (slide 9). C is impossible because static analysis never executes the code.|||Static analysis xem code mà không chạy, nên tìm được defect như unreachable code, buffer overflow hay biến dùng trước khi được định nghĩa, những thứ test động có thể không bao giờ kích hoạt (slide 9). C không thể đúng vì static analysis không chạy code.'),
      qx('If reviews are categorised into two types, they are… (s.32)|||Nếu chia review thành hai loại, đó là… (s.32)', ['static and dynamic review|||review tĩnh và động', 'experience-based and metrics-based review|||review dựa kinh nghiệm và dựa số liệu', 'formal and informal review|||review chính quy và không chính quy', 'official and unofficial review|||review chính thức và không chính thức'], 2, 'SWT3 slide 23 divides reviews into two kinds: formal and informal, depending on how defined the process and its documented output are. Static/dynamic is wrong because a review is always static, and experience-based/metrics-based are names of technique and estimation groups, not review types.|||SWT3 slide 23 chia review thành hai loại: formal và informal, tùy mức độ xác định của quy trình và kết quả được ghi lại. Static/dynamic sai vì review luôn là tĩnh, còn experience-based/metrics-based là tên nhóm kỹ thuật và ước lượng, không phải loại review.'),
      qx('Which are the correct steps of the review process? (s.33)|||Các bước đúng của quy trình review? (s.33)', ['Planning, monitoring, analysis, design, executing', 'Planning, initial review, technical review, checklist based, roles', 'Planning, initiate review, individual review, issue communication and analysis, fixing and reporting', 'Planning, initial review, executing, technical review, fixing and reporting'], 2, 'The CTFL 2018 review process has five activities in this order: planning, initiate review, individual review, issue communication and analysis, fixing and reporting (slide 25). A lists test-process activities, while B and D mix in a review type (technical review), a technique or executing.|||Quy trình review CTFL 2018 có năm hoạt động theo thứ tự: planning, initiate review, individual review, issue communication and analysis, fixing and reporting (slide 25). A là các hoạt động của test process, còn B và D trộn vào một loại review (technical review), một kỹ thuật hoặc executing.'),
      qx('Which occurs in the Planning step of the review process? (s.34)|||Việc nào thuộc bước Planning của quy trình review? (s.34)', ['Distributing the work product|||Phát sản phẩm cần review', 'Defining scope and estimating effort|||Xác định phạm vi và ước lượng công sức', 'Noting potential defects|||Ghi defect tiềm năng', 'Logging defects and issues|||Ghi nhận defect và vấn đề'], 1, 'Planning defines the scope (purpose, documents, quality characteristics), estimates effort and timeframe and selects review type, roles and entry/exit criteria (slide 26). Distributing the work product belongs to Initiate review, noting potential defects to Individual review.|||Planning xác định phạm vi (mục đích, tài liệu, đặc tính chất lượng), ước lượng công sức và thời gian, chọn loại review, vai trò và entry/exit criteria (slide 26). Phát work product thuộc Initiate review, ghi defect tiềm năng thuộc Individual review.'),
      qx('Difference between entry and exit criteria in the review process? (s.35)|||Khác biệt giữa entry và exit criteria trong quy trình review? (s.35)', ['Entry = goals to achieve; exit = prerequisites to begin|||Entry = mục tiêu cần đạt; exit = điều kiện để bắt đầu', 'Entry = prerequisites that might be met to begin; exit = goals to achieve|||Entry = điều kiện có thể đạt để bắt đầu; exit = mục tiêu cần đạt', 'Entry = prerequisites that must be met to begin; exit = goals to achieve|||Entry = điều kiện phải đạt để bắt đầu; exit = mục tiêu cần đạt'], 2, 'Entry criteria are prerequisites that must be met before the review can begin; exit criteria are the goals to achieve before it can end. B differs by one word: a criterion that only might be met is not a criterion. A swaps the two.|||Entry criteria là điều kiện tiên quyết phải đạt trước khi bắt đầu review; exit criteria là mục tiêu cần đạt để kết thúc. B chỉ khác một chữ: tiêu chí chỉ có thể đạt thì không còn là tiêu chí. A đảo ngược hai khái niệm.'),
      qx('Which occurs in the "Initiate review" step? (s.36)|||Việc nào thuộc bước "Initiate review"? (s.36)', ['Evaluating and documenting quality characteristics|||Đánh giá và ghi lại đặc tính chất lượng', 'Gathering metrics|||Thu thập metric', 'Answering participants\' questions about the review|||Trả lời câu hỏi của người tham gia về buổi review', 'Identifying review characteristics|||Xác định đặc điểm review'], 2, 'Initiate review means distributing the work product, explaining scope, objectives, process and roles, and answering participants\' questions (slide 28). Gathering metrics belongs to Fixing and reporting, and evaluating quality characteristics to Issue communication and analysis.|||Initiate review là phát work product, giải thích phạm vi, mục tiêu, quy trình, vai trò và trả lời câu hỏi của người tham gia (slide 28). Thu thập metric thuộc Fixing and reporting, đánh giá đặc tính chất lượng thuộc Issue communication and analysis.'),
      qx('Reviewing all or part of the work product and noting potential defects and questions occur in… (s.37)|||Đọc toàn bộ/một phần sản phẩm, ghi defect tiềm năng và câu hỏi thuộc bước… (s.37)', ['Initiate review', 'Individual review', 'Planning', 'Issue communication & analysis'], 1, 'In Individual review each reviewer works alone, reviewing all or part of the work product and noting potential defects, recommendations and questions (slide 29). Issue communication and analysis comes afterwards, when these notes are shared and analysed.|||Ở Individual review, mỗi reviewer làm việc một mình, đọc toàn bộ hoặc một phần work product và ghi defect tiềm năng, đề xuất, câu hỏi (slide 29). Issue communication and analysis đến sau, khi các ghi chú này được chia sẻ và phân tích.'),
      qx('"Evaluating the review findings against the exit criteria to make a review decision" happens in… (s.38)|||"So kết quả review với exit criteria để ra quyết định" diễn ra ở… (s.38)', ['Fixing and reporting', 'Initiate review', 'Individual review (individual preparation)', 'Issue communication & analysis'], 3, 'The review decision (reject, accept with major changes, accept) is made by evaluating the findings against the exit criteria in Issue communication and analysis (slide 30). Fixing and reporting is tempting but comes later: it handles rework and finally accepts the product.|||Quyết định review (từ chối, chấp nhận kèm sửa lớn, chấp nhận) được đưa ra khi so kết quả với exit criteria ở Issue communication and analysis (slide 30). Fixing and reporting dễ bị chọn nhầm nhưng đến sau: nó lo sửa lỗi và cuối cùng chấp nhận sản phẩm.'),
      qx('"Accepting the work product when the exit criteria are reached" happens in… (s.39)|||"Chấp nhận sản phẩm khi đạt exit criteria" diễn ra ở… (s.39)', ['Issue communication and analysis', 'Individual review', 'Fixing and reporting', 'Initiate review'], 2, 'Accepting the work product once the exit criteria are reached is the very last task of the process, in Fixing and reporting (slide 31). Issue communication and analysis only makes the review decision; acceptance follows after the rework is done.|||Chấp nhận work product khi đã đạt exit criteria là việc cuối cùng của quy trình, thuộc Fixing and reporting (slide 31). Issue communication and analysis chỉ ra quyết định review; việc chấp nhận diễn ra sau khi đã sửa xong.'),
      qx('Who creates the work product under review? (s.47)|||Ai tạo ra sản phẩm được review? (s.47)', ['Reviewer', 'Manager', 'Author', 'Facilitator'], 2, 'By definition the author creates the work product under review and fixes its defects (slide 41). The facilitator runs the meeting, the manager provides resources and the reviewers find defects.|||Theo định nghĩa, author là người tạo ra work product được review và sửa defect của nó (slide 41). Facilitator điều hành buổi họp, manager cấp nguồn lực, reviewer tìm defect.'),
      qx('In a review of the code of a mobile app, who is the author? (s.48)|||Review code của một ứng dụng di động, ai là tác giả? (s.48)', ['The tester|||Tester', 'The developer|||Developer', 'The product owner|||Product owner', 'The scrum master|||Scrum master'], 1, 'The author is whoever created the work product under review, and code is written by the developer (notes of slide 41). A tester would be the author of test cases and a product owner the author of user stories.|||Author là người tạo ra work product được review, mà code do developer viết (ghi chú slide 41). Tester sẽ là author của test case, còn product owner là author của user story.'),
      qx('Responsible for review planning, assigning staff and budget, monitoring cost-effectiveness: (s.49)|||Chịu trách nhiệm lập kế hoạch review, cấp người và ngân sách, theo dõi hiệu quả chi phí: (s.49)', ['the reviewer', 'the facilitator', 'the author', 'the manager'], 3, 'Planning the review, assigning staff, budget and time and monitoring cost-effectiveness are the manager\'s duties, word for word on slide 42. The facilitator runs meetings but does not assign budget.|||Lập kế hoạch review, cấp người, ngân sách, thời gian và theo dõi hiệu quả chi phí là nhiệm vụ của manager, đúng từng chữ trên slide 42. Facilitator điều hành buổi họp nhưng không cấp ngân sách.'),
      qx('Who ensures the review meeting is run effectively? (s.50)|||Ai đảm bảo buổi họp review diễn ra hiệu quả? (s.50)', ['the reviewer', 'the facilitator', 'the manager', 'the author'], 1, 'The facilitator (often called moderator) ensures review meetings run effectively and mediates between the different points of view (slide 43). The manager decides that reviews take place, not how the meeting is run.|||Facilitator (thường gọi là moderator) đảm bảo buổi họp review diễn ra hiệu quả và dung hòa các quan điểm khác nhau (slide 43). Manager quyết định có review hay không, không lo việc điều hành buổi họp.'),
      qx('Who takes overall responsibility for the review? (s.51)|||Ai chịu trách nhiệm tổng thể cho buổi review? (s.51)', ['The scribe', 'The author', 'The moderator', 'The review leader'], 3, 'The review leader takes overall responsibility for the review and decides who is involved and where and when it takes place (slide 44). The tempting moderator (facilitator) only runs the meeting.|||Review leader chịu trách nhiệm tổng thể cho buổi review, quyết định ai tham gia, ở đâu và khi nào (slide 44). Moderator (facilitator) dễ bị chọn nhầm nhưng chỉ điều hành buổi họp.'),
      qx('Who collates potential defects found during individual review? (s.52)|||Ai tổng hợp defect tiềm năng tìm được ở bước review cá nhân? (s.52)', ['the moderator', 'the scribe', 'the author', 'the reviewer'], 1, 'The scribe (recorder) collates the potential defects found during individual review and records new defects, open points and decisions in the meeting (slide 46). Reviewers find defects individually; they do not collate them.|||Scribe (thư ký) tổng hợp các defect tiềm năng tìm được ở bước review cá nhân và ghi lại defect mới, điểm còn mở và quyết định trong buổi họp (slide 46). Reviewer tự tìm defect, không phải người tổng hợp.'),
      qx('Which review type aims at quickly solving minor problems? (s.73)|||Loại review nào nhằm giải quyết nhanh vấn đề nhỏ? (s.73)', ['Informal review (pairing)', 'Walkthrough', 'Technical review', 'Inspection'], 0, 'Quickly solving minor problems is a possible additional purpose of the informal review, e.g. pair review or pairing (slide 56). Walkthrough, technical review and inspection all involve preparation or a meeting, so none of them is quick.|||Giải quyết nhanh vấn đề nhỏ là mục đích phụ có thể có của informal review, ví dụ pair review hay pairing (slide 56). Walkthrough, technical review và inspection đều cần chuẩn bị hoặc họp, nên không loại nào nhanh.'),
      qx('The informal review is called a "buddy check" because it is performed by… (s.74)|||Informal review được gọi là "buddy check" vì do… thực hiện (s.74)', ['a colleague of the facilitator|||đồng nghiệp của facilitator', 'a colleague of the author|||đồng nghiệp của tác giả', 'a colleague of the review leader|||đồng nghiệp của review leader', 'a colleague of the reviewers|||đồng nghiệp của reviewer'], 1, 'In a buddy check the author asks a colleague, a buddy, to look over the work (slide 57). Informal reviews have no facilitator or review leader, so options about their colleagues make no sense.|||Trong buddy check, author nhờ một đồng nghiệp (buddy) xem giúp sản phẩm (slide 57). Informal review không có facilitator hay review leader, nên các lựa chọn về đồng nghiệp của họ không có nghĩa.'),
      qx('Main purposes of the … review type include improving the software product, considering alternative implementations and finding defects. (SWT3 hidden pptx s.75)|||Mục đích chính của loại review … gồm cải thiện sản phẩm, xem xét phương án cài đặt khác và tìm defect. (SWT3 slide ẩn pptx 75)', ['Formal', 'Informal', 'Walkthrough', 'Technical'], 2, 'Improving the product, considering alternative implementations and finding defects are the main purposes of the walkthrough (slide 58). Technical is the tempting distractor because it may also consider alternatives, but its main purposes are gaining consensus and detecting defects.|||Cải thiện sản phẩm, xem xét phương án cài đặt khác và tìm defect là mục đích chính của walkthrough (slide 58). Technical dễ bị chọn nhầm vì nó cũng có thể xem xét phương án khác, nhưng mục đích chính của nó là đạt đồng thuận và tìm defect.'),
      qx('In a walkthrough the … is mandatory and the meeting is led by the … (s.75)|||Trong walkthrough … là bắt buộc và buổi họp do … dẫn (s.75)', ['Moderator / Facilitator', 'Manager / Reviewer', 'Scribe / Author of the work product|||Scribe / Tác giả của sản phẩm', 'Scribe / Moderator'], 2, 'In a walkthrough the scribe is the only mandatory role and the meeting is led by the author of the work product (slide 59). A moderator or facilitator as leader describes a technical review or an inspection.|||Trong walkthrough, scribe là vai trò bắt buộc duy nhất và buổi họp do author của work product dẫn (slide 59). Moderator hay facilitator dẫn họp là đặc điểm của technical review hoặc inspection.'),
      qx('Purposes include detecting defects, building confidence and preventing future similar defects through author learning and root cause analysis: (s.76)|||Mục đích gồm tìm defect, tạo niềm tin và ngăn defect tương tự nhờ tác giả rút kinh nghiệm và phân tích nguyên nhân gốc: (s.76)', ['Formal', 'Inspection', 'Walkthrough', 'Technical review'], 1, 'Preventing future similar defects through author learning and root cause analysis is a purpose that only the inspection has (slide 62). A technical review aims at consensus and defect detection, a walkthrough at improving the product and considering alternatives.|||Ngăn defect tương tự trong tương lai nhờ author rút kinh nghiệm và phân tích nguyên nhân gốc là mục đích chỉ inspection có (slide 62). Technical review nhắm tới đồng thuận và tìm defect, walkthrough nhắm tới cải thiện sản phẩm và xem xét phương án khác.'),
      qx('In a technical review, the … are mandatory, and the technical peers of the author are the … (s.77)|||Trong technical review, … là bắt buộc, và đồng nghiệp kỹ thuật của tác giả là … (s.77)', ['individual preparation and author / moderators', 'individual preparation and scribe / reviewers', 'product owner and facilitator / managers', 'checklists and scribe / reviewers'], 1, 'In a technical review individual preparation and a scribe (ideally not the author) are mandatory, and the reviewers are technical peers of the author (slide 61). Checklists are only optional there, and the peers are reviewers, not moderators.|||Trong technical review, chuẩn bị cá nhân và scribe (tốt nhất không phải author) là bắt buộc, còn reviewer là đồng nghiệp kỹ thuật của author (slide 61). Checklist chỉ là tùy chọn, và đồng nghiệp là reviewer chứ không phải moderator.'),
      qx('When … are done properly they also prevent future similar defects and improve the process through author learning and RCA. (s.78)|||Khi … được làm đúng, chúng còn ngăn defect tương tự và cải tiến quy trình nhờ tác giả rút kinh nghiệm và RCA. (s.78)', ['Informal', 'Inspection', 'Walkthrough', 'Technical'], 1, 'Same idea as slide 76: done properly, inspections not only find defects but also prevent similar ones and improve the process through author learning and root cause analysis (slide 62). No other review type lists this purpose.|||Cùng ý với slide 76: làm đúng cách, inspection không chỉ tìm defect mà còn ngăn defect tương tự và cải tiến quy trình nhờ author rút kinh nghiệm và phân tích nguyên nhân gốc (slide 62). Không loại review nào khác có mục đích này.'),
      qx('Which review type has a defined process, individual preparation and a scribe, and produces defect logs and a review report? (s.79)|||Loại review nào có quy trình xác định, chuẩn bị cá nhân, scribe, và tạo defect log cùng review report? (s.79)', ['Walkthrough review', 'Technical review', 'Informal review', 'Inspection review'], 3, 'Only the inspection combines a defined process with mandatory individual preparation, a scribe, defect logs and a review report (slide 62). In a technical review logs and reports are optional, and a walkthrough only requires a scribe.|||Chỉ inspection có quy trình xác định cùng chuẩn bị cá nhân, scribe, defect log và review report bắt buộc (slide 62). Ở technical review, log và report chỉ là tùy chọn, còn walkthrough chỉ bắt buộc có scribe.'),
      qx('In an inspection, the review meeting is led by the… (s.80)|||Trong inspection, buổi họp review do… dẫn (s.80)', ['Facilitator', 'Author', 'Manager', 'Reviewer'], 0, 'An inspection meeting is led by a trained facilitator, also called moderator (slide 63). The author may not be leader, reader or scribe in an inspection, and managers or reviewers do not lead it.|||Buổi họp inspection do facilitator được đào tạo (còn gọi là moderator) dẫn (slide 63). Trong inspection, author không được làm leader, reader hay scribe, còn manager hay reviewer cũng không dẫn họp.'),
      qx('Reviews done by colleagues at a similar organisational level are… (s.81)|||Review do đồng nghiệp cùng cấp bậc tổ chức thực hiện gọi là… (s.81)', ['Formal reviews', 'Peer reviews', 'Technical reviews', 'Inspection reviews'], 1, 'A review done by colleagues at about the same organisational level is a peer review; walkthroughs, technical reviews and inspections can all be done as peer reviews. Technical and inspection are specific review types, and formal describes the process, not who reviews.|||Review do đồng nghiệp ở cùng cấp bậc tổ chức thực hiện gọi là peer review; walkthrough, technical review và inspection đều có thể làm dưới dạng peer review. Technical và inspection là loại review cụ thể, còn formal nói về quy trình, không nói ai review.'),
      qx('… is a systematic technique using a set of questions, distributed at review initiation, based on potential defects. (s.88)|||… là kỹ thuật có hệ thống dùng bộ câu hỏi dựa trên defect tiềm năng, phát ở bước initiate. (s.88)', ['Ad hoc', 'Checklist-based', 'Scenario and dry runs', 'Role based'], 1, 'Checklist-based reviewing is the systematic technique that uses a set of questions based on potential defects, distributed at review initiation (slide 84). Ad hoc gives no guidance, scenarios give reading guidelines and role-based gives stakeholder viewpoints.|||Checklist-based là kỹ thuật có hệ thống dùng bộ câu hỏi dựa trên defect tiềm năng, phát ra ở bước initiate review (slide 84). Ad hoc không có hướng dẫn, scenario cho hướng dẫn cách đọc, còn role-based cho góc nhìn của stakeholder.'),
      qx('Role-based review as a senior citizen, reviewing the UI of a mobile banking prototype: which area should you review? (s.89)|||Review theo vai người cao tuổi, xem giao diện bản mẫu ngân hàng di động: bạn nên review phần nào? (s.89)', ['Speed of response from the banking backend|||Tốc độ phản hồi của backend', 'Attractiveness of the application|||Độ hấp dẫn của ứng dụng', 'Size and clarity of the instruction text|||Cỡ chữ và độ rõ ràng của hướng dẫn', 'Reliability when the connection drops|||Độ tin cậy khi mất kết nối'], 2, 'Role-based review takes the viewpoint of one stakeholder; for a senior citizen looking at the UI, the key concern is readable, clear instruction text. Backend speed and dropped connections are not UI issues, and attractiveness matters to every user, not especially to this role.|||Role-based review đứng ở góc nhìn của một stakeholder; với người cao tuổi xem UI, điều quan trọng là chữ hướng dẫn đủ to và rõ. Tốc độ backend và mất kết nối không phải vấn đề UI, còn độ hấp dẫn quan trọng với mọi user chứ không riêng vai này.'),
      qx('… is used when we depend mainly on reviewer skills, with little or no planning and preparation. (s.90)|||… dùng khi phụ thuộc chủ yếu vào tay nghề người review, ít hoặc không lập kế hoạch, ít chuẩn bị. (s.90)', ['Ad hoc', 'Checklist-based', 'Scenario and dry run', 'Role based'], 0, 'In ad hoc reviewing the reviewers get little or no guidance and little preparation, and read sequentially, so the result depends mainly on reviewer skill (slide 83). Checklist, scenario and role-based techniques all give the reviewer structure.|||Với ad hoc, reviewer gần như không có hướng dẫn, ít chuẩn bị và đọc tuần tự, nên kết quả phụ thuộc chủ yếu vào tay nghề reviewer (slide 83). Checklist, scenario và role-based đều cho reviewer một cấu trúc để làm theo.'),
      qx('Main difference between a walkthrough and an inspection? (s.91)|||Khác biệt chính giữa walkthrough và inspection? (s.91)', ['An inspection is led by the authors, a walkthrough by a trained moderator|||Inspection do tác giả dẫn, walkthrough do moderator được đào tạo dẫn', 'An inspection has a trained leader, a walkthrough has no leader|||Inspection có leader được đào tạo, walkthrough không có leader', 'Authors are absent in inspections but present in walkthroughs|||Tác giả vắng mặt trong inspection nhưng có mặt trong walkthrough', 'A walkthrough is led by the author, an inspection by a trained moderator|||Walkthrough do tác giả dẫn, inspection do moderator được đào tạo dẫn'], 3, 'The key difference is who leads: the author leads a walkthrough, a trained moderator (facilitator) leads an inspection. A swaps the leaders, B is wrong because a walkthrough does have a leader, and C is wrong because the author does attend an inspection.|||Khác biệt chính là ai dẫn: author dẫn walkthrough, moderator (facilitator) được đào tạo dẫn inspection. A đảo ngược hai người dẫn, B sai vì walkthrough vẫn có người dẫn, C sai vì author vẫn có mặt trong inspection.'),
      qx('Slide 102: a flow graph contains four decision diamonds. Its cyclomatic complexity is…|||Slide 102: một đồ thị luồng có bốn hình thoi quyết định. Độ phức tạp cyclomatic là…', ['3', '4', '5', '6'], 2, 'Cyclomatic complexity = number of decisions + 1, so 4 diamonds give 4 + 1 = 5 (slide 102, right-hand graph). Answering 4 forgets the +1; boxes and join points never add complexity.|||Cyclomatic complexity = số quyết định + 1, nên 4 hình thoi cho 4 + 1 = 5 (slide 102, đồ thị bên phải). Chọn 4 là quên cộng 1; các hộp và điểm nối không làm tăng độ phức tạp.'),
      qx('Which work product can NOT be examined by static testing?|||Sản phẩm nào KHÔNG thể xem xét bằng kiểm thử tĩnh?', ['A user guide|||Hướng dẫn sử dụng', 'A test plan|||Test plan', 'A contract|||Hợp đồng', 'The response time of the running system|||Thời gian phản hồi của hệ thống đang chạy'], 3, 'Static testing examines any work product without executing it, so user guides, test plans and contracts can all be reviewed (slide 6). Response time only exists while the system runs, so it needs dynamic (performance) testing.|||Static testing xem xét mọi work product mà không chạy nó, nên hướng dẫn sử dụng, test plan và hợp đồng đều review được (slide 6). Thời gian phản hồi chỉ có khi hệ thống đang chạy, nên cần dynamic testing (performance test).'),
      qx('Rough guide to the cost of reviews (slide 11):|||Chi phí review ước chừng (slide 11):', ['1–2% of development effort|||1–2% công sức phát triển', '5–15% of development effort|||5–15% công sức phát triển', '30–40% of development effort|||30–40% công sức phát triển', 'Reviews cost nothing|||Review không tốn gì'], 1, 'SWT3 slide 11 gives the rough guide: reviews cost about 5-15% of development effort (half a day a week is about 10%). They are not free, but they pay back by cutting the number of defects that reach testing, so 1-2% is too low and 30-40% far too high.|||SWT3 slide 11 đưa ra con số ước chừng: review tốn khoảng 5-15% công sức phát triển (nửa ngày mỗi tuần là khoảng 10%). Review không miễn phí nhưng bù lại nhờ giảm số defect lọt tới giai đoạn test, nên 1-2% là quá thấp còn 30-40% là quá cao.'),
      qx('The optimum checking rate for an inspection is about…|||Tốc độ kiểm tra tối ưu của inspection khoảng…', ['1 page per hour|||1 trang mỗi giờ', '5–10 pages per hour|||5–10 trang mỗi giờ', '50 pages per hour|||50 trang mỗi giờ', '100 pages per hour|||100 trang mỗi giờ'], 0, 'The optimum inspection checking rate is about 1 page per hour, where 1 page = 300 important words (slides 29 and 64, lesson 3.3). 5-10 pages per hour is the rate for less formal reviews, which is why it looks plausible.|||Tốc độ kiểm tra tối ưu của inspection là khoảng 1 trang mỗi giờ, với 1 trang = 300 từ quan trọng (slide 29 và 64, bài 3.3). 5-10 trang mỗi giờ là tốc độ của review ít chính thức hơn, vì vậy nó trông hợp lý.'),
      qx('Holding a kick-off meeting (initiate review) was recorded to increase major defects found per page by about…|||Có buổi kick-off (initiate review) được ghi nhận làm tăng số defect nặng tìm được mỗi trang khoảng…', ['10%', '25%', '70%', '200%'], 2, 'SWT3 slide 28 records a 70% increase in major defects found per page when a kick-off meeting (a form of review initiation) was held, because reviewers who understand the objectives look for the right things.|||SWT3 slide 28 ghi nhận số defect nặng tìm được mỗi trang tăng 70% khi có buổi kick-off (một dạng initiate review), vì reviewer hiểu mục tiêu thì tìm đúng chỗ cần tìm.'),
      qx('In the review severity classes, "critical" means the defect…|||Trong các mức nghiêm trọng của review, "critical" nghĩa là defect…', ['will cause downstream damage beyond the work product under review|||chắc chắn gây hại về sau, vượt ra ngoài sản phẩm đang review', 'violates a template or standard only|||chỉ vi phạm mẫu hoặc chuẩn', 'is a spelling mistake|||là lỗi chính tả', 'was found by the author|||do tác giả tìm ra'], 0, 'Critical means the defect will cause downstream damage: its impact reaches beyond the work product under review (notes of slide 30). A defect that only violates a template or standard is minor; one that could cause a downstream effect is major.|||Critical nghĩa là defect sẽ gây hại về sau: ảnh hưởng của nó vượt ra ngoài work product đang review (ghi chú slide 30). Defect chỉ vi phạm mẫu hay chuẩn là minor; defect có thể gây ảnh hưởng về sau là major.'),
      qx('Empirical studies show the most effective general technique for reviewing requirements is…|||Nghiên cứu thực nghiệm cho thấy kỹ thuật tổng quát hiệu quả nhất để review yêu cầu là…', ['Ad hoc reviewing', 'Checklist-based reviewing', 'Perspective-based reading', 'Buddy check'], 2, 'Empirical studies show perspective-based reading is the most effective general technique for reviewing requirements: each reviewer takes a stakeholder viewpoint and tries to build a derived product, e.g. acceptance tests (lesson 3.3). Checklist-based is systematic but gives less depth and more duplicates.|||Nghiên cứu thực nghiệm cho thấy perspective-based reading là kỹ thuật tổng quát hiệu quả nhất để review yêu cầu: mỗi reviewer đứng ở góc nhìn một stakeholder và thử dựng sản phẩm dẫn xuất, ví dụ acceptance test (bài 3.3). Checklist-based có hệ thống nhưng ít chiều sâu và trùng lặp nhiều hơn.'),
      qx('In an inspection, the author may NOT act as…|||Trong inspection, tác giả KHÔNG được làm…', ['a participant who answers questions|||người tham gia trả lời câu hỏi', 'the person who fixes the defects|||người sửa defect', 'the reader, scribe or review leader|||reader, scribe hay review leader', 'the creator of the work product|||người tạo ra sản phẩm'], 2, 'In an inspection the author cannot be review leader, reader or scribe, so the work product is presented and recorded independently (lesson 3.3). The author still attends, answers questions and fixes the defects afterwards.|||Trong inspection, author không được làm review leader, reader hay scribe, để work product được trình bày và ghi nhận một cách độc lập (bài 3.3). Author vẫn tham dự, trả lời câu hỏi và sửa defect sau đó.'),
      qx('In "x = y + z", data-flow analysis says…|||Trong "x = y + z", data-flow analysis cho biết…', ['x is used; y and z are defined|||x được dùng; y và z được định nghĩa', 'x is defined; y and z are used|||x được định nghĩa; y và z được dùng', 'all three are declared|||cả ba được khai báo', 'all three are undefined|||cả ba chưa xác định'], 1, 'The assignment stores a value in x, so x is defined; y and z are only read, so they are used (referenced). Defined is not the same as declared: int y; declares y but leaves it undefined.|||Phép gán lưu giá trị vào x, nên x được định nghĩa (defined); y và z chỉ được đọc, nên chúng được dùng (used). Defined khác declared: int y; khai báo y nhưng y vẫn chưa được định nghĩa.'),
      qx('"n := 0; read(x); n := 1; …" — the anomaly on n is…|||"n := 0; read(x); n := 1; …" — anomaly của n là…', ['used before it is defined|||dùng trước khi được định nghĩa', 're-defined without being used|||được định nghĩa lại khi chưa được dùng', 'declared twice|||khai báo hai lần', 'an infinite loop|||vòng lặp vô hạn'], 1, 'n receives 0 and is overwritten by 1 before anyone reads it: a dd-anomaly, defined twice without being used (slide 98). Used before being defined (ur) is the anomaly of y in the same example, not of n.|||n nhận giá trị 0 rồi bị ghi đè bằng 1 trước khi có ai đọc: đó là dd-anomaly, định nghĩa hai lần mà chưa dùng (slide 98). Dùng trước khi định nghĩa (ur) là anomaly của y trong cùng ví dụ, không phải của n.'),
      qx('Buffsize = 1000, Mailboxmax = 1000; IF Buffsize < Mailboxmax THEN Error-Exit. Static analysis reports…|||Buffsize = 1000, Mailboxmax = 1000; IF Buffsize < Mailboxmax THEN Error-Exit. Phân tích tĩnh báo…', ['an undeclared variable|||biến chưa khai báo', 'the THEN clause is unreachable|||nhánh THEN không bao giờ tới được', 'a parameter type mismatch|||sai kiểu tham số', 'nothing|||không có gì'], 1, 'Both constants are 1000, so the condition Buffsize less than Mailboxmax is always false and the THEN branch can never run; static analysis flags it as unreachable code. Both variables are declared and have the same type, so there is no declaration or type error.|||Hai hằng đều bằng 1000, nên điều kiện Buffsize nhỏ hơn Mailboxmax luôn sai và nhánh THEN không bao giờ chạy được; static analysis báo đó là unreachable code. Cả hai biến đều đã khai báo và cùng kiểu, nên không có lỗi khai báo hay sai kiểu.'),
      qx('The flow graph of slide 103 has 10 nodes and 12 edges. V(G) = …|||Đồ thị luồng ở slide 103 có 10 đỉnh và 12 cạnh. V(G) = …', ['2', '3', '4', '12'], 2, 'V(G) = E - N + 2 = 12 - 10 + 2 = 4, the same as decisions + 1 = 3 + 1 for the three decisions (do, if, if) of the quiz-grading routine on slide 103. 12 is the edge count, not the complexity.|||V(G) = E - N + 2 = 12 - 10 + 2 = 4, trùng với số quyết định + 1 = 3 + 1 cho ba quyết định (do, if, if) của đoạn chấm điểm quiz ở slide 103. 12 là số cạnh, không phải độ phức tạp.'),
      qx('A limitation of static analysis tools is that they…|||Một giới hạn của công cụ phân tích tĩnh là chúng…', ['must execute the code|||phải chạy code', 'cannot find unreachable code|||không tìm được code không tới được', 'produce false positives and say nothing about operating conditions|||sinh cảnh báo thừa và không nói gì về điều kiện vận hành', 'only work on requirements documents|||chỉ dùng được cho tài liệu yêu cầu'], 2, 'Static analysis cannot tell deliberate code from real faults, so it often produces false positives, and because it never executes the code its results say nothing about operating conditions (lesson 3.4). It needs no execution and it does find unreachable code, so A and B are wrong.|||Static analysis không phân biệt được code viết cố ý với lỗi thật, nên thường sinh false positive, và vì không chạy code nên kết quả không nói gì về điều kiện vận hành (bài 3.4). Nó không cần chạy code và vẫn tìm được unreachable code, nên A và B sai.'),
