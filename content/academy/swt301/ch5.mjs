...
const QUIZ5 = {
  ...
  quiz: {
    ...
    questions: [
      qx('How many test cases are necessary to achieve 100% statement coverage of printSum(a, b) (if result > 0 print "red"… else if result < 0 print "blue"…)? (SWT4 s.98)|||Cần bao nhiêu test case để đạt 100% statement coverage cho printSum(a, b) (if result > 0 in "red"… else if result < 0 in "blue"…)? (SWT4 s.98)', ['1', '2', '4', '3'], 1,
        'Every println must run: one test with a + b > 0 prints "red", one with a + b < 0 prints "blue". One test can never reach both prints, and a third test (a + b = 0) adds no new statement, so 2.|||Mọi lệnh println đều phải chạy: một test có a + b > 0 in "red", một test có a + b < 0 in "blue". Một test không thể chạy cả hai lệnh in, còn test thứ ba (a + b = 0) không thêm lệnh nào mới, nên cần 2.'),
      qx('A section of code has one simple IF statement. How many tests are needed for 100% decision coverage? (s.99)|||Một đoạn code có một lệnh IF đơn giản. Cần bao nhiêu test để đạt 100% decision coverage? (s.99)', ['1', '2', '3', 'Unknown with this information|||Không xác định được với thông tin này'], 1,
        'Decision coverage needs both outcomes of the decision, True and False, and one test takes only one of them: 2 tests. The count is known because a simple IF has exactly two outcomes; 1 test is enough only for statement coverage of an IF without ELSE.|||Decision coverage cần cả hai kết quả của quyết định, True và False, mà một test chỉ đi một nhánh: 2 test. Con số xác định được vì IF đơn luôn có đúng hai kết quả; 1 test chỉ đủ cho statement coverage của IF không có ELSE.'),
      qx('Example 1 (valid card? → valid PIN? nested): minimum tests for 100% statement coverage? (s.91–92)|||Ví dụ 1 (thẻ hợp lệ? → PIN hợp lệ? lồng nhau): số test tối thiểu cho 100% statement coverage? (s.91–92)', ['1', '2', '3', '4'], 2,
        'There are three end actions, each on its own path: invalid card (reject card), valid card + invalid PIN (display "PIN invalid"), valid card + valid PIN (select transaction). One test reaches only one of them, so 3 (SWT4 slide 92).|||Có ba hành động cuối, mỗi cái nằm trên một đường riêng: thẻ không hợp lệ (reject card), thẻ hợp lệ + PIN sai (display "PIN invalid"), thẻ hợp lệ + PIN đúng (select transaction). Một test chỉ tới được một chỗ, nên cần 3 (SWT4 slide 92).'),
      qx('Example 2 (IF A > 0 THEN IF A = 21 THEN Print): cyclomatic complexity? (s.93)|||Ví dụ 2 (IF A > 0 THEN IF A = 21 THEN Print): cyclomatic complexity? (s.93)', ['1', '2', '3', '4'], 2,
        'Two binary decisions: V(G) = decisions + 1 = 2 + 1 = 3 (same as E − N + 2 on the flowchart). Slide 93 answer: CC 3, statement coverage 1 test (A = 21), branch coverage 3 tests.|||Hai quyết định nhị phân: V(G) = số quyết định + 1 = 2 + 1 = 3 (bằng E − N + 2 trên lưu đồ). Đáp án slide 93: CC 3, statement coverage 1 test (A = 21), branch coverage 3 test.'),
      qx('Example 3 (three nested decisions A>0, B=0, A>21): minimum tests for 100% decision coverage? (s.94)|||Ví dụ 3 (ba quyết định lồng A>0, B=0, A>21): số test tối thiểu cho 100% decision coverage? (s.94)', ['2', '3', '4', '5'], 2,
        'Nested decisions need one test per leaf: A ≤ 0; A > 0, B = 0; A > 0, B ≠ 0, A > 21; A > 0, B ≠ 0, A ≤ 21 → 4 tests. Only 2 are needed for statement coverage (slide 94: CC 4, SC 2, BC 4).|||Quyết định lồng nhau cần một test cho mỗi lá: A ≤ 0; A > 0, B = 0; A > 0, B ≠ 0, A > 21; A > 0, B ≠ 0, A ≤ 21 → 4 test. Statement coverage chỉ cần 2 (slide 94: CC 4, SC 2, BC 4).'),
      qx('Example 4 (two IF-THEN-ELSE in sequence): number of paths and minimum tests for 100% decision coverage? (s.95)|||Ví dụ 4 (hai IF-THEN-ELSE nối tiếp): số đường đi và số test tối thiểu cho 100% decision coverage? (s.95)', ['2 paths, 2 tests|||2 đường, 2 test', '4 paths, 2 tests|||4 đường, 2 test', '4 paths, 4 tests|||4 đường, 4 test', '3 paths, 3 tests|||3 đường, 3 test'], 1,
        'Two IFs in sequence give 2 × 2 = 4 paths. Decision coverage only needs each outcome once: (A < 0, B < 0) and (A ≥ 0, B ≥ 0) → 2 tests (slide 95). Running all 4 paths would be path coverage, not decision coverage.|||Hai IF nối tiếp cho 2 × 2 = 4 đường. Decision coverage chỉ cần mỗi kết quả chạy một lần: (A < 0, B < 0) và (A ≥ 0, B ≥ 0) → 2 test (slide 95). Chạy đủ 4 đường là path coverage, không phải decision coverage.'),
      qx('Example 5 (IF A < 0 THEN Print; IF B < 0 THEN Print — no ELSE): minimum tests for 100% statement coverage? (s.96)|||Ví dụ 5 (IF A < 0 THEN Print; IF B < 0 THEN Print — không ELSE): số test tối thiểu cho 100% statement coverage? (s.96)', ['1', '2', '3', '4'], 0,
        'With no ELSE, one test with A < 0 and B < 0 runs both Print statements, so 1 test gives 100% statement coverage. Decision coverage would need 2, because the False outcomes must be taken too (slide 96: CC 3, SC 1, BC 2).|||Không có ELSE nên một test với A < 0 và B < 0 chạy cả hai lệnh Print: 1 test đạt 100% statement coverage. Decision coverage mới cần 2, vì còn phải đi cả nhánh False (slide 96: CC 3, SC 1, BC 2).'),
      qx('Example 6 (IF A < 0 THEN Print; IF A > 0 THEN Print): minimum tests for 100% decision coverage? (s.97)|||Ví dụ 6 (IF A < 0 THEN Print; IF A > 0 THEN Print): số test tối thiểu cho 100% decision coverage? (s.97)', ['1', '2', '3', '4'], 1,
        'A = −1 takes A < 0 True and A > 0 False; A = 1 takes A < 0 False and A > 0 True, so 2 tests cover all four outcomes (slide 97: CC 3, SC 2, BC 2). One test cannot do it: A cannot be negative and positive at once.|||A = −1 đi A < 0 True và A > 0 False; A = 1 đi A < 0 False và A > 0 True, nên 2 test phủ đủ bốn kết quả (slide 97: CC 3, SC 2, BC 2). Một test không làm được vì A không thể vừa âm vừa dương.'),
      qx('A program has 120 decision outcomes and the tests exercise 60 of them. Decision coverage is… (s.88)|||Chương trình có 120 kết quả quyết định và test chạy qua 60. Decision coverage là… (s.88)', ['40%', '50%', '60%', '120%'], 1,
        'Decision coverage = outcomes exercised / total outcomes = 60 / 120 = 50% (slide 88). 60% mistakes the raw count 60 for a percentage.|||Decision coverage = số kết quả đã chạy / tổng số kết quả = 60 / 120 = 50% (slide 88). 60% là nhầm con số 60 thành phần trăm.'),
      qx('Which statement is TRUE?|||Phát biểu nào ĐÚNG?', ['100% statement coverage guarantees 100% decision coverage|||100% statement coverage bảo đảm 100% decision coverage', '100% decision coverage guarantees 100% statement coverage|||100% decision coverage bảo đảm 100% statement coverage', 'The two are always equal|||Hai loại luôn bằng nhau', '100% coverage means the code is fully tested|||100% coverage nghĩa là code đã được test đầy đủ'], 1,
        'Every statement lies on some decision outcome, so taking all outcomes runs all statements: 100% DC implies 100% SC. The reverse fails: in Example 5 one test gives 100% statement coverage but only 2 of 4 outcomes (50% DC). And slide 85: 100% coverage does not mean 100% tested.|||Mọi câu lệnh đều nằm trên một kết quả quyết định nào đó, nên đi hết các kết quả là chạy hết câu lệnh: 100% DC kéo theo 100% SC. Chiều ngược lại sai: ở Ví dụ 5 một test đạt 100% statement coverage nhưng chỉ 2/4 kết quả (50% DC). Và slide 85: 100% coverage không có nghĩa là đã test 100%.'),
      qx('A control-flow graph has 13 edges and 10 nodes (one connected routine). V(G) = ?|||Một đồ thị luồng điều khiển có 13 cạnh và 10 node (một routine liên thông). V(G) = ?', ['3', '4', '5', '23'], 2,
        'V(G) = E − N + 2P = 13 − 10 + 2 × 1 = 5 for one connected routine (lesson 5.2). 23 wrongly adds edges and nodes; 3 forgets the + 2.|||V(G) = E − N + 2P = 13 − 10 + 2 × 1 = 5 với một routine liên thông (bài 5.2). 23 là cộng nhầm cạnh với node; 3 là quên + 2.'),
      qx('PE FA23: countCharacters(String) loops over the characters with if isUpperCase / else if isLowerCase / else if isDigit / else. Minimum number of test cases for 100% statement AND 100% decision coverage?|||PE FA23: countCharacters(String) lặp qua từng ký tự với if isUpperCase / else if isLowerCase / else if isDigit / else. Số test case tối thiểu cho 100% statement VÀ 100% decision coverage?', ['1', '4', '5', '8'], 0,
        'Each loop iteration can take a different branch, so one input such as "Ab1@" hits upper, lower, digit and other, and the loop both runs and exits: 18/18 lines, 8/8 branches in lesson 5.2. V(G) = 5 is only an upper bound on the tests needed, so 5 is the trap.|||Mỗi vòng lặp có thể đi một nhánh khác, nên một input như "Ab1@" chạm đủ chữ hoa, chữ thường, chữ số và ký tự khác, và vòng lặp vừa chạy vừa thoát: 18/18 dòng, 8/8 nhánh ở bài 5.2. V(G) = 5 chỉ là cận trên của số test, nên 5 là bẫy.'),
      qx('PE SU24 tax flowchart: 9 binary decisions, 8 distinct return boxes, and "Return −1" is reached from three different decisions. Minimum tests for 100% statement / 100% decision coverage?|||Lưu đồ thuế PE SU24: 9 quyết định nhị phân, 8 hộp return khác nhau, và "Return −1" được tới từ ba quyết định khác nhau. Số test tối thiểu cho 100% statement / 100% decision coverage?', ['8 / 10', '9 / 10', '8 / 8', '10 / 10'], 0,
        'Statement: 8 return boxes and each test ends in exactly one → 8. Decision: Return −1 is reached by three different True outcomes (sal < 0, te < 0, nod < 0) needing three separate tests, so 7 other returns + 3 = 10 (lesson 5.2). 8 / 8 forgets those extra True outcomes.|||Statement: 8 hộp return và mỗi test kết thúc ở đúng một hộp → 8. Decision: Return −1 được tới bởi ba kết quả True khác nhau (sal < 0, te < 0, nod < 0) cần ba test riêng, nên 7 return còn lại + 3 = 10 (bài 5.2). 8 / 8 là quên các nhánh True thêm đó.'),
      qx('Why is 100% path coverage rarely achievable in real code? (s.90)|||Vì sao 100% path coverage hiếm khi đạt được với code thật? (s.90)', ['Tools cannot measure paths|||Công cụ không đo được đường đi', 'Loops make the number of paths unbounded|||Vòng lặp làm số đường đi không giới hạn', 'Every decision has three outcomes|||Mỗi quyết định có ba kết quả', 'Paths only exist in black-box testing|||Đường đi chỉ có trong black-box'], 1,
        'Each extra trip round a loop is a new path, and a loop can repeat as many times as possible, even without limit, so the number of paths becomes unbounded (slide 90). Tools can measure paths; the problem is how many there are, not measuring them.|||Mỗi vòng lặp thêm là một đường mới, mà vòng lặp có thể lặp bao nhiêu lần tuỳ ý, thậm chí vô hạn, nên số đường đi không có giới hạn (slide 90). Công cụ vẫn đo được đường đi; vấn đề là số lượng quá lớn, không phải việc đo.'),
