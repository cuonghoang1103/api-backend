...
const QUIZ7 = {
  ...
  quiz: {
    ...
    questions: [
      ...
      q('Match 1 Analytical, 2 Methodical, 3 Model-based, 4 Consultative with A state diagram, B level of risk, C predefined set of test conditions, D views of domain experts (s.52)|||Ghép 1 Analytical, 2 Methodical, 3 Model-based, 4 Consultative với A sơ đồ trạng thái, B mức rủi ro, C tập test condition định sẵn, D ý kiến chuyên gia nghiệp vụ (s.52)', ['1D, 2B, 3A, 4C', '1A, 2C, 3D, 4B', '1D, 2C, 3B, 4A', '1B, 2C, 3A, 4D'], 3),
      q('Which is the characteristic of a metrics-based approach to test estimation? (s.53)|||Đâu là đặc trưng của ước lượng dựa trên số liệu? (s.53)', ['Budget used by a previous similar test project|||Ngân sách của một dự án test tương tự trước đây', 'Overall experience collected in interviews with test managers|||Kinh nghiệm thu qua phỏng vấn test manager', 'Overall estimate agreed with the developers|||Ước lượng thống nhất với developer', 'Average of calculations collected from business experts|||Trung bình các tính toán thu từ chuyên gia nghiệp vụ'], 0),
      q('R1 → R3, R1 → R2, R3 → R2, R2 → R5 and R6, the R2 group → R4 and R7 ("X → Y" = Y depends on X). Which schedule respects the dependencies? (s.54)|||R1 → R3, R1 → R2, R3 → R2, R2 → R5 và R6, cụm R2 → R4 và R7 ("X → Y" = Y phụ thuộc X). Lịch nào đúng phụ thuộc? (s.54)', ['R1 → R3 → R1 → R2 → R5 → R6 → R4 → R7', 'R1 → R3 → R2 → R5 → R2 → R6 → R4 → R7', 'R1 → R3 → R2 → R5 → R6 → R4 → R7', 'R1 → R2 → R5 → R6 → R3 → R4 → R7'], 2),
      q('Test cases vary a lot in length and detail. Where should the test case guidelines have been documented? (s.55)|||Các test case chênh nhau nhiều về độ dài và độ chi tiết. Hướng dẫn viết test case lẽ ra phải ghi ở đâu? (s.55)', ['The test approach|||Test approach', 'The test plan|||Test plan', 'The test case template|||Mẫu test case', 'The project plan|||Project plan'], 1),
      q('TC1 (Low, needs 6), TC2 (Medium, none), TC3 (High, needs 1), TC4 (High, needs 2), TC5 (Medium, needs 4), TC6 (Low, needs 2). Run by risk with fast feedback — best order? (s.56)|||TC1 (Low, cần 6), TC2 (Medium, không), TC3 (High, cần 1), TC4 (High, cần 2), TC5 (Medium, cần 4), TC6 (Low, cần 2). Chạy theo rủi ro, phản hồi nhanh — thứ tự tốt nhất? (s.56)', ['4, 3, 2, 5, 6, 1', '2, 4, 5, 6, 1, 3', '2, 5, 6, 4, 1, 3', '6, 1, 3, 2, 4, 5'], 1),
      q('A test strategy based on the list of ISO 25010 quality characteristics is… (s.57)|||Chiến lược test dựa trên danh sách đặc tính chất lượng ISO 25010 là… (s.57)', ['Regulatory', 'Analytical', 'Methodical', 'Reactive'], 2),
      q('Another upgrade of an ERP system whose previous upgrade your team tested years ago — best estimation technique? (s.58)|||Thêm một lần nâng cấp hệ thống ERP mà lần trước nhóm bạn đã test — kỹ thuật ước lượng phù hợp nhất? (s.58)', ['Effort-based', 'Expert-based', 'Metric-based', 'Schedule-based'], 2),
      q('Which metric is MOST useful to monitor during test execution? (s.70)|||Số đo nào hữu ích NHẤT để theo dõi trong lúc thực thi test? (s.70)', ['Percentage of executed test cases|||Phần trăm test case đã thực thi', 'Percentage of work done in test environment preparation|||Phần trăm công việc dựng môi trường', 'Percentage of planned test cases prepared|||Phần trăm test case đã chuẩn bị', 'Percentage of work done in test case preparation|||Phần trăm công chuẩn bị test case'], 0),
      ...
      q('The purpose of configuration management is to…|||Mục đích của quản lý cấu hình là…', ['store defect reports|||lưu defect report', 'establish and maintain the integrity of the system, the testware and their relationships|||thiết lập và duy trì tính toàn vẹn của hệ thống, testware và quan hệ giữa chúng', 'estimate the test effort|||ước lượng công sức test', 'decide the release date|||quyết định ngày phát hành'], 1),
      q('"Response times may be inadequate for a high-performance transaction system" is a…|||"Thời gian phản hồi có thể không đủ cho hệ thống giao dịch hiệu năng cao" là…', ['project risk|||rủi ro dự án', 'product risk|||rủi ro sản phẩm', 'supplier issue|||vấn đề nhà cung cấp', 'political issue|||vấn đề "chính trị"'], 1),
      q('"The supplier may deliver the test environment late" is a…|||"Nhà cung cấp có thể giao môi trường test trễ" là…', ['product risk|||rủi ro sản phẩm', 'quality risk|||rủi ro chất lượng', 'project risk|||rủi ro dự án', 'defect|||defect'], 2),
