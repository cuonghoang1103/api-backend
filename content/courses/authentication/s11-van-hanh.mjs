...
<div class="pitfall">
<p><strong>Trap — an alert nobody acts on trains everybody to ignore the channel.</strong> Six alerts that fire correctly are worth more than sixty that mostly do not, because the cost of a false positive is not the interruption — it is the next real alert being dismissed by reflex. Every alert needs three things before it is switched on: a threshold derived from a fortnight of your own data rather than a blog post, a named owner, and a linked runbook. If you cannot write the runbook, the alert is not actionable and should be a dashboard panel instead. And review the ones that fire without anyone doing anything: they are the ones eroding your response.</p>
</div>
