/**
 * _slides.mjs — nhúng ảnh slide của khoá "Docker" vào bài học.
 *
 * Slide do khoá tự soạn, dựng bằng
 *   node scripts/_render-slides.mjs --deck scripts/slides-src/dk-NN.mjs --out <dir>
 * rồi đẩy lên R2 bằng
 *   node --env-file=.env scripts/upload-academy-slides.mjs --dir <dir> --prefix DK/v1 --decks dk-NN
 * ⇒ images/academy/DK/v1/<deck>/<NNN>.webp
 *
 ...
 * sang prefix mới (DK/v2) và ghi deck đó vào VER bên dưới — đừng ghi đè v1.
 *
 * Việc "slide số n có thật không" được kiểm ở bước ghép bằng
 * `node scripts/dk-ghep-chuong.mjs <file chương> --render <dir> [--cdn]`.
 */
...
export function slide(deck, n, caption) {
  if (!/^dk-\d\d$/.test(deck)) throw new Error(`deck lạ: ${deck}`);
