...
<div class="pitfall">
<p><strong>Bẫy — row 4 is a <code>200</code> with an empty body, and every layer above it will call that a success.</strong> Nginx had already sent <code>HTTP/1.1 200 OK</code> and <code>Transfer-Encoding: chunked</code> when the upstream went quiet; when the read timed out there was no way to retract that, so it simply closed the connection without the terminating chunk. A monitoring check that looks at status codes sees <code>200</code>. A browser fetch resolves. An HTTP client that does not verify the chunked terminator returns an empty string, and your code treats it as an empty result rather than an error. The only honest signals are the truncated body and the <code>while reading upstream</code> line in the error log — note it says <em>reading upstream</em>, not <em>reading response header from upstream</em>, and that one word is how you tell row 4 from row 1 at three in the morning.</p>
</div>
...
<div class="pitfall">
<p><strong>Bẫy — the failure is a <code>200</code>, so nothing you would normally check reports a problem.</strong> Row A returned a successful status with a body from your own application; Nginx logged a normal request; the upstream logged a normal request; no error appeared anywhere. The client library reports something vague like "connection closed before the handshake completed", which sends people looking at the client, at TLS, at the firewall — everywhere except the four lines missing from the proxy block. The one diagnostic that answers it immediately is a raw handshake: send the <code>GET</code> with <code>Upgrade: websocket</code> yourself and read the status line. <code>101</code> means the proxy is fine and the problem is elsewhere; <code>200</code> means it is this.</p>
</div>
