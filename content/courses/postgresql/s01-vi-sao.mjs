...
<h3>Why the relational model won</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">Each fact lives in exactly one place</span><span class="lz-d">A user's name is stored once. Nothing can disagree with it, because there is no second copy to disagree.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">Relationships are values, not pointers</span><span class="lz-d">A row references another by its key. The database can then check that reference is real — which a pointer in application memory never could.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">Queries are declarative</span><span class="lz-d">You describe the result you want; the planner chooses how to get it. That is why the same query gets faster when you add an index, with no code change.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">The structure is enforced, not hoped for</span><span class="lz-d">Types, <code>NOT NULL</code>, foreign keys and <code>CHECK</code> are guarantees the data must satisfy — including against code you have not written yet.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — duplicating a value "to save a join".</strong> Copying <code>user_name</code> into the orders table makes one query simpler today and creates a second source of truth forever. The two copies agree until the first rename, and after that nothing tells you which is correct — the bug surfaces months later as two screens showing different names for the same person. Denormalisation is a real technique, but it is a performance decision taken <em>after</em> measuring, with a written plan for keeping the copies in sync (a trigger, a job, a materialised view). Copying a column because a join felt like effort is not that.</p></div>
...
<h3>Vì sao mô hình quan hệ thắng</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">Mỗi sự thật sống ở ĐÚNG một chỗ</span><span class="lz-d">Tên một người dùng được lưu một lần. Không gì mâu thuẫn với nó được, vì không có bản sao thứ hai để mà mâu thuẫn.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">Quan hệ là GIÁ TRỊ, không phải con trỏ</span><span class="lz-d">Một dòng tham chiếu dòng khác bằng khoá của nó. Nhờ vậy cơ sở dữ liệu KIỂM được tham chiếu ấy có thật — điều một con trỏ trong bộ nhớ ứng dụng không bao giờ làm được.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">Truy vấn là KHAI BÁO</span><span class="lz-d">Bạn mô tả kết quả mình muốn; planner chọn cách lấy. Đó là lý do cùng một truy vấn nhanh lên khi bạn thêm chỉ mục, mà không đổi một dòng mã nào.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">Cấu trúc được THI HÀNH, không phải được HY VỌNG</span><span class="lz-d">Kiểu dữ liệu, <code>NOT NULL</code>, khoá ngoại và <code>CHECK</code> là những bảo đảm mà dữ liệu BẮT BUỘC phải thoả — kể cả trước thứ mã bạn còn chưa viết.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — nhân bản một giá trị "cho đỡ phải join".</strong> Chép <code>user_name</code> vào bảng đơn hàng làm một truy vấn đơn giản hơn HÔM NAY và tạo ra một nguồn sự thật thứ hai MÃI MÃI. Hai bản sao còn khớp cho tới lần đổi tên đầu tiên, và sau đó không gì cho bạn biết cái nào đúng — con bug nổi lên vài tháng sau dưới dạng hai màn hình hiện hai cái tên khác nhau cho cùng một người. Phi chuẩn hoá là một kỹ thuật thật, nhưng nó là một quyết định HIỆU NĂNG đưa ra SAU khi đo, kèm một kế hoạch VIẾT RA để giữ các bản sao đồng bộ (một trigger, một job, một materialized view). Chép một cột vì thấy join hơi mệt thì không phải là thế.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<h3>What each letter actually buys you</h3>
<div class="lz-stack">
<div class="lz-layer"><span class="lz-lname">Atomicity</span><span class="lz-lnote">All of the transaction, or none of it. A transfer cannot debit one account and fail before crediting the other — the crash leaves no half-state to clean up.</span></div>
<div class="lz-layer"><span class="lz-lname">Consistency</span><span class="lz-lnote">Every constraint holds at commit. A foreign key cannot point at a deleted row, a <code>CHECK</code> cannot be bypassed — including by a script written at 2am by someone in a hurry.</span></div>
<div class="lz-layer"><span class="lz-lname">Isolation</span><span class="lz-lnote">Concurrent transactions do not see each other's unfinished work. This is the letter with settings — the isolation level — and the one people get wrong.</span></div>
<div class="lz-layer"><span class="lz-lname">Durability</span><span class="lz-lnote">Once <code>COMMIT</code> returns, the data survives a power cut. Bought with a disk flush, which is why a commit costs more than a write.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — assuming the default isolation level prevents every race.</strong> PostgreSQL's default is <code>READ COMMITTED</code>, and it does exactly what its name says: each statement sees data committed before <em>that statement</em> started. It does not stop the classic read-then-write race — two transactions both read a balance of 100, both subtract 30, and the result is 70 instead of 40. Nothing errors. The fixes are to make the write itself conditional (<code>UPDATE … SET balance = balance - 30 WHERE balance &gt;= 30</code>), to lock the row with <code>SELECT … FOR UPDATE</code>, or to raise the isolation level and be prepared to retry on serialization failures.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<h3>Mỗi chữ cái thật sự mua cho bạn cái gì</h3>
<div class="lz-stack">
<div class="lz-layer"><span class="lz-lname">Atomicity — Nguyên tử</span><span class="lz-lnote">Hoặc trọn giao dịch, hoặc không gì cả. Một cú chuyển tiền không thể trừ một tài khoản rồi hỏng trước khi cộng cho tài khoản kia — cú sập không để lại nửa trạng thái nào để bạn phải dọn.</span></div>
<div class="lz-layer"><span class="lz-lname">Consistency — Nhất quán</span><span class="lz-lnote">Mọi ràng buộc đều đúng tại thời điểm commit. Một khoá ngoại không trỏ được vào dòng đã xoá, một <code>CHECK</code> không bị đi vòng — kể cả bởi một script viết lúc 2 giờ sáng bởi người đang vội.</span></div>
<div class="lz-layer"><span class="lz-lname">Isolation — Cô lập</span><span class="lz-lnote">Các giao dịch chạy song song không thấy phần việc chưa xong của nhau. Đây là chữ CÓ THAM SỐ — mức cô lập — và là chữ người ta hay làm sai.</span></div>
<div class="lz-layer"><span class="lz-lname">Durability — Bền vững</span><span class="lz-lnote">Một khi <code>COMMIT</code> trả về, dữ liệu sống sót qua một cú mất điện. Mua bằng một lần xả đĩa, và đó là lý do một cú commit đắt hơn một cú ghi.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — cho rằng mức cô lập mặc định ngăn được MỌI cuộc đua.</strong> Mặc định của PostgreSQL là <code>READ COMMITTED</code>, và nó làm đúng như tên gọi: mỗi CÂU LỆNH thấy dữ liệu đã commit trước khi <em>câu lệnh đó</em> bắt đầu. Nó KHÔNG chặn cuộc đua đọc-rồi-ghi kinh điển — hai giao dịch cùng đọc số dư 100, cùng trừ 30, và kết quả là 70 thay vì 40. Không có lỗi nào cả. Cách chữa là làm cho chính lệnh GHI có điều kiện (<code>UPDATE … SET balance = balance - 30 WHERE balance &gt;= 30</code>), khoá dòng bằng <code>SELECT … FOR UPDATE</code>, hoặc nâng mức cô lập lên và chuẩn bị sẵn tinh thần THỬ LẠI khi gặp lỗi serialization.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<div class="pitfall"><p><strong>Trap — choosing a database by benchmark numbers from someone else's workload.</strong> "X is 10× faster than PostgreSQL" is almost always true of one specific operation under one specific shape of data, and almost always irrelevant to you. The questions that actually decide the outcome are duller and never in the benchmark: does it have transactions across the rows you update together; can you query it in ways you have not thought of yet; what happens to the data when the schema changes; who operates it at 3am. PostgreSQL wins most of these not by being fastest but by being <em>predictable</em> — and by the time raw speed genuinely dominates, you will have measured your own workload rather than read someone else's chart.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<div class="pitfall"><p><strong>Bẫy — chọn cơ sở dữ liệu theo con số benchmark của khối lượng công việc CỦA NGƯỜI KHÁC.</strong> "X nhanh hơn PostgreSQL 10 lần" gần như luôn ĐÚNG với một thao tác cụ thể trên một hình dạng dữ liệu cụ thể, và gần như luôn KHÔNG LIÊN QUAN tới bạn. Những câu hỏi thật sự quyết định kết cục thì tẻ nhạt hơn và không bao giờ có trong benchmark: nó có giao dịch bao trọn những dòng bạn cập nhật cùng lúc không; bạn truy vấn nó được theo những cách bạn CHƯA NGHĨ RA không; chuyện gì xảy ra với dữ liệu khi lược đồ đổi; ai vận hành nó lúc 3 giờ sáng. PostgreSQL thắng phần lớn những câu này không phải vì nhanh nhất mà vì DỰ ĐOÁN ĐƯỢC — và tới lúc tốc độ thô thật sự lấn át, bạn sẽ đã đo khối lượng công việc của chính mình chứ không phải đọc biểu đồ của ai đó.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
