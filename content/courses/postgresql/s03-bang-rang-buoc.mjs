...
<h3>What a primary key promises</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">Unique and never NULL</span><span class="lz-d">Two constraints in one declaration. It is the row's identity, and an identity that can be missing is not one.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">It creates an index automatically</span><span class="lz-d">You never need to add one on the primary key — PostgreSQL already did, which is why lookups by id are fast from the first row.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">It is what foreign keys point at</span><span class="lz-d">Every relationship in the schema is built on it. Change a primary key value and every child row must change with it — which is the argument for surrogate keys.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">Surrogate by default, natural by exception</span><span class="lz-d">A generated id has no business meaning and therefore never needs to change. A natural key (an email, an ISBN) means one business change rewrites half your tables.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — using an email address as the primary key.</strong> It is unique, it is never NULL, and it reads well in a foreign key — every argument for it is true. Then a user changes their email, and that single edit must cascade through every table that referenced them, inside one transaction, while the application is running. Worse, the old value may still be sitting in logs, exports and other systems that now point at nothing. Generate the key, put a <code>UNIQUE</code> constraint on the email, and let the identity stay still while the attributes change.</p></div>
...
<h3>Một khoá chính hứa hẹn điều gì</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">Duy nhất và KHÔNG BAO GIỜ NULL</span><span class="lz-d">Hai ràng buộc trong một lời khai. Nó là DANH TÍNH của dòng, và một danh tính có thể vắng mặt thì không phải danh tính.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">Nó TỰ tạo một chỉ mục</span><span class="lz-d">Bạn không bao giờ cần thêm chỉ mục cho khoá chính — PostgreSQL đã làm rồi, và đó là lý do tra theo id nhanh ngay từ dòng đầu tiên.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">Nó là thứ mà khoá ngoại TRỎ VÀO</span><span class="lz-d">Mọi quan hệ trong lược đồ đều dựng trên nó. Đổi giá trị một khoá chính là mọi dòng con phải đổi theo — chính là lý lẽ cho khoá thay thế.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">Mặc định khoá thay thế, ngoại lệ mới dùng khoá tự nhiên</span><span class="lz-d">Một id sinh ra không mang nghĩa nghiệp vụ nào nên không bao giờ cần đổi. Một khoá tự nhiên (email, mã ISBN) nghĩa là MỘT thay đổi nghiệp vụ viết lại nửa số bảng của bạn.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — dùng địa chỉ email làm khoá chính.</strong> Nó duy nhất, nó không bao giờ NULL, và nó đọc rất xuôi trong một khoá ngoại — mọi lý lẽ ủng hộ nó đều ĐÚNG. Rồi một người dùng đổi email, và đúng một thao tác sửa ấy phải lan qua MỌI bảng từng tham chiếu tới họ, bên trong MỘT giao dịch, trong lúc ứng dụng đang chạy. Tệ hơn, giá trị cũ có thể vẫn còn nằm trong log, trong các bản xuất và trong những hệ thống khác — giờ trỏ vào hư không. Hãy SINH ra khoá, đặt ràng buộc <code>UNIQUE</code> lên email, và để danh tính đứng yên trong khi các thuộc tính thay đổi.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<h3>Choosing an ON DELETE action</h3>
<div class="lz-stack">
<div class="lz-layer"><span class="lz-lname">NO ACTION / RESTRICT</span><span class="lz-lnote">The default. Refuses the delete while children exist. Right when the child is valuable — you want an error, not a silent disappearance.</span></div>
<div class="lz-layer"><span class="lz-lname">CASCADE</span><span class="lz-lnote">Deletes the children too. Right when the child cannot exist alone — an order line without its order. Dangerous everywhere else, because one delete can remove far more than the person expected.</span></div>
<div class="lz-layer"><span class="lz-lname">SET NULL</span><span class="lz-lnote">Keeps the child and clears the link. Right for optional relationships — a post whose category was deleted is still a post.</span></div>
<div class="lz-layer"><span class="lz-lname">the missing index</span><span class="lz-lnote">PostgreSQL indexes the parent side automatically and <em>not</em> the child's foreign-key column. Deleting a parent then scans the whole child table to check — the most common cause of a slow delete.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — <code>ON DELETE CASCADE</code> chosen for convenience.</strong> It removes the annoying error during development and quietly builds a chain: deleting one user cascades to their orders, which cascades to order lines, which cascades to shipment records. Someone deletes a test account in the admin panel two years later and takes out forty thousand rows across nine tables, in one transaction, with no confirmation step. Use <code>CASCADE</code> only where the child is genuinely a part of the parent, and prefer soft-delete (a <code>deleted_at</code> column) for anything a human might click.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<h3>Chọn hành động ON DELETE</h3>
<div class="lz-stack">
<div class="lz-layer"><span class="lz-lname">NO ACTION / RESTRICT</span><span class="lz-lnote">Mặc định. Từ chối cú xoá khi vẫn còn dòng con. Đúng khi dòng con có giá trị — bạn muốn một LỖI, không muốn một sự biến mất im lặng.</span></div>
<div class="lz-layer"><span class="lz-lname">CASCADE</span><span class="lz-lnote">Xoá luôn các dòng con. Đúng khi dòng con không thể tồn tại một mình — một dòng hàng không có đơn hàng của nó. Nguy hiểm ở mọi chỗ khác, vì MỘT cú xoá có thể gỡ đi nhiều hơn hẳn điều người ta trông đợi.</span></div>
<div class="lz-layer"><span class="lz-lname">SET NULL</span><span class="lz-lnote">Giữ dòng con và xoá mối liên kết. Đúng cho quan hệ TUỲ CHỌN — một bài viết bị xoá danh mục thì vẫn là một bài viết.</span></div>
<div class="lz-layer"><span class="lz-lname">cái chỉ mục bị thiếu</span><span class="lz-lnote">PostgreSQL tự đánh chỉ mục phía CHA và KHÔNG đánh cho cột khoá ngoại phía CON. Xoá một dòng cha khi ấy phải quét TRỌN bảng con để kiểm — nguyên nhân phổ biến nhất của một cú xoá chậm.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — chọn <code>ON DELETE CASCADE</code> cho TIỆN.</strong> Nó dẹp đi cái lỗi phiền phức lúc phát triển và lặng lẽ dựng lên một chuỗi: xoá một user lan sang các đơn hàng của họ, lan tiếp sang các dòng hàng, lan tiếp sang các bản ghi vận chuyển. Hai năm sau có người xoá một tài khoản test trong trang quản trị và cuốn theo bốn mươi nghìn dòng trên chín bảng, trong MỘT giao dịch, không có bước xác nhận nào. Chỉ dùng <code>CASCADE</code> ở chỗ dòng con thật sự là một BỘ PHẬN của dòng cha, và ưu tiên xoá mềm (một cột <code>deleted_at</code>) cho bất cứ thứ gì con người có thể bấm vào.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<h3>Where a rule should live</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">In the database</span><span class="lz-d">Constraints hold against every writer — your API, a migration script, a colleague in psql, a job written next year. This is the only layer with that property.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">In the application</span><span class="lz-d">Good error messages and rules that need context the database lacks. Necessary, but it protects only the paths that go through your code.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">Both, deliberately</span><span class="lz-d">Validate in the app for the user's sake, constrain in the database for the data's sake. The duplication is the point, not an oversight.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">NOT NULL is the cheapest one</span><span class="lz-d">It costs nothing, catches an entire class of bug, and is far harder to add later once NULLs are already in the table.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — leaving columns nullable "for flexibility".</strong> Every nullable column doubles the states your code must handle, and the compiler cannot help: <code>user.name</code> being possibly NULL means every display, every comparison and every concatenation needs a guard, forever. Worse, the flexibility is an illusion — a column that should never be empty and is nullable will eventually contain NULLs written by a partial import or a bug, and then you cannot add <code>NOT NULL</code> without cleaning data first. Make columns <code>NOT NULL</code> by default and justify each exception.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<h3>Một luật nên sống ở đâu</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">Trong cơ sở dữ liệu</span><span class="lz-d">Ràng buộc có hiệu lực với MỌI bên ghi — API của bạn, một script migration, một đồng nghiệp gõ psql, một job viết vào năm sau. Đây là tầng DUY NHẤT có tính chất ấy.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">Trong ứng dụng</span><span class="lz-d">Thông báo lỗi tử tế và những luật cần bối cảnh mà cơ sở dữ liệu không có. Cần thiết, nhưng nó chỉ bảo vệ những con đường ĐI QUA mã của bạn.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">Cả hai, một cách CÓ CHỦ Ý</span><span class="lz-d">Kiểm ở app vì người dùng, ràng buộc ở cơ sở dữ liệu vì dữ liệu. Sự trùng lặp ấy chính là CHỦ ĐÍCH, không phải sơ suất.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">NOT NULL là thứ rẻ nhất</span><span class="lz-d">Không tốn gì, bắt được cả một họ lỗi, và khó thêm về sau hơn HẲN một khi bảng đã có NULL trong đó.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — để cột cho phép NULL "cho linh hoạt".</strong> Mỗi cột nullable nhân đôi số trạng thái mà mã của bạn phải xử lý, và trình biên dịch không giúp được: <code>user.name</code> có thể NULL nghĩa là mọi lần hiển thị, mọi phép so sánh và mọi lần nối chuỗi đều cần một lớp chắn, mãi mãi. Tệ hơn, sự linh hoạt ấy là ẢO — một cột lẽ ra không bao giờ được rỗng mà lại nullable thì rốt cuộc sẽ chứa NULL do một lần nhập liệu dở dang hoặc một con bug ghi vào, và khi ấy bạn không thêm được <code>NOT NULL</code> nếu chưa dọn dữ liệu. Hãy mặc định <code>NOT NULL</code> cho các cột và BIỆN MINH cho từng ngoại lệ.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<h3>Which ALTER locks the table</h3>
<div class="lz-stack">
<div class="lz-layer"><span class="lz-lname">Instant (metadata only)</span><span class="lz-lnote">Adding a nullable column, adding a column with a constant default (PG 11+), dropping a column, renaming. Milliseconds regardless of table size.</span></div>
<div class="lz-layer"><span class="lz-lname">Rewrites the whole table</span><span class="lz-lnote">Changing a column's type, adding a column with a volatile default. On ten million rows this holds an exclusive lock for minutes — an outage, not a migration.</span></div>
<div class="lz-layer"><span class="lz-lname">Scans but does not rewrite</span><span class="lz-lnote">Adding <code>NOT NULL</code> or a <code>CHECK</code>. Faster, still an exclusive lock. Add the constraint <code>NOT VALID</code> first, then <code>VALIDATE</code> separately — that second step takes a weaker lock.</span></div>
<div class="lz-layer"><span class="lz-lname">The lock queue</span><span class="lz-lnote">An <code>ALTER</code> waiting for a lock blocks every query that arrives after it. One slow migration can stall reads that would otherwise have been fine.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — testing a migration on a table with a thousand rows.</strong> Every <code>ALTER</code> is instant on development data, so the plan looks safe and the timing you measured is meaningless. The same statement on the production table takes a lock proportional to its size, and the failure mode is not a slow deploy — it is every request timing out while the migration waits. Before shipping a schema change, check the row count on production, look up whether that operation rewrites, and set <code>lock_timeout</code> so a blocked migration fails fast instead of taking the site down with it.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<h3>Lệnh ALTER nào KHOÁ bảng</h3>
<div class="lz-stack">
<div class="lz-layer"><span class="lz-lname">Tức thì (chỉ đụng metadata)</span><span class="lz-lnote">Thêm một cột nullable, thêm cột có giá trị mặc định là HẰNG (PG 11+), bỏ một cột, đổi tên. Tính bằng mili-giây bất kể bảng to cỡ nào.</span></div>
<div class="lz-layer"><span class="lz-lname">Viết lại TRỌN bảng</span><span class="lz-lnote">Đổi kiểu của một cột, thêm cột có mặc định không tất định. Trên mười triệu dòng, việc này giữ khoá độc quyền hàng PHÚT — một sự cố, không phải một cú migration.</span></div>
<div class="lz-layer"><span class="lz-lname">Quét nhưng không viết lại</span><span class="lz-lnote">Thêm <code>NOT NULL</code> hoặc một <code>CHECK</code>. Nhanh hơn, vẫn là khoá độc quyền. Hãy thêm ràng buộc dạng <code>NOT VALID</code> trước, rồi <code>VALIDATE</code> riêng — bước thứ hai lấy khoá NHẸ hơn.</span></div>
<div class="lz-layer"><span class="lz-lname">Hàng đợi khoá</span><span class="lz-lnote">Một lệnh <code>ALTER</code> đang CHỜ khoá sẽ chặn MỌI truy vấn đến sau nó. Một cú migration chậm đủ sức làm nghẽn cả những lượt đọc lẽ ra hoàn toàn ổn.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — thử migration trên một bảng có một nghìn dòng.</strong> Mọi lệnh <code>ALTER</code> đều tức thì trên dữ liệu phát triển, nên kế hoạch trông an toàn và con số bạn đo được thì vô nghĩa. Cùng câu lệnh ấy trên bảng production lấy một cái khoá TỈ LỆ với kích thước bảng, và kiểu hỏng không phải là một cú deploy chậm — mà là MỌI request hết giờ trong lúc migration còn đang chờ. Trước khi đẩy một thay đổi lược đồ, hãy kiểm số dòng trên production, tra xem thao tác ấy có viết lại bảng không, và đặt <code>lock_timeout</code> để một cú migration bị chặn hỏng SỚM thay vì kéo cả trang web xuống theo.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
