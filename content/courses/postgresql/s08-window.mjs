...
<h3>GROUP BY collapses; OVER does not</h3>
<div class="lz-stack">
<div class="lz-layer"><span class="lz-lname">GROUP BY user_id</span><span class="lz-lnote">Twelve notes become one row. The individual notes are gone — you can no longer show a title beside the count, because there is no single title left to show.</span></div>
<div class="lz-layer"><span class="lz-lname">COUNT(*) OVER (PARTITION BY user_id)</span><span class="lz-lnote">Twelve notes stay twelve rows, and each one carries the number 12 alongside it. Every original column is still available, because nothing was collapsed.</span></div>
<div class="lz-layer"><span class="lz-lname">the price</span><span class="lz-lnote">Window functions run <em>after</em> <code>WHERE</code> and <code>GROUP BY</code>, so you cannot filter on their result in <code>WHERE</code> — that is what the subquery wrapper in 8.2 is for.</span></div>
<div class="lz-layer"><span class="lz-lname">the tell</span><span class="lz-lnote">Any time you catch yourself joining a table back to an aggregate of itself, a window function is the shorter, faster answer.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — <code>PARTITION BY</code> read as <code>GROUP BY</code>.</strong> The words sound like synonyms and they are not: <code>PARTITION BY</code> divides the rows into windows for the calculation and then hands every row back, while <code>GROUP BY</code> keeps one row per group and discards the rest. Write <code>SELECT title, COUNT(*) OVER (PARTITION BY user_id)</code> and you get every title with its user's total beside it; write <code>GROUP BY user_id</code> and PostgreSQL refuses to let you select <code>title</code> at all. If you are unsure which you want, ask how many rows the answer should have — one per user, or one per note.</p></div>
...
<h3>GROUP BY gom lại; OVER thì không</h3>
<div class="lz-stack">
<div class="lz-layer"><span class="lz-lname">GROUP BY user_id</span><span class="lz-lnote">Mười hai note thành một dòng. Từng note biến mất — bạn không còn hiện được một cái title cạnh con đếm nữa, vì chẳng còn cái title đơn lẻ nào để hiện.</span></div>
<div class="lz-layer"><span class="lz-lname">COUNT(*) OVER (PARTITION BY user_id)</span><span class="lz-lnote">Mười hai note VẪN là mười hai dòng, và mỗi dòng mang theo con số 12 bên cạnh. Mọi cột gốc vẫn còn dùng được, vì không có gì bị gom cả.</span></div>
<div class="lz-layer"><span class="lz-lname">cái giá</span><span class="lz-lnote">Hàm cửa sổ chạy SAU <code>WHERE</code> và <code>GROUP BY</code>, nên bạn không lọc được theo kết quả của nó trong <code>WHERE</code> — đó là việc của lớp bọc subquery ở bài 8.2.</span></div>
<div class="lz-layer"><span class="lz-lname">dấu hiệu</span><span class="lz-lnote">Bất cứ khi nào bạn bắt gặp mình đang join một bảng ngược lại với một phép tổng hợp CỦA CHÍNH NÓ, thì hàm cửa sổ là câu trả lời ngắn hơn và nhanh hơn.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — đọc <code>PARTITION BY</code> thành <code>GROUP BY</code>.</strong> Hai chữ nghe như đồng nghĩa mà không phải: <code>PARTITION BY</code> CHIA các dòng thành các cửa sổ để tính rồi TRẢ LẠI mọi dòng, còn <code>GROUP BY</code> giữ một dòng mỗi nhóm và vứt phần còn lại. Viết <code>SELECT title, COUNT(*) OVER (PARTITION BY user_id)</code> thì bạn có MỌI title kèm tổng của user đó bên cạnh; viết <code>GROUP BY user_id</code> thì PostgreSQL từ chối cho bạn chọn <code>title</code> luôn. Nếu chưa chắc mình muốn cái nào, hãy hỏi: câu trả lời nên có BAO NHIÊU DÒNG — một dòng mỗi user, hay một dòng mỗi note?</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<h3>Three ranking functions, three answers to a tie</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">ROW_NUMBER()</span><span class="lz-d">1, 2, 3, 4 — always distinct, ties broken arbitrarily. Use it when you need exactly one row per partition and do not care which of the tied rows wins.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">RANK()</span><span class="lz-d">1, 2, 2, 4 — ties share a rank and the next value skips. This is the sports ranking: two silver medals, no bronze.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">DENSE_RANK()</span><span class="lz-d">1, 2, 2, 3 — ties share a rank and nothing is skipped. Use it when the rank number itself is shown to a user and gaps would look like a bug.</span></div>
<div class="lz-step"><span class="lz-k">→</span><span class="lz-t">Filtering needs a wrapper</span><span class="lz-d">"Top 3 per user" cannot be written in <code>WHERE</code>, because the rank does not exist yet. Compute it in a subquery or CTE, then filter the outer query on <code>rn &lt;= 3</code>.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — <code>ROW_NUMBER()</code> with an <code>ORDER BY</code> that is not deterministic.</strong> <code>ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY created_at DESC)</code> looks precise, and it is — until two notes share a timestamp. Then which one gets rank 1 is genuinely undefined, and PostgreSQL is free to answer differently on the next run, after a <code>VACUUM</code>, or on a replica. A "latest note per user" query that flickers between two values every few days is this bug, and it is maddening to reproduce. Always add a unique tiebreaker: <code>ORDER BY created_at DESC, id DESC</code>. It costs nothing and makes the result reproducible.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<h3>Ba hàm xếp hạng, ba câu trả lời cho một cú HOÀ</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">ROW_NUMBER()</span><span class="lz-d">1, 2, 3, 4 — luôn khác nhau, hoà thì phá tuỳ tiện. Dùng khi bạn cần ĐÚNG một dòng mỗi phân vùng và không quan tâm dòng nào trong số hoà thắng.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">RANK()</span><span class="lz-d">1, 2, 2, 4 — hoà thì chung hạng và giá trị kế tiếp NHẢY CÓC. Đây là cách xếp hạng thể thao: hai huy chương bạc, không có đồng.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">DENSE_RANK()</span><span class="lz-d">1, 2, 2, 3 — hoà thì chung hạng và KHÔNG nhảy cóc. Dùng khi chính con số hạng được hiện cho người dùng và khoảng trống sẽ trông như một con bug.</span></div>
<div class="lz-step"><span class="lz-k">→</span><span class="lz-t">Muốn LỌC thì cần lớp bọc</span><span class="lz-d">"Top 3 mỗi user" không viết được trong <code>WHERE</code>, vì lúc ấy thứ hạng chưa tồn tại. Tính nó trong một subquery hoặc CTE, rồi lọc ở truy vấn ngoài bằng <code>rn &lt;= 3</code>.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — <code>ROW_NUMBER()</code> với một <code>ORDER BY</code> không tất định.</strong> <code>ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY created_at DESC)</code> trông chính xác, và nó chính xác thật — cho tới khi hai note trùng dấu thời gian. Lúc đó cái nào nhận hạng 1 là THỰC SỰ không xác định, và PostgreSQL được phép trả lời khác ở lần chạy sau, sau một cú <code>VACUUM</code>, hoặc trên một bản sao. Một truy vấn "note mới nhất mỗi user" cứ vài ngày lại nhấp nháy giữa hai giá trị chính là con bug này, và nó điên người ở chỗ rất khó tái hiện. Luôn thêm một khoá phá hoà DUY NHẤT: <code>ORDER BY created_at DESC, id DESC</code>. Không tốn gì và làm kết quả tái lập được.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<h3>Looking sideways at neighbouring rows</h3>
<div class="lz-map">
<div class="lz-stage lz-badge">
<span class="lz-node"><span class="lz-ntitle">LAG / LEAD</span><span class="lz-nsub">the previous and next row</span></span>
<span class="lz-nbody">Day-over-day change, time between two events, "did this value change from the last reading". Both take a default: <code>LAG(total, 1, 0)</code> gives 0 instead of NULL on the first row, which usually saves a <code>COALESCE</code> later.</span>
</div>
<div class="lz-stage lz-badge">
<span class="lz-node"><span class="lz-ntitle">Running total</span><span class="lz-nsub">SUM with an ordered window</span></span>
<span class="lz-nbody"><code>SUM(amount) OVER (ORDER BY day)</code> accumulates as it goes. The <code>ORDER BY</code> inside <code>OVER</code> is what turns a total into a running total — without it you get the same grand total on every row.</span>
</div>
<div class="lz-stage lz-badge">
<span class="lz-node"><span class="lz-ntitle">FIRST_VALUE / LAST_VALUE</span><span class="lz-nsub">and why LAST_VALUE surprises</span></span>
<span class="lz-nbody">With an <code>ORDER BY</code> and no explicit frame, the window ends at the <em>current row</em> — so <code>LAST_VALUE</code> returns the current row, not the partition's last. Fixing it needs an explicit frame, which is exactly what 8.4 covers.</span>
</div>
</div>
<div class="pitfall"><p><strong>Trap — a running total over a series with gaps.</strong> <code>SUM(revenue) OVER (ORDER BY day)</code> accumulates over the rows that exist, not over the calendar. If your table has no row for a day with no sales, the chart simply skips that day — the running total stays flat and correct, but the x-axis lies, and a week with three quiet days looks like a four-day week. The fix belongs before the window function: generate the full date range with <code>generate_series</code> and <code>LEFT JOIN</code> your data onto it, so every day exists as a row before you accumulate. This is one of the few genuinely good uses of a <code>CROSS JOIN</code> (5.4).</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<h3>Nhìn ngang sang các dòng láng giềng</h3>
<div class="lz-map">
<div class="lz-stage lz-badge">
<span class="lz-node"><span class="lz-ntitle">LAG / LEAD</span><span class="lz-nsub">dòng trước và dòng sau</span></span>
<span class="lz-nbody">Chênh lệch ngày qua ngày, khoảng thời gian giữa hai sự kiện, "giá trị này có đổi so với lần đọc trước không". Cả hai đều nhận một giá trị mặc định: <code>LAG(total, 1, 0)</code> cho 0 thay vì NULL ở dòng đầu, thường tiết kiệm cho bạn một cú <code>COALESCE</code> về sau.</span>
</div>
<div class="lz-stage lz-badge">
<span class="lz-node"><span class="lz-ntitle">Tổng luỹ tiến</span><span class="lz-nsub">SUM với một cửa sổ CÓ THỨ TỰ</span></span>
<span class="lz-nbody"><code>SUM(amount) OVER (ORDER BY day)</code> cộng dồn khi đi tới. Chính cái <code>ORDER BY</code> BÊN TRONG <code>OVER</code> biến một con tổng thành tổng luỹ tiến — không có nó thì bạn nhận cùng một tổng lớn ở MỌI dòng.</span>
</div>
<div class="lz-stage lz-badge">
<span class="lz-node"><span class="lz-ntitle">FIRST_VALUE / LAST_VALUE</span><span class="lz-nsub">và vì sao LAST_VALUE gây bất ngờ</span></span>
<span class="lz-nbody">Có <code>ORDER BY</code> mà không khai khung tường minh thì cửa sổ KẾT THÚC Ở DÒNG HIỆN TẠI — nên <code>LAST_VALUE</code> trả về chính dòng hiện tại, không phải dòng cuối của phân vùng. Chữa nó cần một khung tường minh, đúng thứ bài 8.4 nói tới.</span>
</div>
</div>
<div class="pitfall"><p><strong>Bẫy — tổng luỹ tiến trên một chuỗi có LỖ HỔNG.</strong> <code>SUM(revenue) OVER (ORDER BY day)</code> cộng dồn trên các DÒNG ĐANG CÓ, không phải trên tờ lịch. Nếu bảng của bạn không có dòng nào cho một ngày không bán được gì, biểu đồ đơn giản là BỎ QUA ngày ấy — tổng luỹ tiến vẫn phẳng và vẫn đúng, nhưng trục x thì nói dối, và một tuần có ba ngày ế trông như một tuần bốn ngày. Cách chữa nằm TRƯỚC hàm cửa sổ: sinh trọn dải ngày bằng <code>generate_series</code> rồi <code>LEFT JOIN</code> dữ liệu vào đó, để mọi ngày đều tồn tại dưới dạng một dòng trước khi bạn cộng dồn. Đây là một trong số ít công dụng thật sự tốt của <code>CROSS JOIN</code> (5.4).</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<h3>The frame is what most people never set</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">No ORDER BY inside OVER</span><span class="lz-d">The frame is the whole partition. <code>SUM(x) OVER (PARTITION BY u)</code> gives every row that user's grand total — the same number repeated.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">With ORDER BY, the default frame appears</span><span class="lz-d">It becomes <code>RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW</code>. You did not write it, and it is why the total starts running.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">RANGE groups peers; ROWS counts rows</span><span class="lz-d">With ties, <code>RANGE</code> includes every row sharing the current value — so all of them see the same running total. <code>ROWS</code> counts physical rows and does not. On a column with duplicates the two give different answers.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">A moving average needs an explicit frame</span><span class="lz-d"><code>ROWS BETWEEN 6 PRECEDING AND CURRENT ROW</code> is a 7-day average. Nothing about the default frame gives you this; you must say it.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — a moving average that is wrong at the edges and nobody notices.</strong> <code>ROWS BETWEEN 6 PRECEDING AND CURRENT ROW</code> on day 3 of a series averages three rows, not seven — and reports it as a "7-day average". The first six points of every such chart are computed on a shorter window, which is why new products and new users always appear to start with wild swings that settle down. That settling is an artefact of the frame, not a real trend. If the distinction matters, guard it: <code>CASE WHEN COUNT(*) OVER (…same frame…) = 7 THEN AVG(…) END</code> returns NULL until a full window exists, and a gap at the start of a chart is far more honest than a number computed from three days.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<h3>Cái KHUNG là thứ phần lớn người ta không bao giờ đặt</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">Không có ORDER BY trong OVER</span><span class="lz-d">Khung là TRỌN phân vùng. <code>SUM(x) OVER (PARTITION BY u)</code> cho mỗi dòng con tổng lớn của user ấy — cùng một con số lặp lại.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">Có ORDER BY thì khung MẶC ĐỊNH hiện ra</span><span class="lz-d">Nó thành <code>RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW</code>. Bạn không viết dòng ấy, và đó là lý do con tổng bắt đầu luỹ tiến.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">RANGE gom các dòng NGANG NHAU; ROWS đếm DÒNG</span><span class="lz-d">Khi có hoà, <code>RANGE</code> gồm MỌI dòng chia sẻ giá trị hiện tại — nên tất cả chúng thấy CÙNG một tổng luỹ tiến. <code>ROWS</code> đếm dòng vật lý và không như vậy. Trên một cột có trùng lặp, hai cái cho hai câu trả lời khác nhau.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">Trung bình trượt CẦN khung tường minh</span><span class="lz-d"><code>ROWS BETWEEN 6 PRECEDING AND CURRENT ROW</code> là trung bình 7 ngày. Không có gì trong khung mặc định cho bạn thứ này; bạn PHẢI nói ra.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — một trung bình trượt sai ở HAI ĐẦU mà không ai để ý.</strong> <code>ROWS BETWEEN 6 PRECEDING AND CURRENT ROW</code> ở ngày thứ 3 của chuỗi thì lấy trung bình BA dòng, không phải bảy — và báo cáo nó là "trung bình 7 ngày". Sáu điểm đầu của MỌI biểu đồ kiểu ấy đều được tính trên một cửa sổ ngắn hơn, và đó là lý do sản phẩm mới và người dùng mới lúc nào cũng trông như khởi đầu bằng những dao động dữ dội rồi lắng dần. Cái "lắng dần" ấy là hiện vật của cái KHUNG, không phải một xu hướng thật. Nếu phân biệt ấy quan trọng, hãy chốt lại: <code>CASE WHEN COUNT(*) OVER (…cùng khung…) = 7 THEN AVG(…) END</code> trả NULL cho tới khi có đủ một cửa sổ trọn vẹn, và một khoảng trống ở đầu biểu đồ trung thực hơn nhiều so với một con số tính từ ba ngày.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
