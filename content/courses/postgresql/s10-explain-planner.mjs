...
<h3>Reading a plan from the inside out</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">The deepest indented node runs first</span><span class="lz-d">A plan is a tree printed upside down. Start at the most indented line, not the top — that is where the data actually enters.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">cost=start..total is an estimate</span><span class="lz-d">Arbitrary units, not milliseconds. The first number is the cost to return the first row, the second to return all of them. Useful for comparing two plans, meaningless as an absolute.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">rows= is the estimate; actual rows= is the truth</span><span class="lz-d">Only <code>EXPLAIN ANALYZE</code> gives you both. The gap between them is the single most useful number in the whole output.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">loops= multiplies everything</span><span class="lz-d">A node showing <code>actual time=0.03..0.04 rows=1 loops=5000</code> did not take 0.04ms — it took 200ms. Always multiply by loops before deciding a node is cheap.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — running <code>EXPLAIN ANALYZE</code> on an <code>UPDATE</code> or <code>DELETE</code> in production.</strong> Plain <code>EXPLAIN</code> only plans; <code>EXPLAIN ANALYZE</code> genuinely <em>executes</em> the statement. On a <code>SELECT</code> that is harmless. On a <code>DELETE FROM order WHERE …</code> it deletes the rows, and the plan you print is the receipt. Wrap it in a transaction you roll back — <code>BEGIN; EXPLAIN ANALYZE DELETE …; ROLLBACK;</code> — and make that a habit rather than something you remember to do on the dangerous ones, because the dangerous one is exactly where you will forget.</p></div>
...
<h3>Đọc một kế hoạch từ TRONG ra</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">Nút thụt sâu nhất chạy TRƯỚC</span><span class="lz-d">Một kế hoạch là một cái cây in ngược. Bắt đầu ở dòng thụt sâu nhất, đừng bắt đầu từ trên xuống — đó mới là chỗ dữ liệu thật sự đi vào.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">cost=đầu..tổng là một ƯỚC LƯỢNG</span><span class="lz-d">Đơn vị quy ước, không phải mili-giây. Số đầu là chi phí để trả về dòng ĐẦU TIÊN, số sau là để trả về HẾT. Hữu ích để so hai kế hoạch, vô nghĩa khi đứng một mình.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">rows= là ước lượng; actual rows= là SỰ THẬT</span><span class="lz-d">Chỉ <code>EXPLAIN ANALYZE</code> mới cho bạn cả hai. KHOẢNG CÁCH giữa chúng là con số hữu ích nhất trong toàn bộ đầu ra.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">loops= NHÂN mọi thứ lên</span><span class="lz-d">Một nút ghi <code>actual time=0.03..0.04 rows=1 loops=5000</code> KHÔNG tốn 0,04ms — nó tốn 200ms. Luôn nhân với loops trước khi kết luận một nút là rẻ.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — chạy <code>EXPLAIN ANALYZE</code> trên một lệnh <code>UPDATE</code> hay <code>DELETE</code> ở production.</strong> <code>EXPLAIN</code> trần chỉ lập kế hoạch; <code>EXPLAIN ANALYZE</code> THẬT SỰ THI HÀNH câu lệnh. Trên một <code>SELECT</code> thì vô hại. Trên một <code>DELETE FROM order WHERE …</code> thì nó XOÁ các dòng đó, và cái kế hoạch bạn in ra chính là tờ biên lai. Hãy bọc nó trong một giao dịch rồi cuộn ngược — <code>BEGIN; EXPLAIN ANALYZE DELETE …; ROLLBACK;</code> — và biến nó thành THÓI QUEN chứ đừng là thứ bạn nhớ làm với những câu nguy hiểm, vì câu nguy hiểm chính là chỗ bạn sẽ quên.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<div class="pitfall"><p><strong>Trap — reading "Nested Loop" as a problem.</strong> The name sounds like the O(n²) loop everyone is taught to fear, and people rewrite perfectly good queries to escape it. A nested loop with an index on the inner side is often the <em>fastest</em> possible plan: it reads a handful of outer rows and does one index lookup each. What actually signals trouble is a nested loop whose inner node shows a sequential scan, or a huge <code>loops=</code> count — that is the O(n²) case, and it is caused by a missing index, not by the join strategy. Judge the plan by <code>loops=</code> and by whether the inner side uses an index, never by the name of the node.</p></div>
<h3>Three join strategies and the shape each suits</h3>
<div class="lz-stack">
<div class="lz-layer"><span class="lz-lname">Nested Loop</span><span class="lz-lnote">For each outer row, look up the inner side. Excellent when the outer side is small <em>and</em> the inner side has an index — a handful of lookups. Catastrophic when the inner side is a sequential scan.</span></div>
<div class="lz-layer"><span class="lz-lname">Hash Join</span><span class="lz-lnote">Build a hash table from the smaller side, then stream the larger one past it. The usual winner for joining two big tables on equality, and it needs <code>work_mem</code> to hold the hash.</span></div>
<div class="lz-layer"><span class="lz-lname">Merge Join</span><span class="lz-lnote">Walk both sides in sorted order. Cheap when both are already sorted — typically because both sides are being read through an index — and expensive when it has to sort them first.</span></div>
<div class="lz-layer"><span class="lz-lname">who decides</span><span class="lz-lnote">The planner, from the row estimates. That is why a wrong estimate produces a wrong strategy, and why <code>ANALYZE</code> (10.3) fixes plans that looked inexplicable.</span></div>
</div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<div class="pitfall"><p><strong>Bẫy — đọc "Nested Loop" thành một VẤN ĐỀ.</strong> Cái tên nghe như vòng lặp O(n²) mà ai cũng được dạy phải sợ, nên người ta viết lại những truy vấn vốn hoàn toàn tốt chỉ để thoát khỏi nó. Một nested loop CÓ chỉ mục ở phía trong thường là kế hoạch NHANH NHẤT có thể: nó đọc một nhúm dòng ngoài rồi mỗi dòng tra chỉ mục đúng một lần. Thứ thật sự báo hiệu rắc rối là một nested loop mà nút bên trong hiện quét tuần tự, hoặc một con số <code>loops=</code> khổng lồ — ĐÓ mới là trường hợp O(n²), và nó do THIẾU CHỈ MỤC gây ra chứ không phải do chiến lược join. Hãy đánh giá kế hoạch bằng <code>loops=</code> và bằng việc phía trong có dùng chỉ mục hay không, đừng bao giờ đánh giá bằng TÊN của nút.</p></div>
<h3>Ba chiến lược join và hình dạng phù hợp với mỗi cái</h3>
<div class="lz-stack">
<div class="lz-layer"><span class="lz-lname">Nested Loop</span><span class="lz-lnote">Với mỗi dòng ngoài, tra một lần ở phía trong. Tuyệt vời khi phía ngoài NHỎ <em>và</em> phía trong CÓ chỉ mục — chỉ một nhúm lượt tra. Thảm hoạ khi phía trong là một lượt quét tuần tự.</span></div>
<div class="lz-layer"><span class="lz-lname">Hash Join</span><span class="lz-lnote">Dựng một bảng băm từ phía NHỎ hơn, rồi cho phía lớn chảy qua nó. Kẻ thắng thông thường khi join hai bảng lớn theo phép bằng, và nó cần <code>work_mem</code> để chứa bảng băm.</span></div>
<div class="lz-layer"><span class="lz-lname">Merge Join</span><span class="lz-lnote">Đi song song hai phía theo thứ tự đã sắp. Rẻ khi cả hai VỐN đã sắp — thường là vì cả hai phía đều đang được đọc qua chỉ mục — và đắt khi nó phải tự sắp trước.</span></div>
<div class="lz-layer"><span class="lz-lname">ai quyết định</span><span class="lz-lnote">Planner, dựa trên ƯỚC LƯỢNG SỐ DÒNG. Đó là lý do một ước lượng sai sinh ra một chiến lược sai, và là lý do <code>ANALYZE</code> (10.3) chữa được những kế hoạch trông không giải thích nổi.</span></div>
</div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<div class="pitfall"><p><strong>Trap — raising <code>work_mem</code> globally because one query spilled to disk.</strong> Seeing "Sort Method: external merge Disk: 45MB" in a plan is a real signal, and the obvious fix — bump <code>work_mem</code> in <code>postgresql.conf</code> — is the dangerous one. That setting is per sort, per node, per connection: a single query with three sorts across 100 connections can allocate 300× your value. Raising it from 4MB to 256MB looks harmless and can exhaust the server's memory under load, which arrives as the OOM killer rather than as a slow query. Set it for the one session or the one query that needs it — <code>SET LOCAL work_mem = '256MB'</code> inside a transaction — and leave the global default alone.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<div class="pitfall"><p><strong>Bẫy — nâng <code>work_mem</code> TOÀN CỤC vì một truy vấn tràn ra đĩa.</strong> Thấy "Sort Method: external merge Disk: 45MB" trong kế hoạch là một tín hiệu THẬT, và cách chữa hiển nhiên — tăng <code>work_mem</code> trong <code>postgresql.conf</code> — lại là cách nguy hiểm. Tham số ấy tính theo TỪNG phép sắp, TỪNG nút, TỪNG kết nối: một truy vấn có ba phép sắp trên 100 kết nối có thể cấp phát gấp 300 lần giá trị bạn đặt. Nâng từ 4MB lên 256MB trông vô hại và có thể vắt kiệt bộ nhớ máy chủ khi có tải, thứ tới dưới dạng OOM killer chứ không phải dưới dạng một truy vấn chậm. Hãy đặt nó cho ĐÚNG phiên hoặc ĐÚNG truy vấn cần — <code>SET LOCAL work_mem = '256MB'</code> bên trong một giao dịch — và để yên giá trị mặc định toàn cục.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<div class="pitfall"><p><strong>Trap — timing a query twice and believing the second number.</strong> The first run reads from disk; the second finds everything in the buffer cache and the OS page cache, and can be ten times faster. Optimise against that second number and you have tuned for a state production may never be in — a query that runs in 8ms warm and 800ms cold is an 800ms query for the user who arrives first each morning. <code>EXPLAIN (ANALYZE, BUFFERS)</code> is what tells you which you are looking at: <code>shared hit</code> means cache, <code>shared read</code> means disk. Report both numbers, and size your expectations by the cold one.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
...
<div class="pitfall"><p><strong>Bẫy — bấm giờ một truy vấn hai lần rồi tin con số thứ hai.</strong> Lần chạy đầu đọc từ đĩa; lần thứ hai tìm thấy mọi thứ trong buffer cache và page cache của hệ điều hành, và có thể nhanh gấp mười. Tối ưu theo con số thứ hai là bạn đã tinh chỉnh cho một trạng thái mà production có thể không bao giờ ở trong đó — một truy vấn chạy 8ms lúc ấm và 800ms lúc nguội là một truy vấn 800ms đối với người dùng đến đầu tiên mỗi sáng. <code>EXPLAIN (ANALYZE, BUFFERS)</code> chính là thứ cho bạn biết mình đang nhìn cái nào: <code>shared hit</code> là cache, <code>shared read</code> là đĩa. Hãy báo cáo CẢ HAI con số, và đặt kỳ vọng theo con số NGUỘI.</p></div>
<a class="link-card codelab" href="/code-lab/postgresql${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">🗄️</span>
