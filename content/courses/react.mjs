...
import s10 from './react/s10-du-an.mjs';
...
export default {
  ...
  course: {
    ...
    description: 'Khoá React nền tảng với TypeScript và Vite. Đi từ JSX và component, props, state và cách React render lại, danh sách và key, sự kiện, form (kiểm soát, React Hook Form + Zod), effect và vòng đời (và vì sao bạn cần ít effect hơn bạn nghĩ), context và quản lý state (Zustand), lấy dữ liệu với TanStack Query, routing, hiệu năng (memo, lazy, đo bằng Profiler), khả năng tiếp cận, testing, tới dự án cuối khoá giao diện đặt lịch khám. Là nền trước khi học khoá Next.js. Học tiếp sau khoá này: Next.js (/courses/nextjs) — framework dựng trên React, thêm định tuyến theo thư mục, Server Component, cache dữ liệu, SEO và deploy; mọi thứ học ở đây dùng nguyên được bên đó. Đang học FER202 ở trường: học FER202 (Academy) trước để qua môn, rồi học khoá này để làm theo cách công ty làm.',
    ...
    requirements: 'HTML, CSS, JavaScript cơ bản (khoá Web Foundations là đủ). Nên biết TypeScript cơ bản (khoá TypeScript). Học xong khoá này thì học tiếp Next.js: The React Framework.',
