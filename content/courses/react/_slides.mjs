/**
 * _slides.mjs — nhúng ảnh slide của khoá "React" vào bài học.
 *
 * Slide do khoá tự soạn, dựng bằng
 *   node scripts/_render-slides.mjs --deck scripts/slides-src/rx-NN.mjs --out <dir>
 * rồi đẩy lên R2 bằng
 *   node --env-file=.env scripts/upload-academy-slides.mjs --dir <dir> --prefix RX/v1 --decks rx-NN
 * ⇒ images/academy/RX/v1/<deck>/<NNN>.webp
 *
 ...
 * sang prefix mới (RX/v2) và ghi deck đó vào VER bên dưới — đừng ghi đè v1.
 *
 * Việc "slide số n có thật không" được kiểm ở bước ghép bằng
 * `node scripts/rx-ghep-chuong.mjs <file chương> --render <dir> [--cdn]`.
 */
export const CDN_ROOT = 'https://media.cuongthai.com/images/academy/RX';

/** Deck nào đã phải dựng lại thì ghi phiên bản ở đây, vd { 'rx-05': 'v2' }. */
const VER = {};
...
export function slide(deck, n, caption) {
  if (!/^rx-\d\d$/.test(deck)) throw new Error(`deck lạ: ${deck}`);
