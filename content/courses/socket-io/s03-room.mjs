...
<div class="callout">
<p><strong>One sentence.</strong> Room là tag string routing per-event, <code>io.to(A).to(B)</code> là intersection (dùng <code>[A, B]</code> cho union), kho này có 4 pattern (user, resource, feature-instance, device) — và mọi socket tự động ở room mang sid của nó (nên <code>socket.rooms.size ≥ 1</code>).</p>
</div>
...
<div class="callout">
<p><strong>One sentence.</strong> Room tự tạo lúc join đầu và tự xoá lúc leave cuối — không có event &quot;room deleted&quot; nào fire, nên state tuỳ biến (<code>hostBySid</code>, <code>hostState</code>) phải được cleanup thủ công ở handler <code>disconnect</code> hoặc <code>leave</code>, không thì leak dần theo mỗi ephemeral room bị bỏ.</p>
</div>
...
<div class="callout">
<p><strong>One sentence.</strong> <code>io.emit</code> đến tất, <code>io.to(room)</code> đến room, <code>io.to(A).to(B)</code> là intersection, <code>[A, B]</code> là union, <code>socket.to</code> loại trừ chính socket, <code>except(ids)</code> loại có chọn, <code>volatile</code> drop nếu chậm — chọn nhầm cái nào cũng là bug rất khó phát hiện ở dev nhưng bung ở production.</p>
</div>
...
<div class="callout">
<p><strong>One sentence.</strong> Convention <code>&lt;feature&gt;:&lt;verb&gt;</code> tránh xung đột tên event giữa 5 gateway của kho này (15 emit + 21 listen, 0 clash), cho grep dễ tìm mọi thứ liên quan feature, và tránh reserved names (<code>connect</code>, <code>disconnect</code>, <code>error</code>, <code>ping</code>) mà override sẽ vỡ cơ chế bên dưới.</p>
</div>
