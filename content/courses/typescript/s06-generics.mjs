...
<h3>What a type parameter buys you</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">any: the relationship is destroyed</span><span class="lz-d"><code>function first(a: any[]): any</code> forgets that the output came from the input. Every caller gets <code>any</code> back and every downstream mistake goes unchecked.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">Overloads: the relationship is enumerated</span><span class="lz-d">One signature per element type. Correct, but you must know the list in advance and it grows forever.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">&lt;T&gt;: the relationship is stated once</span><span class="lz-d"><code>function first&lt;T&gt;(a: T[]): T</code> says "whatever the element type is, that is the return type" — a rule, not a list.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">Inference fills it in at each call</span><span class="lz-d">Call it with <code>string[]</code> and <code>T</code> is <code>string</code> for that call only. You almost never write the type argument yourself.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — writing <code>&lt;T&gt;</code> but annotating the parameter as <code>any[]</code>.</strong> <code>function first&lt;T&gt;(a: any[]): T</code> looks generic and is worse than <code>any</code>: <code>T</code> appears only in the return position, so there is nothing to infer it from, and it defaults to <code>unknown</code> — or, if the caller writes <code>first&lt;User&gt;(rows)</code>, to whatever they claimed, with no check at all. That signature is a cast wearing a generic's clothes: <code>first&lt;User&gt;(['a','b'])</code> compiles and hands you a string typed as a User. A type parameter has to appear in the <em>input</em> for the compiler to have anything to solve. Rule of thumb from lesson 6.4: a type parameter used exactly once is not doing any work.</p></div>
...
<h3>Một tham số kiểu mua cho bạn cái gì</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">any: quan hệ bị phá huỷ</span><span class="lz-d"><code>function first(a: any[]): any</code> quên mất rằng đầu ra đến từ đầu vào. Mọi chỗ gọi nhận lại <code>any</code> và mọi sai lầm phía sau đều không được kiểm.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">Overload: quan hệ bị liệt kê ra</span><span class="lz-d">Mỗi kiểu phần tử một chữ ký. Đúng, nhưng bạn phải biết trước danh sách và nó cứ dài mãi.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">&lt;T&gt;: quan hệ được phát biểu một lần</span><span class="lz-d"><code>function first&lt;T&gt;(a: T[]): T</code> nói "kiểu phần tử là gì thì kiểu trả về là cái đó" — một luật, không phải một danh sách.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">Suy luận điền vào ở từng chỗ gọi</span><span class="lz-d">Gọi nó với <code>string[]</code> thì <code>T</code> là <code>string</code> chỉ cho lần gọi đó. Bạn gần như chẳng bao giờ phải tự viết đối số kiểu.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — viết <code>&lt;T&gt;</code> nhưng gán kiểu tham số là <code>any[]</code>.</strong> <code>function first&lt;T&gt;(a: any[]): T</code> nhìn có vẻ generic mà còn tệ hơn <code>any</code>: <code>T</code> chỉ xuất hiện ở vị trí trả về nên chẳng có gì để suy ra nó, và nó rơi về mặc định <code>unknown</code> — hoặc, nếu chỗ gọi viết <code>first&lt;User&gt;(rows)</code>, thì rơi về đúng thứ họ khai, không kiểm gì hết. Chữ ký đó là một phép ép kiểu khoác áo generic: <code>first&lt;User&gt;(['a','b'])</code> biên dịch được và đưa cho bạn một chuỗi mang kiểu User. Tham số kiểu phải xuất hiện ở <em>đầu vào</em> thì trình biên dịch mới có gì để giải. Quy tắc ngón tay cái từ bài 6.4: một tham số kiểu chỉ dùng đúng một lần là nó chẳng làm việc gì cả.</p></div>
<a class="link-card codelab" href="/code-lab/typescript${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">⌨️</span>
...
<h3>keyof and T[K], step by step</h3>
<div class="lz-stack">
<div class="lz-layer"><span class="lz-k">T extends object</span><span class="lz-t">Bound the shape</span><span class="lz-d">Without a constraint, <code>T</code> could be anything, so you may not touch a single member of it. The constraint is what unlocks access.</span></div>
<div class="lz-layer"><span class="lz-k">keyof T</span><span class="lz-t">The union of its key names</span><span class="lz-d">For <code>{ id: string; age: number }</code> it is <code>'id' | 'age'</code>. A literal union — so <code>===</code> and <code>switch</code> narrow it, and a typo is a compile error.</span></div>
<div class="lz-layer"><span class="lz-k">K extends keyof T</span><span class="lz-t">One specific key, chosen by the caller</span><span class="lz-d">Inferred from the argument at each call site: <code>get(user, 'age')</code> makes <code>K</code> exactly <code>'age'</code>, not the whole union.</span></div>
<div class="lz-layer"><span class="lz-k">T[K]</span><span class="lz-t">The type stored at that key</span><span class="lz-d">An indexed access type. With <code>K = 'age'</code> it resolves to <code>number</code> — so the getter returns exactly the right type per call, not a union of all of them.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — <code>keyof</code> on a type with an index signature collapses to <code>string | number</code>.</strong> For <code>type Bag = Record&lt;string, number&gt;</code>, <code>keyof Bag</code> is <code>string | number</code>, not a list of keys — because an index signature says "any key". So a getter typed <code>get&lt;T, K extends keyof T&gt;</code> accepts <em>any string at all</em> for such a T, and <code>get(bag, 'typoed')</code> compiles and returns <code>number</code> at a key that does not exist. The type never lied; the index signature genuinely promises every key. If you need key checking, close the key set (<code>Record&lt;'a' | 'b', number&gt;</code>) or drop the index signature and declare the properties.</p></div>
<a class="link-card codelab" href="/code-lab/typescript${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">⌨️</span>
...
<h3>keyof và T[K], từng bước</h3>
<div class="lz-stack">
<div class="lz-layer"><span class="lz-k">T extends object</span><span class="lz-t">Chặn dáng lại</span><span class="lz-d">Không có ràng buộc thì <code>T</code> có thể là bất cứ gì, nên bạn không được đụng vào một thành viên nào của nó. Chính ràng buộc mới mở khoá quyền truy cập.</span></div>
<div class="lz-layer"><span class="lz-k">keyof T</span><span class="lz-t">Union các tên khoá của nó</span><span class="lz-d">Với <code>{ id: string; age: number }</code> nó là <code>'id' | 'age'</code>. Một union literal — nên <code>===</code> và <code>switch</code> thu hẹp được nó, và gõ nhầm là lỗi biên dịch.</span></div>
<div class="lz-layer"><span class="lz-k">K extends keyof T</span><span class="lz-t">Một khoá cụ thể, do chỗ gọi chọn</span><span class="lz-d">Suy ra từ đối số ở từng chỗ gọi: <code>get(user, 'age')</code> làm <code>K</code> đúng bằng <code>'age'</code>, chứ không phải cả union.</span></div>
<div class="lz-layer"><span class="lz-k">T[K]</span><span class="lz-t">Kiểu nằm ở khoá đó</span><span class="lz-d">Một indexed access type. Với <code>K = 'age'</code> nó rút về <code>number</code> — nên getter trả đúng kiểu ở từng lần gọi, không phải union của tất cả.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — <code>keyof</code> trên một kiểu có index signature sụp xuống thành <code>string | number</code>.</strong> Với <code>type Bag = Record&lt;string, number&gt;</code>, <code>keyof Bag</code> là <code>string | number</code>, không phải một danh sách khoá — vì index signature nói "khoá nào cũng được". Nên một getter kiểu <code>get&lt;T, K extends keyof T&gt;</code> nhận <em>bất kỳ chuỗi nào</em> với một T như thế, và <code>get(bag, 'goinham')</code> biên dịch được rồi trả về <code>number</code> ở một khoá không tồn tại. Kiểu chưa hề nói dối; index signature quả thật hứa là khoá nào cũng có. Nếu cần kiểm khoá, hãy đóng tập khoá lại (<code>Record&lt;'a' | 'b', number&gt;</code>) hoặc bỏ index signature đi mà khai từng thuộc tính.</p></div>
<a class="link-card codelab" href="/code-lab/typescript${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">⌨️</span>
...
<h3>Where the type parameter lives</h3>
<div class="lz-map">
<div class="lz-node"><span class="lz-k">On the function</span><span class="lz-t">Fresh per call</span><span class="lz-d"><code>function first&lt;T&gt;(a: T[])</code>. <code>T</code> is solved anew at every call site and forgotten afterwards.</span></div>
<div class="lz-node"><span class="lz-k">On the interface / alias</span><span class="lz-t">Fixed when you name the type</span><span class="lz-d"><code>Box&lt;string&gt;</code> is one concrete type. Every use of that name carries the same <code>T</code>.</span></div>
<div class="lz-node"><span class="lz-k">On the class</span><span class="lz-t">Fixed per instance</span><span class="lz-d"><code>new Stack&lt;number&gt;()</code> binds <code>T</code> for the object's whole lifetime — every method shares it, which is what makes <code>push</code>/<code>pop</code> agree.</span></div>
<div class="lz-node"><span class="lz-k">Default: &lt;T = string&gt;</span><span class="lz-t">Used only when nothing else decides</span><span class="lz-d">A default is a fallback for the <em>unannotated, uninferable</em> case — it never overrides inference, and it never overrides an explicit argument.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — a generic class with no way to infer T silently becomes <code>unknown</code>.</strong> <code>class Stack&lt;T&gt; { private items: T[] = []; … }</code> constructed as <code>const s = new Stack()</code> — no type argument, no constructor parameter mentioning <code>T</code> — leaves nothing to infer from, so <code>T</code> is <code>unknown</code>. Then <code>s.push(1)</code> is fine (everything is assignable to <code>unknown</code>) and <code>s.pop()! + 1</code> is TS18046 <em>'x' is of type 'unknown'</em>, an error at the consumer far from the <code>new</code> that caused it. Write <code>new Stack&lt;number&gt;()</code>, or annotate the variable, or give the class a default type parameter — and if the class is always used one way, consider not making it generic at all.</p></div>
<a class="link-card codelab" href="/code-lab/typescript${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">⌨️</span>
...
<h3>Tham số kiểu sống ở đâu</h3>
<div class="lz-map">
<div class="lz-node"><span class="lz-k">Trên hàm</span><span class="lz-t">Tươi mới ở mỗi lần gọi</span><span class="lz-d"><code>function first&lt;T&gt;(a: T[])</code>. <code>T</code> được giải lại từ đầu ở từng chỗ gọi rồi quên đi.</span></div>
<div class="lz-node"><span class="lz-k">Trên interface / alias</span><span class="lz-t">Cố định khi bạn gọi tên kiểu</span><span class="lz-d"><code>Box&lt;string&gt;</code> là một kiểu cụ thể. Mọi lần dùng cái tên đó đều mang cùng một <code>T</code>.</span></div>
<div class="lz-node"><span class="lz-k">Trên class</span><span class="lz-t">Cố định theo từng thực thể</span><span class="lz-d"><code>new Stack&lt;number&gt;()</code> gắn <code>T</code> cho suốt đời object — mọi phương thức dùng chung nó, và đó chính là thứ làm <code>push</code>/<code>pop</code> khớp nhau.</span></div>
<div class="lz-node"><span class="lz-k">Mặc định: &lt;T = string&gt;</span><span class="lz-t">Chỉ dùng khi không còn gì quyết định</span><span class="lz-d">Mặc định là phương án dự phòng cho trường hợp <em>không gán kiểu, không suy ra được</em> — nó không bao giờ đè lên suy luận, cũng không đè lên đối số tường minh.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — một class generic không có đường nào để suy ra T sẽ âm thầm thành <code>unknown</code>.</strong> <code>class Stack&lt;T&gt; { private items: T[] = []; … }</code> dựng bằng <code>const s = new Stack()</code> — không đối số kiểu, không tham số constructor nào nhắc tới <code>T</code> — thì chẳng còn gì để suy ra, nên <code>T</code> là <code>unknown</code>. Rồi <code>s.push(1)</code> vẫn ổn (mọi thứ đều gán được vào <code>unknown</code>) còn <code>s.pop()! + 1</code> là TS18046 <em>'x' is of type 'unknown'</em>, một lỗi ở chỗ tiêu thụ, xa tít cái <code>new</code> đã gây ra nó. Hãy viết <code>new Stack&lt;number&gt;()</code>, hoặc gán kiểu cho biến, hoặc cho class một tham số kiểu mặc định — và nếu class luôn được dùng theo đúng một cách, hãy cân nhắc đừng làm nó generic nữa.</p></div>
<a class="link-card codelab" href="/code-lab/typescript${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">⌨️</span>
...
<h3>Is this type parameter earning its place?</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">Count its appearances</span><span class="lz-d">Two or more (at least one in a parameter) means it is relating things. Exactly one means it is relating nothing.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">One appearance, in the return type</span><span class="lz-d">This is an unchecked cast with extra syntax. The caller declares the answer and the compiler agrees. Return <code>unknown</code> instead and make them narrow.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">One appearance, in a parameter</span><span class="lz-d">Then the constraint alone would have done the job: <code>(x: object)</code> instead of <code>&lt;T extends object&gt;(x: T)</code>. Simpler signature, same safety.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">Two or more, connected</span><span class="lz-d"><code>map&lt;T, U&gt;(a: T[], f: (x: T) =&gt; U): U[]</code> — every parameter is anchored to an input and pinned to an output. This is a generic doing real work.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — a typed <code>fetch</code> wrapper proves nothing about the response body.</strong> <code>async function getJson&lt;T&gt;(url: string): Promise&lt;T&gt;</code> is the single-use type parameter above, at its most dangerous: <code>T</code> comes only from the caller, and <code>res.json()</code> returns <code>any</code>, so the whole "type" is a wish. The server ships a renamed field, the type still says <code>User</code>, and you get <code>undefined</code> at a property TypeScript swore existed. The wrapper is fine as ergonomics but must not be mistaken for validation: return <code>unknown</code> and parse at the boundary (chapter 13 does this with Zod), so the one place that can actually check the bytes is the one place that assigns the type.</p></div>
<a class="link-card codelab" href="/code-lab/typescript${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">⌨️</span>
...
<h3>Tham số kiểu này có xứng chỗ của nó không?</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">Đếm số lần nó xuất hiện</span><span class="lz-d">Từ hai lần trở lên (ít nhất một ở tham số) nghĩa là nó đang nối các thứ lại. Đúng một lần nghĩa là nó chẳng nối gì cả.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">Một lần, ở kiểu trả về</span><span class="lz-d">Đây là một phép ép kiểu không kiểm, kèm thêm cú pháp. Chỗ gọi tự khai đáp án và trình biên dịch gật đầu. Hãy trả <code>unknown</code> và bắt họ thu hẹp.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">Một lần, ở tham số</span><span class="lz-d">Thế thì riêng ràng buộc đã đủ việc: <code>(x: object)</code> thay vì <code>&lt;T extends object&gt;(x: T)</code>. Chữ ký gọn hơn, an toàn như nhau.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">Từ hai lần trở lên, có nối nhau</span><span class="lz-d"><code>map&lt;T, U&gt;(a: T[], f: (x: T) =&gt; U): U[]</code> — mọi tham số đều neo vào một đầu vào và ghim vào một đầu ra. Đây mới là generic làm việc thật.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — một wrapper <code>fetch</code> có kiểu chẳng chứng minh gì về thân phản hồi.</strong> <code>async function getJson&lt;T&gt;(url: string): Promise&lt;T&gt;</code> chính là cái tham số kiểu dùng-một-lần ở trên, ở dạng nguy hiểm nhất: <code>T</code> chỉ đến từ chỗ gọi, còn <code>res.json()</code> trả về <code>any</code>, nên cả cái "kiểu" ấy chỉ là một điều ước. Máy chủ đổi tên một field, kiểu vẫn nói <code>User</code>, và bạn nhận <code>undefined</code> ở một thuộc tính mà TypeScript đã thề là có. Wrapper này ổn ở mặt tiện tay nhưng đừng nhầm nó với kiểm tra dữ liệu: hãy trả <code>unknown</code> và phân tích ở biên (chương 13 làm việc này bằng Zod), để chỗ duy nhất thật sự kiểm được các byte cũng là chỗ duy nhất gán kiểu.</p></div>
<a class="link-card codelab" href="/code-lab/typescript${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">⌨️</span>
