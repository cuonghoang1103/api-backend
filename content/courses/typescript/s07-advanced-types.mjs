...
<h3>How a mapped type is evaluated</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">Get the key union</span><span class="lz-d"><code>[K in keyof T]</code> takes every key of <code>T</code> — <code>'id' | 'title' | 'body'</code> — and iterates it, exactly like a <code>for…of</code> over a list of names.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">Build one property per key</span><span class="lz-d">For each <code>K</code> the body produces a member. The name is <code>K</code>, the type is whatever the right-hand side evaluates to — usually <code>T[K]</code>, possibly wrapped.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">Apply the modifiers</span><span class="lz-d"><code>readonly</code> and <code>?</code> written before the key apply to every generated member. This is how <code>Readonly&lt;T&gt;</code> and <code>Partial&lt;T&gt;</code> are three lines each.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">The result is a fresh object type</span><span class="lz-d">Not a reference to <code>T</code>. Change <code>T</code> later and the mapped type recomputes — which is the entire point: one declaration, forever in sync.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — <code>Readonly&lt;T&gt;</code> is one level deep.</strong> <code>type Frozen = Readonly&lt;{ id: string; meta: { tag: string } }&gt;</code> forbids <code>f.meta = …</code> but allows <code>f.meta.tag = 'x'</code>, because the mapped type only rewrote the top-level members — <code>meta</code>'s own type was copied through untouched. The same shallowness applies to <code>Partial</code>, <code>Required</code> and every built-in utility. If you need it deep, write a recursive mapped type (<code>T[K] extends object ? DeepReadonly&lt;T[K]&gt; : T[K]</code>) and know what you are buying: recursion over large types is one of the real causes of slow <code>tsc</code> runs, which chapter 15 measures.</p></div>
...
<h3>Một mapped type được tính thế nào</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">Lấy union các khoá</span><span class="lz-d"><code>[K in keyof T]</code> lấy mọi khoá của <code>T</code> — <code>'id' | 'title' | 'body'</code> — rồi duyệt qua, y như một <code>for…of</code> trên danh sách tên.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">Dựng một thuộc tính cho mỗi khoá</span><span class="lz-d">Với mỗi <code>K</code>, phần thân sinh ra một thành viên. Tên là <code>K</code>, kiểu là bất cứ thứ gì vế phải tính ra — thường là <code>T[K]</code>, có thể được bọc thêm.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">Áp các modifier</span><span class="lz-d"><code>readonly</code> và <code>?</code> viết trước khoá sẽ áp cho mọi thành viên sinh ra. Đó là lý do <code>Readonly&lt;T&gt;</code> và <code>Partial&lt;T&gt;</code> mỗi cái chỉ ba dòng.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">Kết quả là một kiểu object mới</span><span class="lz-d">Không phải một tham chiếu tới <code>T</code>. Sau này đổi <code>T</code> thì mapped type tính lại — và đó chính là toàn bộ mục đích: khai một lần, đồng bộ mãi mãi.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — <code>Readonly&lt;T&gt;</code> chỉ sâu một tầng.</strong> <code>type Frozen = Readonly&lt;{ id: string; meta: { tag: string } }&gt;</code> cấm <code>f.meta = …</code> nhưng vẫn cho <code>f.meta.tag = 'x'</code>, vì mapped type chỉ viết lại các thành viên ở tầng trên cùng — kiểu của chính <code>meta</code> được chép nguyên sang. Cùng cái nông ấy áp cho <code>Partial</code>, <code>Required</code> và mọi utility có sẵn. Nếu cần sâu, hãy viết mapped type đệ quy (<code>T[K] extends object ? DeepReadonly&lt;T[K]&gt; : T[K]</code>) và biết rõ bạn đang mua gì: đệ quy trên kiểu lớn là một trong những nguyên nhân thật sự làm <code>tsc</code> chạy chậm, thứ mà chương 15 đo.</p></div>
<a class="link-card codelab" href="/code-lab/typescript${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">⌨️</span>
...
<h3>The four modifier operations</h3>
<div class="lz-map">
<div class="lz-node"><span class="lz-k">readonly</span><span class="lz-t">Add immutability</span><span class="lz-d">Every generated member becomes read-only. <code>Readonly&lt;T&gt;</code> is exactly this and nothing else.</span></div>
<div class="lz-node"><span class="lz-k">-readonly</span><span class="lz-t">Strip immutability</span><span class="lz-d">Turns a frozen shape back into a writable one. This is how <code>Mutable&lt;T&gt;</code> is written — there is no built-in for it.</span></div>
<div class="lz-node"><span class="lz-k">?</span><span class="lz-t">Make everything optional</span><span class="lz-d"><code>Partial&lt;T&gt;</code>. Useful for patch payloads and options bags, dangerous as a return type (see the trap).</span></div>
<div class="lz-node"><span class="lz-k">-?</span><span class="lz-t">Make everything required</span><span class="lz-d"><code>Required&lt;T&gt;</code>. It also removes <code>undefined</code> from the member's type, which is what makes it a real guarantee rather than a cosmetic change.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — <code>Partial&lt;T&gt;</code> as a return type pushes the burden onto every caller, forever.</strong> Declaring <code>function loadConfig(): Partial&lt;Config&gt;</code> is honest about a half-filled object, but it means <em>every</em> consumer must check <em>every</em> field before use — and the day one of them writes <code>cfg.port!</code> to shut the compiler up, the guarantee is gone and the crash is back. The alternative is to fill in the gaps once, at the boundary: parse the partial input, apply defaults, and return the complete <code>Config</code>. Then the type says what the rest of the program can actually rely on. Reserve <code>Partial</code> for <em>inputs</em> (a patch body, an options argument), where "some fields may be absent" is the truth you want to express.</p></div>
<a class="link-card codelab" href="/code-lab/typescript${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">⌨️</span>
...
<h3>Bốn phép thao tác modifier</h3>
<div class="lz-map">
<div class="lz-node"><span class="lz-k">readonly</span><span class="lz-t">Thêm tính bất biến</span><span class="lz-d">Mọi thành viên sinh ra đều thành chỉ-đọc. <code>Readonly&lt;T&gt;</code> đúng là cái này và không gì khác.</span></div>
<div class="lz-node"><span class="lz-k">-readonly</span><span class="lz-t">Bóc tính bất biến</span><span class="lz-d">Biến một dáng đã đóng băng trở lại ghi được. Đó là cách viết <code>Mutable&lt;T&gt;</code> — không có utility sẵn cho nó.</span></div>
<div class="lz-node"><span class="lz-k">?</span><span class="lz-t">Cho mọi thứ thành tuỳ chọn</span><span class="lz-d"><code>Partial&lt;T&gt;</code>. Có ích cho thân patch và túi options, nguy hiểm khi làm kiểu trả về (xem bẫy).</span></div>
<div class="lz-node"><span class="lz-k">-?</span><span class="lz-t">Cho mọi thứ thành bắt buộc</span><span class="lz-d"><code>Required&lt;T&gt;</code>. Nó còn gỡ <code>undefined</code> khỏi kiểu của thành viên, và chính điều đó làm nó thành một bảo đảm thật chứ không phải một thay đổi hình thức.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — <code>Partial&lt;T&gt;</code> làm kiểu trả về là đẩy gánh nặng sang mọi chỗ gọi, mãi mãi.</strong> Khai <code>function loadConfig(): Partial&lt;Config&gt;</code> thì thành thật về một object điền dở, nhưng nó có nghĩa <em>mọi</em> chỗ tiêu thụ phải kiểm <em>mọi</em> field trước khi dùng — và cái ngày một trong số đó viết <code>cfg.port!</code> cho trình biên dịch im đi, bảo đảm biến mất và cú sập quay lại. Cách khác là bù chỗ trống một lần, ngay ở biên: phân tích đầu vào dở dang, áp giá trị mặc định, rồi trả về <code>Config</code> đầy đủ. Khi đó kiểu nói đúng thứ mà phần còn lại của chương trình thật sự dựa được. Hãy để dành <code>Partial</code> cho <em>đầu vào</em> (thân patch, đối số options), nơi "vài field có thể vắng" đúng là sự thật bạn muốn diễn đạt.</p></div>
<a class="link-card codelab" href="/code-lab/typescript${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">⌨️</span>
...
<h3>Reading a conditional type</h3>
<div class="lz-stack">
<div class="lz-layer"><span class="lz-k">A extends B ? X : Y</span><span class="lz-t">"Is A assignable to B?"</span><span class="lz-d">Not inheritance — assignability. <code>'a' extends string</code> is true; <code>string extends 'a'</code> is false. The direction matters and is the usual source of confusion.</span></div>
<div class="lz-layer"><span class="lz-k">infer R</span><span class="lz-t">A hole the compiler fills</span><span class="lz-d">Only legal inside the <code>extends</code> clause. <code>T extends (…a: any[]) =&gt; infer R ? R : never</code> says "match this shape, and name the part in the hole R".</span></div>
<div class="lz-layer"><span class="lz-k">Distribution over unions</span><span class="lz-t">A naked T splits</span><span class="lz-d">If <code>T</code> is <code>A | B</code>, the conditional runs once per member and the results union back together. This is what makes <code>Exclude</code> work — and what surprises everyone the first time.</span></div>
<div class="lz-layer"><span class="lz-k">[T] extends [U]</span><span class="lz-t">Turn distribution off</span><span class="lz-d">Wrapping both sides in a tuple stops the split, so the union is tested as a single whole. Reach for it when you want to ask about the union itself.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — distribution silently rewrites your answer when T is a union.</strong> <code>type NonNull&lt;T&gt; = T extends null ? never : T</code> applied to <code>string | null</code> does not test the whole union: it runs on <code>string</code> (→ <code>string</code>) and on <code>null</code> (→ <code>never</code>) and unions the results to <code>string</code> — which happens to be what you wanted. Now write <code>type IsUnion&lt;T&gt; = T extends string ? true : false</code> and apply it to <code>string | number</code>: you get <code>boolean</code>, not <code>false</code>, because the two branches produced <code>true</code> and <code>false</code> and merged. Nothing errors; the type is just wrong, and it stays wrong until someone traces it. If your conditional should see the union as one thing, write <code>[T] extends [string]</code>.</p></div>
<a class="link-card codelab" href="/code-lab/typescript${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">⌨️</span>
...
<h3>Đọc một conditional type</h3>
<div class="lz-stack">
<div class="lz-layer"><span class="lz-k">A extends B ? X : Y</span><span class="lz-t">"A có gán được vào B không?"</span><span class="lz-d">Không phải kế thừa — mà là tính gán được. <code>'a' extends string</code> là đúng; <code>string extends 'a'</code> là sai. Chiều có ý nghĩa, và đó là nguồn nhầm lẫn thường gặp.</span></div>
<div class="lz-layer"><span class="lz-k">infer R</span><span class="lz-t">Một chỗ trống để trình biên dịch điền</span><span class="lz-d">Chỉ hợp lệ bên trong mệnh đề <code>extends</code>. <code>T extends (…a: any[]) =&gt; infer R ? R : never</code> nói "khớp cái dáng này, và đặt tên phần trong lỗ là R".</span></div>
<div class="lz-layer"><span class="lz-k">Phân phối trên union</span><span class="lz-t">Một T trần sẽ tách ra</span><span class="lz-d">Nếu <code>T</code> là <code>A | B</code>, conditional chạy một lần cho mỗi thành viên rồi các kết quả hợp lại. Đó là thứ làm <code>Exclude</code> chạy được — và cũng là thứ làm ai cũng ngạc nhiên lần đầu.</span></div>
<div class="lz-layer"><span class="lz-k">[T] extends [U]</span><span class="lz-t">Tắt phân phối đi</span><span class="lz-d">Bọc cả hai vế trong tuple sẽ chặn phép tách, nên union được kiểm như một khối duy nhất. Với tay tới nó khi bạn muốn hỏi về chính cái union.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — phân phối âm thầm viết lại đáp án của bạn khi T là union.</strong> <code>type NonNull&lt;T&gt; = T extends null ? never : T</code> áp lên <code>string | null</code> không hề kiểm cả union: nó chạy trên <code>string</code> (→ <code>string</code>) và trên <code>null</code> (→ <code>never</code>) rồi hợp kết quả thành <code>string</code> — tình cờ đúng thứ bạn muốn. Giờ viết <code>type IsUnion&lt;T&gt; = T extends string ? true : false</code> rồi áp lên <code>string | number</code>: bạn nhận <code>boolean</code>, không phải <code>false</code>, vì hai nhánh cho ra <code>true</code> và <code>false</code> rồi gộp lại. Chẳng có lỗi nào; kiểu chỉ đơn giản là sai, và nó sai cho tới khi có người lần ra. Nếu conditional của bạn cần nhìn union như một thứ duy nhất, hãy viết <code>[T] extends [string]</code>.</p></div>
<a class="link-card codelab" href="/code-lab/typescript${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">⌨️</span>
...
<h3>Template literal types, and where they end</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">Concatenate in the type system</span><span class="lz-d">&#96;on\${Capitalize&lt;K&gt;}&#96; builds a new literal type from an existing one. Same backtick syntax as runtime strings, evaluated at compile time.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">A union in, a union out</span><span class="lz-d">Interpolating <code>'a' | 'b'</code> produces every combination. Two unions of ten members each produce a hundred literals — cheap here, expensive at scale.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">Key remapping with as</span><span class="lz-d"><code>[K in keyof T as &#96;get\${Capitalize&lt;K &amp; string&gt;}&#96;]</code> renames each generated member — the trick behind typed event maps and generated getters.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">All of it erases</span><span class="lz-d">No getter is created at runtime. The type describes an API something else must actually implement — usually a Proxy or a code generator.</span></div>
</div>
<div class="pitfall"><p><strong>Trap — <code>keyof T</code> is <code>string | number | symbol</code>, so interpolating it directly does not compile.</strong> Writing <code>[K in keyof T as &#96;get\${Capitalize&lt;K&gt;}&#96;]</code> fails with TS2344: <code>Capitalize</code> demands a string, and <code>K</code> might be a symbol — a numeric or symbol key is perfectly legal on an object type. The fix is the intersection <code>K &amp; string</code>, which keeps only the string keys and quietly drops the rest. That silent dropping is worth knowing: a type with a symbol key will simply have no generated getter for it, with no error to tell you. If losing those keys would be a bug, constrain the input (<code>T extends Record&lt;string, unknown&gt;</code>) so the omission cannot happen unnoticed.</p></div>
<a class="link-card codelab" href="/code-lab/typescript${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">⌨️</span>
...
<h3>Template literal type, và giới hạn của nó</h3>
<div class="lz-flow">
<div class="lz-step"><span class="lz-k">1</span><span class="lz-t">Ghép chuỗi trong hệ thống kiểu</span><span class="lz-d">&#96;on\${Capitalize&lt;K&gt;}&#96; dựng một kiểu literal mới từ một cái có sẵn. Cùng cú pháp dấu huyền như chuỗi lúc chạy, nhưng tính ở thời điểm biên dịch.</span></div>
<div class="lz-step"><span class="lz-k">2</span><span class="lz-t">Union vào, union ra</span><span class="lz-d">Nội suy <code>'a' | 'b'</code> sinh ra mọi tổ hợp. Hai union mười thành viên sinh ra một trăm literal — rẻ ở đây, đắt khi quy mô lớn.</span></div>
<div class="lz-step"><span class="lz-k">3</span><span class="lz-t">Đổi tên khoá bằng as</span><span class="lz-d"><code>[K in keyof T as &#96;get\${Capitalize&lt;K &amp; string&gt;}&#96;]</code> đổi tên từng thành viên sinh ra — mẹo đứng sau các map sự kiện có kiểu và getter sinh tự động.</span></div>
<div class="lz-step"><span class="lz-k">4</span><span class="lz-t">Tất cả đều bị xoá</span><span class="lz-d">Không getter nào được tạo lúc chạy. Kiểu mô tả một API mà thứ khác phải thật sự hiện thực — thường là một Proxy hoặc một bộ sinh mã.</span></div>
</div>
<div class="pitfall"><p><strong>Bẫy — <code>keyof T</code> là <code>string | number | symbol</code>, nên nội suy thẳng nó thì không biên dịch được.</strong> Viết <code>[K in keyof T as &#96;get\${Capitalize&lt;K&gt;}&#96;]</code> sẽ hỏng với TS2344: <code>Capitalize</code> đòi một chuỗi, còn <code>K</code> có thể là symbol — khoá kiểu số hay symbol hoàn toàn hợp lệ trên một kiểu object. Cách chữa là phép giao <code>K &amp; string</code>, giữ lại chỉ các khoá chuỗi và lặng lẽ vứt phần còn lại. Cái vứt lặng lẽ ấy đáng biết: một kiểu có khoá symbol sẽ đơn giản là không có getter sinh ra cho nó, mà chẳng lỗi nào báo cho bạn. Nếu mất những khoá đó là một lỗi, hãy ràng buộc đầu vào (<code>T extends Record&lt;string, unknown&gt;</code>) để chuyện thiếu sót không thể xảy ra mà không ai hay.</p></div>
<a class="link-card codelab" href="/code-lab/typescript${REF}" target="_blank" rel="noopener">
  <span class="lc-ico">⌨️</span>
