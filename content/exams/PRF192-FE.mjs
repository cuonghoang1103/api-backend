...
export default {
  ...
  "exams": [
    ...
    },
    {
      ...
      "source": "REAL",
      "sortOrder": 4,
      "title": "Đề 5 — SU25 Block 5 Exam|||Đề 5 — Block 5 Hè 2025",
      "description": "PRF192 real FE multiple-choice paper (Summer 2025, Block 5), transcribed from the exam images; answers reasoned and code-verified here. (50 questions)|||Đề trắc nghiệm FE thật môn PRF192 (kỳ Hè 2025, Block 5), chép từ ảnh đề; đáp án được suy luận và chạy thử mã để kiểm chứng. (50 câu)",
      "durationMinutes": 60,
      "totalPoints": 10,
      "passMark": 4,
      "shuffleQuestions": true,
      ...
      "questions": [
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Which of the following is a valid variable name?|||Tên biến nào sau đây HỢP LỆ?",
          "options": [
            {
              "text": "break|||break"
            },
          ...
          "explanation": "<div class=\"ml-en\">A C identifier must start with a letter or underscore, never a digit — so <code>2_</code> is illegal. <code>break</code> and <code>for</code> are reserved KEYWORDS, which cannot be used as variable names. <code>_2</code> starts with an underscore followed by a digit, which is a perfectly legal identifier.</div><div class=\"ml-vi\">Định danh (identifier) trong C phải bắt đầu bằng chữ cái hoặc dấu gạch dưới, KHÔNG BAO GIỜ bắt đầu bằng chữ số — nên <code>2_</code> không hợp lệ. <code>break</code> và <code>for</code> là các từ khoá DÀNH RIÊNG (reserved keyword), không thể dùng làm tên biến. <code>_2</code> bắt đầu bằng dấu gạch dưới rồi tới chữ số, đây là định danh hoàn toàn hợp lệ.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Which of the following statements is correct regarding the first character of an identifier?|||Phát biểu nào sau đây ĐÚNG về ký tự đầu tiên của một định danh (identifier)?",
          "options": [
            {
              "text": "The first character of an identifier can be a number|||Ký tự đầu của định danh có thể là chữ số"
            },
            {
              "text": "The first character of an identifier must be a special symbol|||Ký tự đầu của định danh phải là ký hiệu đặc biệt"
            },
            {
              "text": "The first character of an identifier must be a letter or an underscore|||Ký tự đầu của định danh phải là chữ cái hoặc dấu gạch dưới"
            },
            {
              "text": "The first character of an identifier can be a space|||Ký tự đầu của định danh có thể là khoảng trắng"
            }
          ],
          "correctIndexes": [
            2
          ],
          "explanation": "<div class=\"ml-en\">C's identifier rule: the FIRST character must be a letter (a-z, A-Z) or an underscore <code>_</code> — never a digit, a special symbol like <code>@</code>/<code>#</code>, or whitespace (spaces are not even allowed anywhere inside an identifier).</div><div class=\"ml-vi\">Quy tắc định danh của C: ký tự ĐẦU TIÊN phải là chữ cái (a-z, A-Z) hoặc dấu gạch dưới <code>_</code> — không bao giờ là chữ số, ký hiệu đặc biệt như <code>@</code>/<code>#</code>, hay khoảng trắng (khoảng trắng thậm chí không được phép ở bất kỳ đâu bên trong định danh).</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          ...
          "options": [
            ...
            {
              "text": "Programs that perform relatively simple tasks and are written in assembly language contain a large number of statements.|||Chương trình thực hiện việc tương đối đơn giản nhưng viết bằng hợp ngữ (assembly) chứa một số lượng LỚN câu lệnh."
            }
          ],
          "correctIndexes": [
            0
          ],
          "explanation": "<div class=\"ml-en\">Higher-level languages exist to make programs SHORTER and easier to write (one line of C can replace many lines of assembly), not longer — so statement A reverses the real reason and is the incorrect one. B, C and D are all true: a simple task written in low-level assembly does indeed need many more statements than the same task in a high-level language.</div><div class=\"ml-vi\">Ngôn ngữ bậc cao ra đời để làm chương trình NGẮN gọn và dễ viết hơn (một dòng C có thể thay nhiều dòng hợp ngữ), không phải dài hơn — nên phát biểu A đã nói ngược lý do thật và là câu SAI. B, C, D đều đúng: một việc đơn giản viết bằng hợp ngữ mức thấp thực sự cần NHIỀU câu lệnh hơn hẳn so với cùng việc đó viết bằng ngôn ngữ bậc cao.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is the numeric code of the NULL character?|||Mã số của ký tự NULL là gì?",
          "options": [
            {
              "text": "1|||1"
            },
          ...
          "explanation": "<div class=\"ml-en\">The NULL character (<code>&#x27;\\0&#x27;</code>) always has ASCII code 0 — it is the special sentinel value C uses to mark the end of a string. <code>32</code> is the space character's code, and <code>-1</code> is the value of <code>EOF</code>, not NULL.</div><div class=\"ml-vi\">Ký tự NULL (<code>&#x27;\\0&#x27;</code>) luôn có mã ASCII bằng 0 — đây là giá trị đặc biệt C dùng để đánh dấu kết thúc một chuỗi. <code>32</code> là mã của ký tự khoảng trắng, còn <code>-1</code> là giá trị của <code>EOF</code>, không phải NULL.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What function in C is used to read a single character from a file?|||Hàm nào trong C dùng để đọc MỘT ký tự đơn từ một tệp?",
          "options": [
            {
              "text": "fgetc()|||fgetc()"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>fgetc(file)</code> reads exactly one character from the given file stream. <code>fgets()</code> reads a whole LINE, <code>fputc()</code> WRITES (not reads) a character, and <code>fread()</code> reads a block of raw binary data, not specifically one character.</div><div class=\"ml-vi\"><code>fgetc(file)</code> đọc đúng một ký tự từ luồng tệp cho trước. <code>fgets()</code> đọc cả một DÒNG, <code>fputc()</code> là GHI (không phải đọc) một ký tự, còn <code>fread()</code> đọc một khối dữ liệu nhị phân thô, không riêng một ký tự.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
        ...
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Choose the correct definition for \"Simple data types\" in C programming.|||Chọn định nghĩa ĐÚNG cho \"Kiểu dữ liệu đơn giản\" (Simple data types) trong lập trình C.",
          "options": [
            {
              "text": "Changing from one form of representation to another|||Việc chuyển đổi từ dạng biểu diễn này sang dạng khác"
            },
            {
              "text": "A category of data that represents individual, non-complex elements such as integers, floating-point numbers, and characters|||Một loại dữ liệu biểu diễn các phần tử đơn lẻ, không phức tạp như số nguyên, số thực dấu phẩy động, và ký tự"
            },
            {
              "text": "Performing operations and expressions on data|||Việc thực hiện các phép toán và biểu thức trên dữ liệu"
            },
            {
              "text": "Storing a collection of elements accessible by an index or a key|||Việc lưu trữ một tập hợp phần tử truy cập được qua chỉ số hoặc khoá"
            }
          ],
          "correctIndexes": [
            1
          ],
          "explanation": "<div class=\"ml-en\">Simple (a.k.a. primitive/basic) data types — <code>int</code>, <code>float</code>, <code>double</code>, <code>char</code> — each hold ONE single, non-decomposable value. Option A describes type CASTING, C describes an EXPRESSION/operator, and D describes a data STRUCTURE like an array — none of those is the definition of a simple data type itself.</div><div class=\"ml-vi\">Kiểu dữ liệu đơn giản (còn gọi nguyên thuỷ/cơ bản) — <code>int</code>, <code>float</code>, <code>double</code>, <code>char</code> — mỗi kiểu chỉ chứa MỘT giá trị đơn lẻ, không thể phân rã thêm. Phương án A mô tả việc ÉP KIỂU, C mô tả BIỂU THỨC/toán tử, còn D mô tả một CẤU TRÚC dữ liệu như mảng — không cái nào là định nghĩa của kiểu dữ liệu đơn giản.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is the result of the expression !1?|||Kết quả của biểu thức !1 là gì?",
          "options": [
            {
              "text": "0|||0"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>!</code> is the LOGICAL NOT operator. Any non-zero value (like <code>1</code>) is treated as \"true\", and <code>!</code> flips true to <code>0</code> (false). Verified by compiling and running: prints 0.</div><div class=\"ml-vi\"><code>!</code> là toán tử PHỦ ĐỊNH LOGIC (NOT). Bất kỳ giá trị khác 0 nào (như <code>1</code>) được coi là \"đúng\" (true), và <code>!</code> đảo true thành <code>0</code> (false). Đã biên dịch và chạy thử kiểm chứng: in ra 0.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Which of the following operators has the lowest priority?|||Toán tử nào sau đây có độ ưu tiên THẤP NHẤT?",
          "options": [
            {
              "text": "+=|||+="
            },
          ...
          "explanation": "<div class=\"ml-en\">Assignment operators like <code>+=</code> sit near the very BOTTOM of C's precedence table (only the comma operator is lower) — they are evaluated LAST, after every arithmetic operation on their right-hand side. <code>++</code> (increment) is near the TOP of the table, and binary <code>+</code>/<code>-</code> (addition/subtraction) sit in the middle, well above assignment.</div><div class=\"ml-vi\">Các toán tử gán như <code>+=</code> nằm gần ĐÁY BẢNG độ ưu tiên của C (chỉ có toán tử dấu phẩy thấp hơn) — chúng được tính SAU CÙNG, sau khi mọi phép toán số học ở vế phải đã xong. <code>++</code> (tăng) nằm gần ĐỈNH bảng, còn <code>+</code>/<code>-</code> (cộng/trừ hai ngôi) nằm ở giữa, cao hơn hẳn phép gán.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Which of the following symbols is used to denote the end statement?|||Ký hiệu nào sau đây dùng để đánh dấu KẾT THÚC một câu lệnh?",
          "options": [
            {
              "text": ";|||;"
            },
          ...
          "explanation": "<div class=\"ml-en\">Every C statement ends with a semicolon <code>;</code> — this is how the compiler knows one statement has finished and the next begins. <code>#</code> starts a PREPROCESSOR directive, <code>!</code> is logical NOT, and <code>-</code> is subtraction/negation — none marks the end of a statement.</div><div class=\"ml-vi\">Mọi câu lệnh trong C kết thúc bằng dấu chấm phẩy <code>;</code> — đây là cách trình biên dịch biết một câu lệnh đã xong và câu tiếp theo bắt đầu. <code>#</code> mở đầu chỉ thị TIỀN XỬ LÝ, <code>!</code> là phủ định logic, còn <code>-</code> là trừ/lấy đối — không cái nào đánh dấu kết thúc câu lệnh.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is the results will be displayed on the screen after executing the following code:<pre><code>char a[] = &quot;Ab&quot;;\nchar *s = a;\nint t=1;\nfor (; *s; ++s)\n    t *= *s;\nprintf(&quot;%d&quot;, t);</code></pre>|||Kết quả hiển thị trên màn hình sau khi chạy đoạn mã sau là gì:<pre><code>char a[] = &quot;Ab&quot;;\nchar *s = a;\nint t=1;\nfor (; *s; ++s)\n    t *= *s;\nprintf(&quot;%d&quot;, t);</code></pre>",
          "options": [
            {
              "text": "Garbage value|||Giá trị rác"
            },
          ...
          "explanation": "<div class=\"ml-en\">The loop walks the string \"Ab\" character by character until it hits the terminating <code>&#x27;\\0&#x27;</code> (which is 0/false, stopping the loop). It multiplies <code>t</code> by each character's ASCII code: <code>t = 1 * &#x27;A&#x27;(65) * &#x27;b&#x27;(98) = 65 * 98 = 6370</code>. Verified by compiling and running.</div><div class=\"ml-vi\">Vòng lặp duyệt chuỗi \"Ab\" từng ký tự cho tới khi gặp ký tự kết thúc <code>&#x27;\\0&#x27;</code> (bằng 0/false, dừng vòng lặp). Nó nhân <code>t</code> với mã ASCII của mỗi ký tự: <code>t = 1 * &#x27;A&#x27;(65) * &#x27;b&#x27;(98) = 65 * 98 = 6370</code>. Đã biên dịch và chạy thử kiểm chứng.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is the result of the \"printf(\\\"%d %d\\\",12<<2,8>>2);\" command?|||Kết quả của lệnh \"printf(\\\"%d %d\\\",12<<2,8>>2);\" là gì?",
          "options": [
            {
              "text": "6 4|||6 4"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>12 &lt;&lt; 2</code> (left shift by 2, i.e. multiply by 4) = <code>48</code>. <code>8 &gt;&gt; 2</code> (right shift by 2, i.e. integer-divide by 4) = <code>2</code>. Verified by compiling and running: prints <code>48 2</code>.</div><div class=\"ml-vi\"><code>12 &lt;&lt; 2</code> (dịch trái 2 bit, tức nhân 4) = <code>48</code>. <code>8 &gt;&gt; 2</code> (dịch phải 2 bit, tức chia nguyên cho 4) = <code>2</code>. Đã biên dịch và chạy thử kiểm chứng: in ra <code>48 2</code>.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "In a for loop, what part of the loop is executed before the loop starts?|||Trong vòng lặp for, phần nào của vòng lặp được thực thi TRƯỚC KHI vòng lặp bắt đầu?",
          "options": [
            {
              "text": "The condition|||Điều kiện (condition)"
            },
            {
              "text": "The initialization block|||Khối khởi tạo (initialization block)"
            },
            {
              "text": "The iteration statement|||Câu lệnh lặp (iteration statement)"
            },
            {
              "text": "The body of the loop|||Thân vòng lặp (body)"
            }
          ],
          "correctIndexes": [
            1
          ],
          "explanation": "<div class=\"ml-en\">A <code>for</code> header is <code>for (init; condition; iteration)</code>. The <code>init</code> block (e.g. <code>int i = 0</code>) runs EXACTLY ONCE, before anything else, to set up the loop variable. The condition is then checked before each iteration (including the first), and <code>iteration</code>/the body run AFTER, on every pass except this initial setup.</div><div class=\"ml-vi\">Đầu vòng lặp <code>for</code> là <code>for (khởi_tạo; điều_kiện; bước_lặp)</code>. Khối <code>khởi_tạo</code> (vd <code>int i = 0</code>) chạy ĐÚNG MỘT LẦN, trước mọi thứ khác, để thiết lập biến lặp. Điều kiện sau đó được kiểm tra trước MỖI lần lặp (kể cả lần đầu), còn <code>bước_lặp</code>/thân vòng chạy SAU, ở mỗi lượt trừ bước khởi tạo ban đầu này.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is the primary purpose of logic constructs in programming?|||Mục đích chính của các cấu trúc điều khiển (logic constructs) trong lập trình là gì?",
          "options": [
            {
              "text": "To perform calculations and execute statements in a sequential order|||Thực hiện tính toán và thực thi các câu lệnh theo một trình tự"
            },
            {
              "text": "To store data in variables|||Lưu trữ dữ liệu vào biến"
            },
            {
              "text": "To allocate memory dynamically|||Cấp phát bộ nhớ động"
            },
            {
              "text": "To define the syntax of a programming language|||Định nghĩa cú pháp của một ngôn ngữ lập trình"
            }
          ],
          "correctIndexes": [
            0
          ],
          "explanation": "<div class=\"ml-en\">Logic constructs — sequence, selection (if/else) and iteration (loops) — control the ORDER in which a program's statements/calculations run. Storing data in variables is the job of variable declarations, allocating memory dynamically is <code>malloc()</code>'s job, and defining a language's syntax is the job of its grammar/specification, not of the constructs written inside a program.</div><div class=\"ml-vi\">Các cấu trúc điều khiển (logic construct) — tuần tự, rẽ nhánh (if/else) và lặp (loop) — điều khiển TRÌNH TỰ các câu lệnh/phép tính của chương trình chạy theo. Lưu dữ liệu vào biến là việc của khai báo biến, cấp phát bộ nhớ động là việc của <code>malloc()</code>, còn định nghĩa cú pháp ngôn ngữ là việc của ngữ pháp/đặc tả ngôn ngữ, không phải của các cấu trúc viết bên trong một chương trình.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
        ...
        },
        {
          ...
          "explanation": "<div class=\"ml-en\">The question asks HOW to declare: a declaration is written as <code>&lt;type&gt; &lt;name&gt;;</code> — you state the data type and the name. Option A is a true rule about C but it does not describe how to declare; C is false (globals/statics live in static storage, not the stack); D describes input, not declaration, and there is no \"OUT statement\" in C; E inverts the definition of scope (scope is where a variable CAN be accessed).</div><div class=\"ml-vi\">Câu hỏi hỏi CÁCH khai báo: một khai báo viết dạng <code>&lt;kiểu&gt; &lt;tên&gt;;</code> — nêu kiểu dữ liệu và tên biến. Phương án A là quy tắc đúng của C nhưng không mô tả cách khai báo; C sai (biến toàn cục/static nằm ở vùng nhớ tĩnh, không phải stack); D mô tả việc nhập dữ liệu, và C không có \"lệnh OUT\"; E định nghĩa NGƯỢC về phạm vi (scope là nơi biến CÓ THỂ được truy cập).</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What term is commonly used to specify both the type and name of a variable or constant in a C programming language?|||Thuật ngữ nào thường dùng để chỉ việc nêu ra cả KIỂU và TÊN của một biến hoặc hằng số trong ngôn ngữ lập trình C?",
          "options": [
            {
          ...
          "explanation": "<div class=\"ml-en\">A DECLARATION is exactly this: <code>&lt;type&gt; &lt;name&gt;;</code>, telling the compiler both what kind of data the variable holds and what it's called. \"Name\" alone is just the identifier without a type; \"Identification\" is not a C term; \"Definition\" more precisely refers to where memory is actually reserved/initialized (relevant for functions and <code>extern</code> variables), a finer distinction than this question needs.</div><div class=\"ml-vi\">KHAI BÁO (declaration) chính là việc này: <code>&lt;kiểu&gt; &lt;tên&gt;;</code>, báo cho trình biên dịch biết cả biến chứa loại dữ liệu gì và tên gọi là gì. \"Tên\" (Name) chỉ là định danh, không có kiểu; \"Identification\" không phải thuật ngữ của C; \"Định nghĩa\" (Definition) chỉ chính xác hơn về nơi bộ nhớ thực sự được cấp/khởi tạo (liên quan tới hàm và biến <code>extern</code>), một phân biệt tinh hơn mức câu hỏi này cần.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
        ...
        },
        {
          ...
          "explanation": "<div class=\"ml-en\">The loop adds every integer from 1 up to 14 (<code>s &lt; 15</code> stops the loop once <code>s</code> reaches 15) into <code>total</code>: <code>1+2+...+14 = 14*15/2 = 105</code>. Verified by compiling and running.</div><div class=\"ml-vi\">Vòng lặp cộng dồn mọi số nguyên từ 1 đến 14 (<code>s &lt; 15</code> dừng vòng lặp khi <code>s</code> chạm 15) vào <code>total</code>: <code>1+2+...+14 = 14*15/2 = 105</code>. Đã biên dịch và chạy thử kiểm chứng.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Analyze the following C function and determine the output?<pre><code>int f(int a, int b, int c) {\n    int t = 2 * (a + b - c) / 5;\n    return t;\n}\n\nint main() {\n    int x = 5, y = 6, z = 7;\n    int t = 3 * f(x, y, z);\n    printf(&quot;%d&quot;, t);\n    return 0;\n}</code></pre>|||Phân tích hàm C sau và xác định kết quả in ra?<pre><code>int f(int a, int b, int c) {\n    int t = 2 * (a + b - c) / 5;\n    return t;\n}\n\nint main() {\n    int x = 5, y = 6, z = 7;\n    int t = 3 * f(x, y, z);\n    printf(&quot;%d&quot;, t);\n    return 0;\n}</code></pre>",
          "options": [
            {
              "text": "6|||6"
            },
          ...
          "explanation": "<div class=\"ml-en\">Inside <code>f(5, 6, 7)</code>: <code>a+b-c = 5+6-7 = 4</code>, so <code>2*4/5 = 8/5</code> which is INTEGER division, truncating to <code>1</code> (not 1.6). <code>f</code> returns 1. Back in <code>main</code>, <code>t = 3 * 1 = 3</code>. Verified by compiling and running.</div><div class=\"ml-vi\">Bên trong <code>f(5, 6, 7)</code>: <code>a+b-c = 5+6-7 = 4</code>, nên <code>2*4/5 = 8/5</code> là phép chia NGUYÊN, cắt xuống còn <code>1</code> (không phải 1.6). <code>f</code> trả về 1. Về <code>main</code>, <code>t = 3 * 1 = 3</code>. Đã biên dịch và chạy thử kiểm chứng.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What will the following program print?<pre><code>#include &lt;stdio.h&gt;\n\nint main() {\n    int i = 1, j = 0;\n    do {\n        printf(&quot;%d %d &quot;, i, j);\n        i+=1;\n        j-=1;\n    } while (i &lt; j);\n\n    return 0;\n}</code></pre>|||Chương trình sau sẽ in ra gì?<pre><code>#include &lt;stdio.h&gt;\n\nint main() {\n    int i = 1, j = 0;\n    do {\n        printf(&quot;%d %d &quot;, i, j);\n        i+=1;\n        j-=1;\n    } while (i &lt; j);\n\n    return 0;\n}</code></pre>",
          "options": [
            {
              "text": "4 -1|||4 -1"
            },
          ...
          "explanation": "<div class=\"ml-en\">A <code>do-while</code> loop ALWAYS runs its body at least once BEFORE checking the condition. First pass prints <code>i</code>=1, <code>j</code>=0, i.e. \"1 0 \", then updates <code>i</code>=2, <code>j</code>=-1. The condition <code>i &lt; j</code> (<code>2 &lt; -1</code>) is now false, so the loop stops after just one pass. Verified by compiling and running.</div><div class=\"ml-vi\">Vòng lặp <code>do-while</code> LUÔN chạy thân vòng ÍT NHẤT MỘT LẦN trước khi kiểm tra điều kiện. Lần đầu in <code>i</code>=1, <code>j</code>=0, tức \"1 0 \", rồi cập nhật <code>i</code>=2, <code>j</code>=-1. Điều kiện <code>i &lt; j</code> (<code>2 &lt; -1</code>) lúc này sai, nên vòng lặp dừng sau đúng một lần. Đã biên dịch và chạy thử kiểm chứng.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is the loop in C language used for?|||Vòng lặp trong ngôn ngữ C dùng để làm gì?",
          "options": [
            {
              "text": "To execute a block of codes repeatedly as long as a specified condition is true.|||Thực thi một khối mã lặp đi lặp lại chừng nào một điều kiện cho trước còn đúng."
            },
            {
              "text": "To perform mathematical calculations.|||Thực hiện các phép tính toán học."
            },
            {
              "text": "To define variables within a program.|||Định nghĩa biến trong chương trình."
            },
            {
              "text": "To execute a statement based on a condition|||Thực thi một câu lệnh dựa trên một điều kiện"
            }
          ],
          "correctIndexes": [
            0
          ],
          "explanation": "<div class=\"ml-en\">This is the defining trait of a loop (<code>for</code>/<code>while</code>/<code>do-while</code>): REPEAT a block while a condition holds. Performing math (B) is just one possible thing a loop's body might do, not the loop's purpose; defining variables (C) is unrelated; \"executing a statement based on a condition\" (D) describes <code>if</code>, which runs AT MOST once, not repeatedly.</div><div class=\"ml-vi\">Đây chính là đặc điểm định nghĩa của vòng lặp (<code>for</code>/<code>while</code>/<code>do-while</code>): LẶP LẠI một khối lệnh chừng nào điều kiện còn đúng. Tính toán (B) chỉ là MỘT việc thân vòng lặp có thể làm, không phải mục đích của vòng lặp; định nghĩa biến (C) không liên quan; \"thực thi câu lệnh dựa trên điều kiện\" (D) mô tả <code>if</code>, vốn chạy TỐI ĐA một lần, không lặp lại.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Which of the following is an incorrect iteration construct?|||Cấu trúc nào sau đây KHÔNG PHẢI một cấu trúc lặp (iteration construct)?",
          "options": [
            {
          ...
          "explanation": "<div class=\"ml-en\">The <code>?:</code> TERNARY (conditional) operator picks ONE of two VALUES based on a condition — it does not repeat anything, so it is not an iteration construct at all; it belongs with <code>if/else</code>. <code>for</code>, <code>do-while</code> and <code>while</code> are C's three real looping constructs.</div><div class=\"ml-vi\">Toán tử BA NGÔI (ternary/conditional) <code>?:</code> chỉ chọn MỘT trong hai GIÁ TRỊ dựa trên điều kiện — nó không lặp lại gì cả, nên hoàn toàn không phải cấu trúc lặp; nó cùng nhóm với <code>if/else</code>. <code>for</code>, <code>do-while</code> và <code>while</code> mới là ba cấu trúc lặp thật sự của C.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What will happen if the condition in a while loop is initially false?|||Điều gì xảy ra nếu điều kiện của vòng lặp while SAI ngay từ đầu?",
          "options": [
            {
              "text": "The loop is skipped entirely.|||Vòng lặp bị BỎ QUA hoàn toàn."
            },
            ...
            {
              "text": "The loop executes once and then terminates.|||Vòng lặp chạy MỘT LẦN rồi dừng."
            },
            {
          ...
          "explanation": "<div class=\"ml-en\"><code>while</code> checks its condition BEFORE every single pass, including the very first one — if it's false from the start, the body never runs at all, zero times. (This is the key difference from <code>do-while</code>, which always runs the body at least once because it checks the condition AFTER.)</div><div class=\"ml-vi\"><code>while</code> kiểm tra điều kiện TRƯỚC mỗi lượt, kể cả lượt ĐẦU TIÊN — nếu sai ngay từ đầu, thân vòng lặp không chạy lần nào cả, KHÔNG LẦN nào. (Đây là khác biệt then chốt so với <code>do-while</code>, vốn luôn chạy thân vòng ít nhất một lần vì nó kiểm tra điều kiện SAU.)</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What will be the output of the C program?<pre><code>int i = 1, j = 2;\nif(i-- == j)\n    printf(&quot;i = %d&quot;, --i);\nelse\n    printf(&quot;j = %d&quot;, ++j);</code></pre>|||Chương trình C sau sẽ in ra gì?<pre><code>int i = 1, j = 2;\nif(i-- == j)\n    printf(&quot;i = %d&quot;, --i);\nelse\n    printf(&quot;j = %d&quot;, ++j);</code></pre>",
          "options": [
            {
              "text": "i = 2|||i = 2"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>i--</code> is POSTFIX: the COMPARISON uses i's value BEFORE decrementing, i.e. <code>1 == 2</code>, which is false (i becomes 0 afterward, but too late to matter here). So the <code>else</code> branch runs: <code>++j</code> is PREFIX, incrementing j to 3 FIRST, then printing \"j = 3\". Verified by compiling and running.</div><div class=\"ml-vi\"><code>i--</code> là HẬU TỐ (postfix): phép SO SÁNH dùng giá trị của i TRƯỚC KHI giảm, tức <code>1 == 2</code>, sai (i giảm còn 0 sau đó, nhưng đã muộn để ảnh hưởng ở đây). Vậy nhánh <code>else</code> chạy: <code>++j</code> là TIỀN TỐ, tăng j lên 3 TRƯỚC, rồi in \"j = 3\". Đã biên dịch và chạy thử kiểm chứng.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
        ...
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Which of the following statements is used to stop the select or loop statement?|||Câu lệnh nào sau đây dùng để DỪNG một câu lệnh chọn (switch) hoặc lặp (loop)?",
          "options": [
            {
              "text": "break|||break"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>break</code> immediately EXITS/stops the innermost enclosing <code>switch</code>, <code>for</code>, <code>while</code> or <code>do-while</code>. <code>continue</code> skips to the next iteration instead of stopping; <code>goto</code> is an unrelated unconditional jump; <code>while</code> itself starts a loop, it does not stop one.</div><div class=\"ml-vi\"><code>break</code> THOÁT/DỪNG NGAY LẬP TỨC câu lệnh <code>switch</code>, <code>for</code>, <code>while</code> hay <code>do-while</code> gần nhất đang bao nó. <code>continue</code> chuyển sang lần lặp tiếp theo thay vì dừng hẳn; <code>goto</code> là lệnh nhảy vô điều kiện không liên quan; bản thân <code>while</code> KHỞI ĐỘNG một vòng lặp, không phải để dừng nó.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Suppose there is a function in C as follows:<pre><code>double g (double x, double y){\n    return (x - 2)*(x - 2) + y*y;\n}</code></pre>What is the value of z after executing the statement z=g(2,0)?|||Giả sử có hàm C sau:<pre><code>double g (double x, double y){\n    return (x - 2)*(x - 2) + y*y;\n}</code></pre>Giá trị của z sau khi thực thi câu lệnh z=g(2,0) là gì?",
          "options": [
            {
              "text": "0|||0"
            },
          ...
          "explanation": "<div class=\"ml-en\">With <code>x=2, y=0</code>: <code>(2-2)*(2-2) + 0*0 = 0*0 + 0 = 0</code>. Verified by compiling and running: z = 0.</div><div class=\"ml-vi\">Với <code>x=2, y=0</code>: <code>(2-2)*(2-2) + 0*0 = 0*0 + 0 = 0</code>. Đã biên dịch và chạy thử kiểm chứng: z = 0.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Consider the following program and answer the following questions.<pre><code>#include&lt;stdio.h&gt;\nvoid hours_and_minutes(int total, int&amp; phrs, int&amp; pmin)\n{\n   pmin = total % 60;\n   phrs = total / 60;\n}\n\nmain()\n{\n   int tmin, hr = 0, min = 0;\n   printf(&quot;Enter total minutes for videotape: &quot;);\n   scanf(&quot;%d&quot;, &amp;tmin);\n   hours_and_minutes(tmin, hr, min);\n   printf(&quot;That is %d hours and %d minutes\\n&quot;, hr, min);\n}</code></pre>What is the result displayed to the screen if the user inputs 85 ?|||Xét chương trình sau và trả lời câu hỏi.<pre><code>#include&lt;stdio.h&gt;\nvoid hours_and_minutes(int total, int&amp; phrs, int&amp; pmin)\n{\n   pmin = total % 60;\n   phrs = total / 60;\n}\n\nmain()\n{\n   int tmin, hr = 0, min = 0;\n   printf(&quot;Enter total minutes for videotape: &quot;);\n   scanf(&quot;%d&quot;, &amp;tmin);\n   hours_and_minutes(tmin, hr, min);\n   printf(&quot;That is %d hours and %d minutes\\n&quot;, hr, min);\n}</code></pre>Kết quả hiển thị trên màn hình nếu người dùng nhập 85 là gì?",
          "options": [
            {
              "text": "That is 8 hours and 5 minutes|||That is 8 hours and 5 minutes"
            },
            {
              "text": "That is 0 hours and 0 minutes|||That is 0 hours and 0 minutes"
            },
            {
              "text": "That is 1 hours and 25 minutes|||That is 1 hours and 25 minutes"
            },
            {
              "text": "That is 0 hours and 85 minutes|||That is 0 hours and 85 minutes"
            }
          ],
          "correctIndexes": [
            2
          ],
          "explanation": "<div class=\"ml-en\">With <code>total = 85</code>: <code>pmin = 85 % 60 = 25</code> (remainder), <code>phrs = 85 / 60 = 1</code> (integer division). So the output is \"That is 1 hours and 25 minutes\" — 85 minutes = 1 full hour plus 25 leftover minutes. Verified by compiling (as pass-by-pointer, the C-legal equivalent of this pass-by-reference pseudocode) and running with input 85.</div><div class=\"ml-vi\">Với <code>total = 85</code>: <code>pmin = 85 % 60 = 25</code> (số dư), <code>phrs = 85 / 60 = 1</code> (chia nguyên). Vậy kết quả là \"That is 1 hours and 25 minutes\" — 85 phút = 1 giờ tròn cộng 25 phút dư. Đã biên dịch (dùng truyền qua con trỏ, dạng hợp lệ trong C tương đương giả mã truyền tham chiếu này) và chạy thử kiểm chứng với input 85.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is incorrect about function prototype?|||Điều gì SAI về nguyên mẫu (prototype) hàm sau đây?",
          "options": [
            {
          ...
          "explanation": "<div class=\"ml-en\">In C, EVERY parameter must have its OWN type written out — <code>int printSum(int a, b);</code> leaves <code>b</code> with NO type at all, which is illegal (modern compilers reject it: \"type specifier missing\"). It is not shorthand for <code>int a, int b</code> the way it might be in math notation. A, B and D each correctly give every parameter an explicit type.</div><div class=\"ml-vi\">Trong C, MỌI tham số đều phải có KIỂU RIÊNG được viết ra — <code>int printSum(int a, b);</code> để <code>b</code> KHÔNG có kiểu nào cả, đây là lỗi (trình biên dịch hiện đại từ chối: \"thiếu chỉ định kiểu\"). Nó không phải cách viết tắt cho <code>int a, int b</code> như trong ký hiệu toán học. A, B, D đều cho mỗi tham số một kiểu tường minh đúng.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "The pow() function in the math.h library is used to calculate the value of a number raised to a specified power. What is the return value type of the pow() function?|||Hàm pow() trong thư viện math.h dùng để tính giá trị một số được nâng lên luỹ thừa xác định. Kiểu trả về của hàm pow() là gì?",
          "options": [
            {
              "text": "double|||double"
            },
          ...
          "explanation": "<div class=\"ml-en\">The standard C prototype is <code>double pow(double base, double exponent);</code> — both parameters and the return value are <code>double</code>, so fractional bases/exponents/results are handled correctly. If it returned <code>int</code> every fractional result would be destroyed by truncation.</div><div class=\"ml-vi\">Nguyên mẫu chuẩn của C là <code>double pow(double base, double exponent);</code> — cả hai tham số lẫn giá trị trả về đều là <code>double</code>, nên cơ số/số mũ/kết quả có phần thập phân đều được xử lý đúng. Nếu trả về <code>int</code> thì mọi kết quả có phần thập phân sẽ bị huỷ do làm tròn cắt bớt.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
        ...
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "The library stdio.h contains functions for printing out data, including ___________|||Thư viện stdio.h chứa các hàm dùng để IN dữ liệu ra, gồm ___________",
          "options": [
            {
              "text": "scanf, printf|||scanf, printf"
            },
            {
              "text": "putchar, printf|||putchar, printf"
            },
            {
              "text": "scanf, putchar|||scanf, putchar"
            },
            {
              "text": "printf, getchar|||printf, getchar"
            }
          ],
          "correctIndexes": [
            1
          ],
          "explanation": "<div class=\"ml-en\">The question asks specifically for OUTPUT (printing) functions: <code>putchar()</code> prints one character and <code>printf()</code> prints formatted data — both are pure output. <code>scanf()</code> and <code>getchar()</code> are INPUT functions (they read, not print), so any option pairing one of them in is wrong for this blank.</div><div class=\"ml-vi\">Câu hỏi hỏi cụ thể về các hàm XUẤT (in ra): <code>putchar()</code> in một ký tự và <code>printf()</code> in dữ liệu có định dạng — cả hai đều thuần XUẤT. <code>scanf()</code> và <code>getchar()</code> là các hàm NHẬP (đọc vào, không phải in ra), nên phương án nào ghép một trong hai hàm đó vào đều sai với chỗ trống này.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is an array in C programming?|||Mảng (array) trong lập trình C là gì?",
          "options": [
            {
              "text": "A collection of elements of the same data type stored in contiguous memory locations.|||Một tập hợp các phần tử CÙNG kiểu dữ liệu, được lưu ở các vị trí bộ nhớ LIÊN TIẾP nhau."
            },
            {
              "text": "A single variable that holds multiple values of different data types.|||Một biến đơn lưu nhiều giá trị thuộc các kiểu dữ liệu KHÁC nhau."
            },
            {
              "text": "A pointer to a single element in memory|||Một con trỏ trỏ tới một phần tử đơn lẻ trong bộ nhớ"
            },
            {
              "text": "A function that performs mathematical calculations.|||Một hàm thực hiện các phép tính toán học."
            }
          ...
          "explanation": "<div class=\"ml-en\">A C array is homogeneous (every element shares ONE data type, e.g. all <code>int</code>) and CONTIGUOUS in memory, which is exactly what lets <code>arr[i]</code> be computed as <code>base_address + i * sizeof(type)</code>. \"Different data types\" describes a <code>struct</code> instead; a pointer to one element is not the whole array; a function is unrelated.</div><div class=\"ml-vi\">Mảng trong C là ĐỒNG NHẤT (mọi phần tử cùng MỘT kiểu dữ liệu, vd tất cả <code>int</code>) và LIÊN TIẾP trong bộ nhớ, chính điều này cho phép <code>arr[i]</code> được tính bằng <code>địa_chỉ_gốc + i * sizeof(kiểu)</code>. \"Các kiểu dữ liệu khác nhau\" là mô tả của <code>struct</code>; con trỏ tới một phần tử không phải cả mảng; hàm thì không liên quan.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What will be the result of performing a binary search on an unsorted array?|||Kết quả khi thực hiện tìm kiếm nhị phân (binary search) trên một mảng CHƯA sắp xếp là gì?",
          "options": [
            {
              "text": "The search may return an incorrect index or fail to locate the target element.|||Tìm kiếm có thể trả về chỉ số SAI hoặc KHÔNG tìm ra được phần tử mục tiêu."
            },
            {
              "text": "The search will always find the target element if it is present somewhere in the array.|||Tìm kiếm luôn tìm ra phần tử mục tiêu nếu nó tồn tại đâu đó trong mảng."
            },
            {
              "text": "The search will perform as expected, but only for elements in the lower half of the array.|||Tìm kiếm vẫn hoạt động đúng như mong đợi, nhưng chỉ với các phần tử ở nửa dưới của mảng."
            },
            {
              "text": "The search will automatically sort the array before locating the target element.|||Tìm kiếm sẽ tự động sắp xếp mảng trước khi tìm phần tử mục tiêu."
            }
          ],
          "correctIndexes": [
            0
          ],
          "explanation": "<div class=\"ml-en\">Binary search relies ENTIRELY on the array being sorted — it decides which half to discard by comparing the target to the MIDDLE element and assuming everything before is smaller / after is larger. On unsorted data that assumption breaks, so the algorithm can skip over the correct half and miss the target or land on the wrong index. It does NOT sort anything itself and gives no such guarantee.</div><div class=\"ml-vi\">Tìm kiếm nhị phân dựa HOÀN TOÀN vào việc mảng đã được sắp xếp — nó quyết định bỏ nửa nào bằng cách so sánh mục tiêu với phần tử Ở GIỮA và giả định mọi thứ trước đó nhỏ hơn / sau đó lớn hơn. Trên dữ liệu chưa sắp xếp, giả định đó sai, nên thuật toán có thể bỏ qua đúng nửa chứa mục tiêu và không tìm ra hoặc trả về chỉ số sai. Bản thân nó KHÔNG tự sắp xếp gì cả và không đảm bảo gì như vậy.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "In C programming, comments are used to add explanatory text that is ignored by the compiler. Which is a correct type of comment?|||Trong lập trình C, chú thích (comment) dùng để thêm văn bản giải thích mà trình biên dịch bỏ qua. Loại chú thích nào ĐÚNG?",
          "options": [
            {
          ...
          "explanation": "<div class=\"ml-en\"><code>/* ... */</code> is C's block comment syntax — everything between <code>/*</code> and <code>*/</code> is ignored by the compiler (C also supports single-line <code>// ...</code> comments). <code>*/ ... */</code> has the delimiters reversed/duplicated, <code>** ... **</code> uses the wrong symbol, and <code>{ }</code> is reserved for code blocks, not comments.</div><div class=\"ml-vi\"><code>/* ... */</code> là cú pháp chú thích KHỐI của C — mọi thứ giữa <code>/*</code> và <code>*/</code> bị trình biên dịch bỏ qua (C cũng hỗ trợ chú thích một dòng <code>// ...</code>). <code>*/ ... */</code> đảo/lặp ký hiệu phân cách sai, <code>** ... **</code> dùng sai ký hiệu, còn <code>{ }</code> dành riêng cho khối lệnh, không phải chú thích.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is a pointer in C programming?|||Con trỏ (pointer) trong lập trình C là gì?",
          "options": [
            {
              "text": "A variable stores anything|||Một biến lưu bất kỳ thứ gì"
            },
            {
              "text": "A variable stores address of another variable or a variable stores address of the dynamically allocated memory|||Một biến lưu địa chỉ của biến khác hoặc lưu địa chỉ của vùng nhớ cấp phát động"
            },
            {
              "text": "An address in the memory|||Một địa chỉ trong bộ nhớ"
            },
            {
              "text": "A variable stores any number|||Một biến lưu bất kỳ con số nào"
            }
          ],
          "correctIndexes": [
            1
          ],
          "explanation": "<div class=\"ml-en\">A pointer is specifically a VARIABLE whose stored value is an ADDRESS (of another variable or of heap memory from <code>malloc</code>). \"An address in memory\" (C) describes only the VALUE it holds, not the pointer variable itself; A and D are too vague — they describe any ordinary variable, not what makes a pointer special.</div><div class=\"ml-vi\">Con trỏ cụ thể là một BIẾN mà giá trị nó lưu là một ĐỊA CHỈ (của biến khác hoặc của bộ nhớ heap từ <code>malloc</code>). \"Một địa chỉ trong bộ nhớ\" (C) chỉ mô tả GIÁ TRỊ nó chứa, không phải bản thân biến con trỏ; A và D quá chung chung — mô tả một biến thông thường bất kỳ, không nêu được điều làm con trỏ đặc biệt.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Which of the following is a valid code to use to declare and initialize a two-dimensional array?|||Cách viết nào sau đây là hợp lệ để khai báo và khởi tạo một mảng HAI CHIỀU?",
          "options": [
            {
          ...
          "explanation": "<div class=\"ml-en\">C array initializers use CURLY BRACES <code>{ }</code>, never square brackets <code>[ ]</code> for the VALUES — that rules out A and C (<code>[[1,2,3],...]</code>). For a 2D array, C requires the SECOND dimension's size (rows can be inferred from the initializer, but columns cannot) — <code>int a[][]</code> (B) is an incomplete type and fails to compile; <code>int a[][3]</code> (D) gives the column count and compiles correctly. Verified by compiling both B and D.</div><div class=\"ml-vi\">Bộ khởi tạo mảng trong C dùng DẤU NGOẶC NHỌN <code>{ }</code>, không bao giờ dùng ngoặc vuông <code>[ ]</code> cho các GIÁ TRỊ — loại ngay A và C (<code>[[1,2,3],...]</code>). Với mảng 2 chiều, C bắt buộc phải ghi kích thước CHIỀU THỨ HAI (số hàng có thể suy ra từ bộ khởi tạo, nhưng số cột thì không) — <code>int a[][]</code> (B) là kiểu KHÔNG ĐẦY ĐỦ và biên dịch lỗi; <code>int a[][3]</code> (D) cho biết số cột và biên dịch đúng. Đã biên dịch kiểm chứng cả B và D.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is the results will be displayed on the screen after executing the following code:<pre><code>int a[] = {1,2,3,4};\nint i, s = 0;\nfor (i=0; i&lt;4; ++i)\n    s += a[i];\nprintf(&quot;%f&quot;, (float)(s/4));</code></pre>|||Kết quả hiển thị trên màn hình sau khi chạy đoạn mã sau là gì:<pre><code>int a[] = {1,2,3,4};\nint i, s = 0;\nfor (i=0; i&lt;4; ++i)\n    s += a[i];\nprintf(&quot;%f&quot;, (float)(s/4));</code></pre>",
          "options": [
            {
              "text": "2.5|||2.5"
            },
          ...
          "explanation": "<div class=\"ml-en\">The loop sums all elements: <code>s = 1+2+3+4 = 10</code>, both <code>int</code>. <code>s/4</code> is computed FIRST as INTEGER division (<code>10/4 = 2</code>, the fraction is truncated) — the <code>(float)</code> cast only converts the already-truncated result 2 into 2.0, it does NOT undo the truncation. <code>%f</code> prints 6 decimals by default: <code>2.000000</code>. Verified by compiling and running.</div><div class=\"ml-vi\">Vòng lặp cộng dồn mọi phần tử: <code>s = 1+2+3+4 = 10</code>, đều là <code>int</code>. <code>s/4</code> được tính TRƯỚC dưới dạng chia NGUYÊN (<code>10/4 = 2</code>, phần lẻ bị cắt) — ép kiểu <code>(float)</code> chỉ chuyển kết quả ĐÃ BỊ CẮT là 2 thành 2.0, KHÔNG cứu lại được phần lẻ đã mất. <code>%f</code> mặc định in 6 chữ số thập phân: <code>2.000000</code>. Đã biên dịch và chạy thử kiểm chứng.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Binary search can be applied to which type of the below data structure in C?|||Tìm kiếm nhị phân (binary search) có thể áp dụng cho loại cấu trúc dữ liệu nào dưới đây trong C?",
          "options": [
            {
          ...
          "explanation": "<div class=\"ml-en\">Binary search is classically defined over a SORTED, linearly-ordered sequence — a one-dimensional array. A 2D array has no single natural ordering to binary-search over directly (you would need to flatten or search row/column separately), so \"every kind of array\" overstates what the plain algorithm handles.</div><div class=\"ml-vi\">Tìm kiếm nhị phân theo định nghĩa kinh điển hoạt động trên một dãy ĐÃ SẮP XẾP, có thứ tự tuyến tính — tức mảng MỘT CHIỀU. Mảng hai chiều không có một thứ tự tự nhiên duy nhất để tìm nhị phân trực tiếp (cần làm phẳng hoặc tìm riêng theo hàng/cột), nên \"mọi loại mảng\" là phóng đại so với những gì thuật toán thuần túy xử lý được.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is the output of the following code snippet?<pre><code>void main(){\n    int x = 4;\n    int *p = &amp;x;\n    int *k = p++;\n    int r = p - k;\n    printf(&quot;%d&quot;, r);\n}</code></pre>|||Kết quả của đoạn mã sau là gì?<pre><code>void main(){\n    int x = 4;\n    int *p = &amp;x;\n    int *k = p++;\n    int r = p - k;\n    printf(&quot;%d&quot;, r);\n}</code></pre>",
          "options": [
            {
              "text": "1|||1"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>p++</code> is POSTFIX: <code>k</code> gets p's value BEFORE the increment (so k still points at x), THEN p is advanced by one <code>int</code>'s width. Subtracting two pointers of the same type gives the number of ELEMENTS between them, not bytes — since p moved exactly one int ahead of k, <code>p - k = 1</code>. Verified by compiling and running.</div><div class=\"ml-vi\"><code>p++</code> là HẬU TỐ: <code>k</code> nhận giá trị của p TRƯỚC KHI tăng (nên k vẫn trỏ tới x), RỒI p mới nhích thêm đúng một độ rộng <code>int</code>. Trừ hai con trỏ cùng kiểu cho ra số PHẦN TỬ giữa chúng, không phải số byte — vì p đã đi trước k đúng một int, <code>p - k = 1</code>. Đã biên dịch và chạy thử kiểm chứng.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What will be the output of the following program?<pre><code>#include &lt;stdio.h&gt;\nint main() {\n    int *pa = a;\n    int a = 10;\n    (*pa)++;\n    a=a+10;\n    printf(&quot;%d&quot;, ++a);\n    getchar();\n    return 0;\n}</code></pre>|||Chương trình sau sẽ in ra gì?<pre><code>#include &lt;stdio.h&gt;\nint main() {\n    int *pa = a;\n    int a = 10;\n    (*pa)++;\n    a=a+10;\n    printf(&quot;%d&quot;, ++a);\n    getchar();\n    return 0;\n}</code></pre>",
          "options": [
            {
              "text": "Compiler error|||Lỗi biên dịch"
            },
          ...
          "explanation": "<div class=\"ml-en\">The line <code>int *pa = a;</code> tries to use <code>a</code> BEFORE it is declared (the declaration <code>int a = 10;</code> comes on the NEXT line) — C resolves identifiers top-to-bottom, so <code>a</code> does not exist yet at that point, giving \"use of undeclared identifier\". The program never reaches the arithmetic at all. Verified by compiling: fails with exactly this error.</div><div class=\"ml-vi\">Dòng <code>int *pa = a;</code> cố dùng <code>a</code> TRƯỚC KHI nó được khai báo (khai báo <code>int a = 10;</code> nằm ở dòng SAU) — C phân giải định danh từ trên xuống dưới, nên <code>a</code> chưa tồn tại ở thời điểm đó, báo lỗi \"dùng định danh chưa khai báo\". Chương trình không bao giờ chạy tới phần tính toán. Đã biên dịch kiểm chứng: lỗi đúng như vậy.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Assume we have the following code:<pre><code>int i=4;\nint *p = &amp;i;</code></pre>Suppose that an int occupies a memory block of 4 bytes, and the address of i is 1008. What is the result of the following expression: p + 4?|||Giả sử ta có đoạn mã sau:<pre><code>int i=4;\nint *p = &amp;i;</code></pre>Giả sử một int chiếm 4 byte bộ nhớ, và địa chỉ của i là 1008. Kết quả của biểu thức p + 4 là gì?",
          "options": [
            {
              "text": "1024|||1024"
            },
          ...
          "explanation": "<div class=\"ml-en\">Pointer arithmetic is scaled by the POINTED-TO type's size — <code>p + 4</code> on an <code>int*</code> means \"4 <code>int</code>s ahead\", i.e. <code>4 * 4 bytes = 16 bytes</code> added to the address: <code>1008 + 16 = 1024</code>. This is why C lets you write <code>p + n</code> instead of manually computing <code>address + n * sizeof(type)</code>.</div><div class=\"ml-vi\">Số học con trỏ được NHÂN THEO kích thước kiểu nó trỏ tới — <code>p + 4</code> trên một <code>int*</code> nghĩa là \"4 <code>int</code> về phía trước\", tức cộng <code>4 * 4 byte = 16 byte</code> vào địa chỉ: <code>1008 + 16 = 1024</code>. Đây là lý do C cho phép viết <code>p + n</code> thay vì tự tính <code>địa_chỉ + n * sizeof(kiểu)</code>.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Which of the following functions is used to read formatted input from the standard input in C?|||Hàm nào sau đây dùng để đọc dữ liệu CÓ ĐỊNH DẠNG từ luồng nhập chuẩn trong C?",
          "options": [
            {
              "text": "gets()|||gets()"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>scanf()</code> reads input according to a FORMAT STRING (<code>%d</code>, <code>%f</code>, <code>%s</code>, …), converting text into the requested types. <code>printf()</code> is the OUTPUT counterpart (writes, not reads); <code>gets()</code> and <code>fgets()</code> read a raw line of text with no format/type conversion at all.</div><div class=\"ml-vi\"><code>scanf()</code> đọc input theo một CHUỖI ĐỊNH DẠNG (<code>%d</code>, <code>%f</code>, <code>%s</code>, …), chuyển văn bản thành các kiểu yêu cầu. <code>printf()</code> là hàm XUẤT tương ứng (ghi, không phải đọc); <code>gets()</code> và <code>fgets()</code> đọc một dòng văn bản thô, không hề chuyển đổi định dạng/kiểu nào.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          ...
          "explanation": "<div class=\"ml-en\">The first loop adds 1 to EVERY character's ASCII code exactly once: <code>a→b, b→c, c→d, d→e, e→f</code>. The second loop then prints those already-shifted characters with <code>%c</code> (as characters, not numbers) — so the output is \"b c d e f\", each letter shifted forward by ONE, not two. Verified by compiling and running.</div><div class=\"ml-vi\">Vòng lặp đầu cộng 1 vào mã ASCII của MỌI ký tự đúng một lần: <code>a→b, b→c, c→d, d→e, e→f</code>. Vòng lặp thứ hai in các ký tự ĐÃ dịch đó bằng <code>%c</code> (dạng ký tự, không phải số) — nên kết quả là \"b c d e f\", mỗi chữ dịch tiến đúng MỘT bước, không phải hai. Đã biên dịch và chạy thử kiểm chứng.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
        ...
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What will be the output of the following code?<pre><code>printf(&quot;%c&quot;, tolower(&#x27;c&#x27;));</code></pre>|||Đoạn mã sau sẽ in ra gì?<pre><code>printf(&quot;%c&quot;, tolower(&#x27;c&#x27;));</code></pre>",
          "options": [
            {
              "text": "'c'|||'c'"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>tolower()</code> (from <code>ctype.h</code>) converts an UPPERCASE letter to lowercase and leaves any character that is ALREADY lowercase (or not a letter) completely unchanged. Since <code>&#x27;c&#x27;</code> is already lowercase, <code>tolower(&#x27;c&#x27;)</code> simply returns <code>&#x27;c&#x27;</code>. Verified by compiling and running.</div><div class=\"ml-vi\"><code>tolower()</code> (trong <code>ctype.h</code>) chuyển chữ HOA thành chữ thường và giữ NGUYÊN mọi ký tự đã là chữ thường (hoặc không phải chữ cái). Vì <code>&#x27;c&#x27;</code> đã là chữ thường sẵn, <code>tolower(&#x27;c&#x27;)</code> chỉ đơn giản trả về <code>&#x27;c&#x27;</code>. Đã biên dịch và chạy thử kiểm chứng.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is the maximum size required to store the string \"Hello\" in C?|||Kích thước tối đa cần để lưu chuỗi \"Hello\" trong C là bao nhiêu?",
          "options": [
            {
              "text": "5 bytes|||5 bytes"
            },
            {
              "text": "6 bytes|||6 bytes"
            },
            {
              "text": "4 bytes|||4 bytes"
            },
            {
              "text": "10 bytes|||10 bytes"
            }
          ],
          "correctIndexes": [
            1
          ],
          "explanation": "<div class=\"ml-en\">\"Hello\" has 5 visible characters, but every C string needs one EXTRA byte for the terminating null character <code>&#x27;\\0&#x27;</code> that marks where it ends — so the buffer must be at least <code>5 + 1 = 6</code> bytes. Verified: <code>sizeof(&quot;Hello&quot;)</code> (which includes the implicit null terminator) evaluates to 6.</div><div class=\"ml-vi\">\"Hello\" có 5 ký tự hiển thị, nhưng mọi chuỗi C đều cần THÊM một byte cho ký tự null kết thúc <code>&#x27;\\0&#x27;</code> đánh dấu điểm kết thúc — nên bộ đệm cần tối thiểu <code>5 + 1 = 6</code> byte. Đã kiểm chứng: <code>sizeof(&quot;Hello&quot;)</code> (đã tính cả ký tự null ngầm định) cho ra 6.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Which function is used to write a string to a file in C?|||Hàm nào dùng để GHI một chuỗi vào tệp trong C?",
          "options": [
            {
              "text": "fputs()|||fputs()"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>fputs(str, file)</code> writes the string <code>str</code> to the given file stream. <code>fscanf()</code> reads formatted data FROM a file, <code>fgets()</code> reads a line FROM a file (both are input, the reverse direction), and <code>fopen()</code> only OPENS a file — it does not write any content itself.</div><div class=\"ml-vi\"><code>fputs(str, file)</code> ghi chuỗi <code>str</code> vào luồng tệp cho trước. <code>fscanf()</code> đọc dữ liệu có định dạng TỪ tệp, <code>fgets()</code> đọc một dòng TỪ tệp (cả hai đều là nhập, chiều ngược lại), còn <code>fopen()</code> chỉ MỞ tệp — bản thân nó không ghi nội dung nào cả.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "The fopen function is used to open a file. Is the following true or false?<pre><code>&quot;r&quot; - tells fopen you will be reading data from the file. If no such file exists, fopen will fail.</code></pre>|||Hàm fopen dùng để mở một tệp. Phát biểu sau ĐÚNG hay SAI?<pre><code>&quot;r&quot; - báo cho fopen biết bạn sẽ đọc dữ liệu từ tệp. Nếu tệp đó không tồn tại, fopen sẽ thất bại.</code></pre>",
          "options": [
            {
              "text": "True|||Đúng"
            },
          ...
          "explanation": "<div class=\"ml-en\">This is an accurate description of mode <code>&quot;r&quot;</code>: it opens an EXISTING file for reading only, and <code>fopen</code> returns <code>NULL</code> (fails) if the file cannot be found — unlike mode <code>&quot;w&quot;</code>, which CREATES the file if it doesn't exist.</div><div class=\"ml-vi\">Đây là mô tả đúng của chế độ <code>&quot;r&quot;</code>: nó mở một tệp ĐÃ TỒN TẠI chỉ để đọc, và <code>fopen</code> trả về <code>NULL</code> (thất bại) nếu không tìm thấy tệp — khác với chế độ <code>&quot;w&quot;</code>, vốn TẠO MỚI tệp nếu chưa có.</div>"
        }
      ]
    },
    {
      ...
      "source": "REAL",
      "sortOrder": 5,
      "title": "Đề 6 — SU25 Final Exam|||Đề 6 — Thi cuối kỳ Hè 2025",
      "description": "PRF192 real FE multiple-choice paper (Summer 2025 final), transcribed from the exam images; answers reasoned and code-verified here. (50 questions)|||Đề trắc nghiệm FE thật môn PRF192 (thi cuối kỳ Hè 2025), chép từ ảnh đề; đáp án được suy luận và chạy thử mã để kiểm chứng. (50 câu)",
      "durationMinutes": 60,
      "totalPoints": 10,
      "passMark": 4,
      "shuffleQuestions": true,
      ...
      "questions": [
        ...
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Which is not a type of bus in a computer?|||Loại bus nào KHÔNG tồn tại trong máy tính?",
          "options": [
            {
          ...
          "explanation": "<div class=\"ml-en\">A computer has exactly three kinds of bus: address (carries memory addresses), data (carries the actual data) and control (carries command/status signals). \"Signal bus\" is not a recognised category — it is a made-up name combining the idea of all three.</div><div class=\"ml-vi\">Máy tính chỉ có đúng ba loại bus: bus địa chỉ (mang địa chỉ bộ nhớ), bus dữ liệu (mang dữ liệu thật) và bus điều khiển (mang tín hiệu lệnh/trạng thái). \"Bus tín hiệu\" không phải một loại được công nhận — đây là tên bịa ra gộp ý của cả ba loại.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "During the compilation process, what will be done with comments?|||Trong quá trình biên dịch, các chú thích (comment) sẽ được xử lý như thế nào?",
          "options": [
            {
              "text": "The comment mark will be removed. The content of the comments will be compiled line by line.|||Ký hiệu chú thích bị xoá. Nội dung chú thích sẽ được biên dịch theo từng dòng."
            },
            {
              "text": "The comment is treated as an array of characters.|||Chú thích được coi như một mảng ký tự."
            },
          ...
          "explanation": "<div class=\"ml-en\">Comments are stripped out during pre-processing, BEFORE the actual compilation of code begins — the compiler never sees their text at all, so they are neither compiled line by line (A), treated as data (B), nor turned into a no-op statement (D).</div><div class=\"ml-vi\">Chú thích bị loại bỏ ở bước tiền xử lý, TRƯỚC KHI quá trình biên dịch mã thật sự bắt đầu — trình biên dịch hoàn toàn không thấy nội dung chú thích, nên nó không được biên dịch theo dòng (A), không được coi là dữ liệu (B), và không biến thành câu lệnh rỗng (D).</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Errors that occur due to syntax errors belong to which of the following?|||Lỗi xảy ra do sai cú pháp thuộc loại nào sau đây?",
          "options": [
            {
              "text": "Compile time error|||Lỗi biên dịch (compile time)"
            },
            {
              "text": "Run time error|||Lỗi khi chạy (run time)"
            },
            {
          ...
          "explanation": "<div class=\"ml-en\">Syntax is checked by the compiler itself before any executable is produced, so a syntax mistake is caught and reported at COMPILE time. Run-time errors happen only after a valid program starts executing; linking errors are about missing/duplicate symbols, not grammar.</div><div class=\"ml-vi\">Cú pháp được chính trình biên dịch kiểm tra trước khi tạo ra file thực thi, nên lỗi cú pháp bị bắt và báo ngay lúc BIÊN DỊCH. Lỗi runtime chỉ xảy ra sau khi một chương trình hợp lệ đã bắt đầu chạy; lỗi linking liên quan tới ký hiệu thiếu/trùng chứ không phải ngữ pháp.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What will be the output of the following code snippet:<pre><code>#include &lt;stdlib.h&gt;\n#include &lt;stdio.h&gt;\nint x = 5;\nvoid someFunction() {\n    int x = 10;\n    {\n        int x;\n        x = 15;\n        printf(&quot;%d &quot;, x);\n    }\n    printf(&quot;%d &quot;, x);\n}\nint main() {\n    someFunction();\n    printf(&quot;%d&quot;, x);\n    return 0;\n}</code></pre>|||Đoạn mã sau in ra gì?<pre><code>#include &lt;stdlib.h&gt;\n#include &lt;stdio.h&gt;\nint x = 5;\nvoid someFunction() {\n    int x = 10;\n    {\n        int x;\n        x = 15;\n        printf(&quot;%d &quot;, x);\n    }\n    printf(&quot;%d &quot;, x);\n}\nint main() {\n    someFunction();\n    printf(&quot;%d&quot;, x);\n    return 0;\n}</code></pre>",
          "options": [
            {
              "text": "15 10 5|||15 10 5"
            },
            {
              "text": "15 5 10|||15 5 10"
            },
            {
              "text": "10 15 15|||10 15 15"
            },
            {
              "text": "10 10 5|||10 10 5"
            }
          ],
          "correctIndexes": [
            0
          ],
          "explanation": "<div class=\"ml-en\">Three nested scopes, three different x's. The innermost block declares its own x = 15 and prints 15. Once that block ends, only someFunction's x = 10 remains visible, printed as 10. Back in main, none of the local x's exist, so the GLOBAL x = 5 is printed. Result: \"15 10 5\". (Verified by running the code.)</div><div class=\"ml-vi\">Ba phạm vi lồng nhau, ba biến x khác nhau. Khối trong cùng khai báo x riêng = 15 và in ra 15. Khi khối đó kết thúc, chỉ còn x = 10 của someFunction hiển thị, in ra 10. Quay lại main, không còn biến x cục bộ nào tồn tại nên biến TOÀN CỤC x = 5 được in. Kết quả: \"15 10 5\". (Đã chạy thử kiểm chứng.)</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
        ...
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What will be the output of the C program?<pre><code>char temp;\nchar arr[10] = {1, 2, 3, 4, 5, 6, 7, 8};\ntemp = (arr + 1)[2];\nprintf(&quot;%d\\n&quot;, temp);</code></pre>|||Chương trình C sau in ra gì?<pre><code>char temp;\nchar arr[10] = {1, 2, 3, 4, 5, 6, 7, 8};\ntemp = (arr + 1)[2];\nprintf(&quot;%d\\n&quot;, temp);</code></pre>",
          "options": [
            {
              "text": "1|||1"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>(arr + 1)[2]</code> means <code>*((arr + 1) + 2)</code> = <code>*(arr + 3)</code> = <code>arr[3]</code>. Since arr = {1,2,3,4,...} with 0-based indexing, arr[3] is the FOURTH element = 4. (Verified by running the code.)</div><div class=\"ml-vi\"><code>(arr + 1)[2]</code> nghĩa là <code>*((arr + 1) + 2)</code> = <code>*(arr + 3)</code> = <code>arr[3]</code>. Vì arr = {1,2,3,4,...} với chỉ số bắt đầu từ 0, arr[3] là phần tử THỨ TƯ = 4. (Đã chạy thử kiểm chứng.)</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "A one dimensional array A has indices 1....100. Each element is a string and takes up three byte. The starting address of the array A[1] is 1110 decimal. The address of A[49] is ____|||Một mảng một chiều A có chỉ số 1....100. Mỗi phần tử là một chuỗi và chiếm ba byte. Địa chỉ bắt đầu của mảng A[1] là 1110 (thập phân). Địa chỉ của A[49] là ____",
          "options": [
            {
              "text": "1254|||1254"
            },
          ...
          "explanation": "<div class=\"ml-en\">The address formula is base + (index − first_index) × element_size. A[49] is 48 elements after A[1], so its address is 1110 + 48 × 3 = 1110 + 144 = 1254.</div><div class=\"ml-vi\">Công thức địa chỉ là base + (chỉ số − chỉ số đầu) × kích thước phần tử. A[49] cách A[1] 48 phần tử nên địa chỉ là 1110 + 48 × 3 = 1110 + 144 = 1254.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          ...
          "explanation": "<div class=\"ml-en\">a[i] and 2 are both int, so <code>a[i]/2</code> is INTEGER division (the fraction is truncated, not rounded): 1/2=0, 2/2=1, 3/2=1, 5/2=2, 6/2=3, 7/2=3 → \"0 1 1 2 3 3\". (Verified by running the code.)</div><div class=\"ml-vi\">a[i] và 2 đều là int nên <code>a[i]/2</code> là phép chia NGUYÊN (bỏ phần dư, không làm tròn): 1/2=0, 2/2=1, 3/2=1, 5/2=2, 6/2=3, 7/2=3 → \"0 1 1 2 3 3\". (Đã chạy thử kiểm chứng.)</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is the issue with the following code?<pre><code>char str[1];\ngets(str);\nprintf(&quot;%s&quot;, str);</code></pre>|||Đoạn mã sau có vấn đề gì?<pre><code>char str[1];\ngets(str);\nprintf(&quot;%s&quot;, str);</code></pre>",
          "options": [
            {
              "text": "Runtime error.|||Lỗi khi chạy (runtime)."
            },
            {
          ...
          "explanation": "<div class=\"ml-en\"><code>gets()</code> never checks the size of the buffer it writes into. <code>str</code> has room for only 1 character (no space even for a terminator), so typing anything writes past the end of the array — undefined behaviour / stack buffer overflow at RUN time. The code compiles fine (gcc only emits a deprecation warning for gets), so it is not a compile-time error.</div><div class=\"ml-vi\"><code>gets()</code> không bao giờ kiểm tra kích thước vùng đệm nó ghi vào. <code>str</code> chỉ có chỗ cho 1 ký tự (không đủ cho cả ký tự kết thúc), nên gõ bất kỳ ký tự nào cũng ghi ĐÈ ra ngoài mảng — hành vi không xác định / tràn bộ đệm stack lúc CHẠY. Mã vẫn biên dịch được (gcc chỉ cảnh báo gets bị deprecated) nên không phải lỗi biên dịch.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is the primary difference between global variables and local variables in C?|||Sự khác biệt CHÍNH giữa biến toàn cục và biến cục bộ trong C là gì?",
          "options": [
            {
              "text": "Global variables have a wider scope and can be accessed from any function, while local variables are limited to their function's scope.|||Biến toàn cục có phạm vi rộng hơn và có thể truy cập từ bất kỳ hàm nào, trong khi biến cục bộ chỉ giới hạn trong phạm vi hàm của nó."
            },
            {
              "text": "Global variables are initialized automatically to zero, while local variables require explicit initialization.|||Biến toàn cục tự động được khởi tạo bằng 0, trong khi biến cục bộ cần khởi tạo tường minh."
            },
            {
              "text": "Local variables cannot be modified after initialization, while global variables allow modification at any time.|||Biến cục bộ không thể thay đổi sau khi khởi tạo, trong khi biến toàn cục có thể thay đổi bất cứ lúc nào."
            },
            {
          ...
          "explanation": "<div class=\"ml-en\">The defining difference is SCOPE/lifetime: a global is declared outside any function and is visible to every function in the file, while a local variable exists only inside the function (or block) that declares it and disappears when that function returns. B is a true side-fact but is secondary, not the primary difference; C is false (locals can absolutely be reassigned); D reverses the truth (globals are usable everywhere, not just in main).</div><div class=\"ml-vi\">Khác biệt CỐT LÕI là PHẠM VI/vòng đời: biến toàn cục khai báo bên ngoài mọi hàm và mọi hàm trong file đều thấy được, còn biến cục bộ chỉ tồn tại trong hàm (hoặc khối) khai báo nó và biến mất khi hàm đó kết thúc. B là một sự thật phụ đúng nhưng không phải khác biệt chính; C sai (biến cục bộ hoàn toàn có thể gán lại); D nói ngược (biến toàn cục dùng được ở khắp nơi chứ không chỉ trong main).</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What does the following code print?<pre><code>float a = 3.0;\nfloat b = 2.0;\nint r = a % b;\nprintf(&quot;%d&quot;, r);</code></pre>|||Đoạn mã sau in ra gì?<pre><code>float a = 3.0;\nfloat b = 2.0;\nint r = a % b;\nprintf(&quot;%d&quot;, r);</code></pre>",
          "options": [
            {
              "text": "Compiler error|||Lỗi biên dịch"
            },
          ...
          "explanation": "<div class=\"ml-en\">The <code>%</code> (modulo) operator in C only accepts INTEGER operands. Here a and b are both <code>float</code>, so <code>a % b</code> does not compile — \"invalid operands to binary %\". To get a remainder of floats you must use <code>fmod(a, b)</code> from math.h instead. (Verified: gcc rejects this exact line.)</div><div class=\"ml-vi\">Toán tử <code>%</code> (chia lấy dư) trong C chỉ chấp nhận toán hạng kiểu SỐ NGUYÊN. Ở đây a và b đều là <code>float</code> nên <code>a % b</code> không biên dịch được — \"invalid operands to binary %\". Muốn lấy phần dư của số thực phải dùng <code>fmod(a, b)</code> trong math.h. (Đã xác nhận: gcc từ chối đúng dòng này.)</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          ...
          "options": [
            {
              "text": "No error, output is 1111|||Không lỗi, in ra 1111"
            },
            {
              "text": "No error, output is 1|||Không lỗi, in ra 1"
            },
            {
              "text": "Compile time error, no break statements|||Lỗi biên dịch, thiếu break"
            },
            {
              "text": "Compile time error, case label outside switch statement|||Lỗi biên dịch, nhãn case nằm ngoài switch"
            }
          ],
          "correctIndexes": [
            3
          ],
          "explanation": "<div class=\"ml-en\">There are no braces after <code>switch (a)</code>, so C's rule \"if/switch/while without braces controls only the single NEXT statement\" applies: only <code>case 1: printf(&quot;%d&quot;, a);</code> belongs to the switch. <code>case 2:</code> and <code>case 3:</code> are left as ordinary labeled statements OUTSIDE any switch, which is illegal — \"case label not within a switch statement\". (Verified: gcc rejects exactly this.)</div><div class=\"ml-vi\">Không có ngoặc nhọn sau <code>switch (a)</code> nên quy tắc của C \"if/switch/while không có ngoặc chỉ điều khiển ĐÚNG MỘT câu lệnh kế tiếp\" được áp dụng: chỉ <code>case 1: printf(&quot;%d&quot;, a);</code> thuộc về switch. <code>case 2:</code> và <code>case 3:</code> trở thành các câu lệnh có nhãn bình thường nằm NGOÀI mọi switch, điều này không hợp lệ — \"case label not within a switch statement\". (Đã xác nhận: gcc từ chối đúng như vậy.)</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          ...
          "options": [
            {
              "text": "Bitwise AND|||Bitwise AND (AND theo bit)"
            },
            {
              "text": "Bitwise OR|||Bitwise OR (OR theo bit)"
            },
            {
              "text": "Logical AND|||Logical AND (AND logic)"
            },
            {
              "text": "Logical OR|||Logical OR (OR logic)"
            }
          ],
          "correctIndexes": [
            0
          ],
          "explanation": "<div class=\"ml-en\">One <code>&amp;</code> is bitwise AND — it operates bit by bit on the whole value. Two <code>&amp;&amp;</code> would be the logical AND. Likewise a single <code>|</code> is bitwise OR and <code>||</code> is logical OR.</div><div class=\"ml-vi\">Một dấu <code>&amp;</code> là AND theo BIT — tác động từng bit của cả giá trị. Hai dấu <code>&amp;&amp;</code> mới là AND logic. Tương tự một dấu <code>|</code> là OR theo bit và <code>||</code> là OR logic.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Which of the following declaration is not supported by C?|||Khai báo nào sau đây KHÔNG được C hỗ trợ?",
          "options": [
            {
              "text": "<code>string a;</code>|||<code>string a;</code>"
            },
            {
              "text": "<code>float a = 3e2;</code>|||<code>float a = 3e2;</code>"
            },
            {
              "text": "<code>int* a;</code>|||<code>int* a;</code>"
            },
            {
              "text": "<code>float *a;</code>|||<code>float *a;</code>"
            },
            {
              "text": "<code>char * a;</code>|||<code>char * a;</code>"
            }
          ],
          "correctIndexes": [
            0
          ],
          "explanation": "<div class=\"ml-en\">C has no built-in <code>string</code> keyword/type — strings in C are just arrays of <code>char</code> or <code>char*</code> pointers (that type belongs to C++'s <code>std::string</code>). <code>3e2</code> is valid scientific-notation syntax for 300.0, and spacing around <code>*</code> in a pointer declaration (<code>int* a</code>, <code>float *a</code>, <code>char * a</code>) never matters to the compiler.</div><div class=\"ml-vi\">C không có kiểu/từ khoá <code>string</code> sẵn có — chuỗi trong C chỉ là mảng <code>char</code> hoặc con trỏ <code>char*</code> (kiểu string thuộc về <code>std::string</code> của C++). <code>3e2</code> là cú pháp khoa học hợp lệ cho 300.0, và khoảng trắng quanh dấu <code>*</code> khi khai báo con trỏ (<code>int* a</code>, <code>float *a</code>, <code>char * a</code>) không hề ảnh hưởng tới trình biên dịch.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          ...
          "options": [
            {
              "text": "A \"for\" loop is used for iterating over a sequence of elements, while a \"while\" loop is used to execute a block of code repeatedly as long as a condition is true.|||Vòng lặp \"for\" dùng để duyệt qua một dãy phần tử (biết trước số lần lặp), còn vòng lặp \"while\" dùng để thực thi một khối lệnh lặp lại chừng nào điều kiện còn đúng."
            },
            {
              "text": "A \"for\" loop is primarily used for conditional statements, while a \"while\" loop is designed for sequential processing.|||Vòng lặp \"for\" chủ yếu dùng cho câu lệnh điều kiện, còn vòng lặp \"while\" dành cho xử lý tuần tự."
            },
            {
              "text": "The use of \"for\" and \"while\" loops is interchangeable; there is no substantial difference between them.|||Vòng lặp \"for\" và \"while\" có thể dùng thay nhau; không có khác biệt đáng kể."
            },
            {
              "text": "A \"for\" loop is exclusively used with arrays, while a \"while\" loop is used with numerical calculations.|||Vòng lặp \"for\" chỉ dùng với mảng, còn vòng lặp \"while\" dùng cho tính toán số học."
            }
          ],
          "correctIndexes": [
            0
          ],
          "explanation": "<div class=\"ml-en\"><code>for</code> bundles initialization, condition and update in one line, making it the natural choice when the number of iterations is known in advance (e.g. walking through an array). <code>while</code> only has a condition, fitting cases where the loop should keep running as long as something stays true, with no fixed iteration count. Both CAN express either pattern, but this is their idiomatic difference — options B and D are made-up restrictions, and C denies a real (if stylistic) difference.</div><div class=\"ml-vi\"><code>for</code> gộp khởi tạo, điều kiện và cập nhật trong một dòng, phù hợp khi biết trước số lần lặp (ví dụ duyệt mảng). <code>while</code> chỉ có điều kiện, phù hợp khi vòng lặp cần chạy tiếp chừng nào một điều gì đó còn đúng, không có số lần lặp cố định. Cả hai ĐỀU có thể diễn đạt cả hai kiểu, nhưng đây là khác biệt thông dụng của chúng — phương án B và D là những ràng buộc bịa ra, còn C phủ nhận một khác biệt có thật (dù mang tính phong cách).</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          ...
          "explanation": "<div class=\"ml-en\">A \"magic value\" is a raw literal such as <code>3.14159</code> or <code>100</code> scattered through the code. Replacing it with <code>#define PI 3.14159</code> gives it a single symbolic name defined in one place, so a value that has to change per platform or per build is changed once — that is what makes the code portable and maintainable. Option C is the exact opposite of what #define does, and it neither shortens the code nor adds complexity.</div><div class=\"ml-vi\">Một \"magic value\" là một hằng số viết trực tiếp như <code>3.14159</code> hay <code>100</code> rải rác trong mã. Thay bằng <code>#define PI 3.14159</code> tạo cho nó MỘT tên gợi nhớ định nghĩa ở một chỗ, nên một giá trị cần đổi theo nền tảng hay theo bản build chỉ cần sửa một lần — đó là điều làm mã khả chuyển và dễ bảo trì. Phương án C ngược hẳn với tác dụng thật của #define, và nó cũng không làm mã ngắn hơn hay phức tạp hơn.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          ...
          "explanation": "<div class=\"ml-en\">Trace i: outer loop first pass — i=0&lt;5, so i becomes 1 and prints \"Hi-\". The inner while now runs while i&lt;4: i becomes 2,3,4 printing \"Hello-\" three times, then stops at i=4. Back to outer: i=4&lt;5, so i becomes 5 and prints one more \"Hi-\"; the inner while condition i&lt;4 is now false so it is skipped; outer condition i&lt;5 is now false too, loop ends. Total: \"Hi-\" + \"Hello-Hello-Hello-\" + \"Hi-\" = \"Hi-Hello-Hello-Hello-Hi-\". (Verified by running the code.)</div><div class=\"ml-vi\">Theo dõi i: vòng ngoài lượt 1 — i=0&lt;5, i tăng lên 1 và in \"Hi-\". Vòng trong chạy khi i&lt;4: i tăng lên 2,3,4, in \"Hello-\" ba lần rồi dừng khi i=4. Quay lại vòng ngoài: i=4&lt;5, i tăng lên 5 và in thêm một \"Hi-\"; điều kiện vòng trong i&lt;4 giờ sai nên bị bỏ qua; điều kiện vòng ngoài i&lt;5 cũng sai nên dừng hẳn. Tổng: \"Hi-\" + \"Hello-Hello-Hello-\" + \"Hi-\" = \"Hi-Hello-Hello-Hello-Hi-\". (Đã chạy thử kiểm chứng.)</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          ...
          "explanation": "<div class=\"ml-en\">For i=3: j runs 5,4 (both &gt; 3), adding 5+4=9. For i=4: j runs only 5 (&gt; 4), adding 5 → result=14. For i=5: no j satisfies j&gt;5, nothing added. Final result = 9+5 = 14. (Verified by running the code.)</div><div class=\"ml-vi\">Với i=3: j chạy 5,4 (đều &gt; 3), cộng 5+4=9. Với i=4: j chỉ chạy 5 (&gt; 4), cộng thêm 5 → result=14. Với i=5: không có j nào thoả j&gt;5, không cộng gì. Kết quả cuối = 9+5 = 14. (Đã chạy thử kiểm chứng.)</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "How many times will the do-while loop execute the code if the condition is false?|||Vòng lặp do-while thực thi bao nhiêu lần nếu điều kiện SAI?",
          "options": [
            {
              "text": "1|||1"
            },
          ...
          "explanation": "<div class=\"ml-en\">A <code>do { ... } while (condition);</code> loop runs its body FIRST and checks the condition only AFTER — that is the defining feature that separates it from a plain <code>while</code>. So even if the condition is false from the very start, the body has already executed exactly once before the check happens.</div><div class=\"ml-vi\">Vòng lặp <code>do { ... } while (condition);</code> chạy thân vòng lặp TRƯỚC rồi mới kiểm tra điều kiện SAU — đó là đặc điểm phân biệt nó với <code>while</code> thường. Vì vậy dù điều kiện sai ngay từ đầu, thân vòng lặp vẫn đã chạy đúng một lần trước khi việc kiểm tra diễn ra.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Which of the following is the correct syntax of the do-while statement?|||Cú pháp nào sau đây ĐÚNG cho câu lệnh do-while?",
          "options": [
            {
              "text": "<pre><code>do{ statements; }\nwhile(condition) ;</code></pre>|||<pre><code>do{ statements; }\nwhile(condition) ;</code></pre>"
            },
            {
              "text": "<pre><code>do while(condition)\n{ statements;}</code></pre>|||<pre><code>do while(condition)\n{ statements;}</code></pre>"
            },
            {
              "text": "<pre><code>do{ statements; }\nwhile (condition)</code></pre>|||<pre><code>do{ statements; }\nwhile (condition)</code></pre>"
            },
          ...
          "explanation": "<div class=\"ml-en\">The correct form is <code>do</code> then a brace-delimited block, then <code>while (condition)</code>, and — unlike a plain while loop — the do-while MUST end with a semicolon after the closing parenthesis. Option C is missing that trailing semicolon; option B puts the condition before the block (wrong order, that is a while-do); option D wraps the statements in parentheses instead of braces.</div><div class=\"ml-vi\">Cú pháp đúng là <code>do</code> rồi một khối lệnh trong ngoặc nhọn, tiếp đến <code>while (condition)</code>, và — khác với while thường — do-while BẮT BUỘC kết thúc bằng dấu chấm phẩy sau dấu ngoặc đóng. Phương án C thiếu dấu chấm phẩy cuối; phương án B đặt điều kiện trước khối lệnh (sai thứ tự, đó là while-do); phương án D bọc các câu lệnh trong ngoặc tròn thay vì ngoặc nhọn.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Why the following code is wrong?<pre><code>#include&lt;stdio.h&gt;\nint main()\n{\n    int choice;\n    scanf(&quot;%d&quot;,&amp;choice);\n    switch(choice)\n    {\n        case 1.5:\n            printf(&quot;You choice 1.5&quot;);\n        case 2.3:\n            printf(&quot;You choice 2.3&quot;);\n    }\n    return 0;\n}</code></pre>|||Vì sao đoạn mã sau SAI?<pre><code>#include&lt;stdio.h&gt;\nint main()\n{\n    int choice;\n    scanf(&quot;%d&quot;,&amp;choice);\n    switch(choice)\n    {\n        case 1.5:\n            printf(&quot;You choice 1.5&quot;);\n        case 2.3:\n            printf(&quot;You choice 2.3&quot;);\n    }\n    return 0;\n}</code></pre>",
          "options": [
            {
              "text": "The case label is the real number|||Nhãn case là số thực"
            },
            {
            ...
            {
              "text": "The variable name is wrong|||Tên biến sai"
            }
          ],
          "correctIndexes": [
            0
          ],
          "explanation": "<div class=\"ml-en\">Every <code>case</code> label in a switch must be an INTEGER constant expression. Here <code>1.5</code> and <code>2.3</code> are floating-point literals, which the compiler rejects outright — \"integer constant expression must have integer type, not double\". Missing break/default are just style issues that still compile; the variable name <code>choice</code> is used consistently. (Verified: gcc rejects exactly this.)</div><div class=\"ml-vi\">Mọi nhãn <code>case</code> trong switch phải là biểu thức hằng SỐ NGUYÊN. Ở đây <code>1.5</code> và <code>2.3</code> là số thực nên trình biên dịch từ chối thẳng — \"integer constant expression must have integer type, not double\". Thiếu break/default chỉ là vấn đề phong cách vẫn biên dịch được; tên biến <code>choice</code> được dùng nhất quán. (Đã xác nhận: gcc từ chối đúng như vậy.)</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What does the following code print?<pre><code>int i = 0;\nwhile (i &lt; 5) {\n    printf(&quot;%d &quot;, i);\n    i--;\n}</code></pre>|||Đoạn mã sau in ra gì?<pre><code>int i = 0;\nwhile (i &lt; 5) {\n    printf(&quot;%d &quot;, i);\n    i--;\n}</code></pre>",
          "options": [
            {
          ...
          "explanation": "<div class=\"ml-en\">i STARTS at 0 and only ever DECREASES (<code>i--</code>), so it moves away from 5, not toward it — the condition <code>i &lt; 5</code> stays true forever (0, -1, -2, -3, …). This prints an endless stream of decreasing numbers: an infinite loop. (Verified: the loop never stops on its own.)</div><div class=\"ml-vi\">i BẮT ĐẦU từ 0 và chỉ luôn GIẢM (<code>i--</code>), nên nó đi ra xa khỏi 5 chứ không tiến gần lại — điều kiện <code>i &lt; 5</code> mãi mãi đúng (0, -1, -2, -3, …). Nó in ra một chuỗi số giảm dần không dứt: lặp vô hạn. (Đã xác nhận: vòng lặp không bao giờ tự dừng.)</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "The statement for( ; ; ) does indeed represent an infinite loop, as it lacks the conditions that would cause the loop to terminate naturally. However, an infinite loop can still be terminated by using ________|||Câu lệnh for( ; ; ) đúng là một vòng lặp vô hạn vì thiếu điều kiện để tự dừng. Tuy nhiên một vòng lặp vô hạn vẫn có thể được KẾT THÚC bằng cách dùng ________",
          "options": [
            {
              "text": "exit(0)|||exit(0)"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>break</code> is the C keyword designed exactly for this: placed inside the loop body (usually behind an <code>if</code>), it immediately exits the nearest enclosing loop or switch — e.g. <code>for(;;) { if (done) break; }</code>. <code>exit(0)</code> and <code>abort()</code> would end the WHOLE PROGRAM, not just the loop, and <code>terminate</code> is not a real C keyword/function.</div><div class=\"ml-vi\"><code>break</code> là từ khoá C dựng riêng cho việc này: đặt trong thân vòng lặp (thường sau một <code>if</code>), nó thoát NGAY khỏi vòng lặp hoặc switch gần nhất — ví dụ <code>for(;;) { if (done) break; }</code>. <code>exit(0)</code> và <code>abort()</code> sẽ kết thúc CẢ CHƯƠNG TRÌNH chứ không chỉ vòng lặp, còn <code>terminate</code> không phải từ khoá/hàm có thật trong C.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          ...
          "explanation": "<div class=\"ml-en\">Two separate problems make this fail to compile. (1) <code>a</code> is declared <code>const int</code> but <code>a++</code> tries to modify it — \"cannot assign to variable with const-qualified type\". (2) Without braces, <code>if(a&gt;3) a++;</code> only owns the single statement <code>a++;</code>; the next line <code>break;</code> is a separate statement (and illegal outside a loop/switch besides), which then leaves a stray <code>else</code> with no <code>if</code> to attach to. (Verified: gcc rejects this exact code.)</div><div class=\"ml-vi\">Hai vấn đề riêng biệt khiến đoạn này không biên dịch được. (1) <code>a</code> được khai báo <code>const int</code> nhưng <code>a++</code> lại cố sửa nó — \"cannot assign to variable with const-qualified type\". (2) Không có ngoặc nhọn, <code>if(a&gt;3) a++;</code> chỉ sở hữu đúng một câu lệnh <code>a++;</code>; dòng <code>break;</code> kế tiếp trở thành câu lệnh riêng (mà bản thân nó cũng không hợp lệ ngoài vòng lặp/switch), khiến <code>else</code> còn lại bơ vơ không có <code>if</code> để gắn vào. (Đã xác nhận: gcc từ chối đúng đoạn mã này.)</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
        ...
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What happens when the return statement has a double expression and the function return type is int?|||Điều gì xảy ra khi câu lệnh return có biểu thức kiểu double nhưng kiểu trả về của hàm là int?",
          "options": [
            {
              "text": "There is error|||Có lỗi"
            },
            {
              "text": "This causes a run-time error|||Gây ra lỗi lúc chạy (run-time)"
            },
            {
              "text": "This is system dependent|||Tuỳ hệ thống"
            },
            {
          ...
          "explanation": "<div class=\"ml-en\">C freely performs an IMPLICIT narrowing conversion here: the double value is truncated (fraction discarded) to fit the declared <code>int</code> return type. This compiles cleanly (good compilers may emit a warning) and behaves the same on every conforming system — it is neither an error nor a run-time failure.</div><div class=\"ml-vi\">C tự động thực hiện chuyển đổi THU HẸP ngầm định ở đây: giá trị double bị cắt bỏ phần thập phân để vừa với kiểu trả về <code>int</code> đã khai báo. Mã vẫn biên dịch sạch (trình biên dịch tốt có thể cảnh báo) và hoạt động giống nhau trên mọi hệ thống tuân chuẩn — đây không phải lỗi biên dịch cũng không phải lỗi runtime.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          ...
          "options": [
            {
              "text": "A lowercase letter|||Chữ cái thường"
            },
            {
              "text": "An uppercase letter|||Chữ cái hoa"
            },
            {
              "text": "A digit|||Chữ số"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>islower(c)</code> tests exactly one thing: whether <code>c</code> is a lowercase alphabetic letter (a-z), returning non-zero (true) if so and 0 otherwise. Its counterpart <code>isupper()</code> tests for uppercase, <code>isdigit()</code> for digits, and <code>ispunct()</code> for punctuation.</div><div class=\"ml-vi\"><code>islower(c)</code> chỉ kiểm tra đúng một điều: <code>c</code> có phải chữ cái thường (a-z) hay không, trả về khác 0 (đúng) nếu phải và 0 nếu không. Hàm tương ứng <code>isupper()</code> kiểm tra chữ hoa, <code>isdigit()</code> kiểm tra chữ số, và <code>ispunct()</code> kiểm tra dấu câu.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "To generate a random integer, you need to declare the _____ library and then you can use the rand() function.|||Để sinh một số nguyên ngẫu nhiên, bạn cần khai báo thư viện _____ rồi mới dùng được hàm rand().",
          "options": [
            {
              "text": "stdio.h|||stdio.h"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>rand()</code> (and <code>srand()</code> to seed it) are declared in <code>stdlib.h</code>. <code>stdio.h</code> is for input/output, <code>ctype.h</code> is for character-testing functions like isdigit, and <code>conio.h</code> is a non-standard DOS console header.</div><div class=\"ml-vi\"><code>rand()</code> (và <code>srand()</code> để đặt hạt giống) được khai báo trong <code>stdlib.h</code>. <code>stdio.h</code> lo nhập/xuất, <code>ctype.h</code> chứa các hàm kiểm tra ký tự như isdigit, còn <code>conio.h</code> là header console DOS không chuẩn.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is the value of variable a after executing the following lines of codes:<pre><code>int a = 0;\nchar line1[] = &quot;Hi&quot;;\nchar line2[] = &quot;Hello&quot;;\na = strcmp(line1, line2);</code></pre>|||Giá trị của biến a là gì sau khi thực thi các dòng mã sau:<pre><code>int a = 0;\nchar line1[] = &quot;Hi&quot;;\nchar line2[] = &quot;Hello&quot;;\na = strcmp(line1, line2);</code></pre>",
          "options": [
            {
              "text": "0|||0"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>strcmp</code> compares character by character. The first character is the same ('H' == 'H'), so the SECOND character decides: 'i' (code 105) is greater than 'e' (code 101), so line1 comes AFTER line2 alphabetically, and strcmp returns a POSITIVE value (not necessarily related to string length). (Verified by running the code — result 4.)</div><div class=\"ml-vi\"><code>strcmp</code> so sánh từng ký tự. Ký tự đầu giống nhau ('H' == 'H'), nên ký tự THỨ HAI quyết định: 'i' (mã 105) lớn hơn 'e' (mã 101), nên line1 đứng SAU line2 theo thứ tự từ điển, và strcmp trả về giá trị DƯƠNG (không nhất thiết liên quan tới độ dài chuỗi). (Đã chạy thử kiểm chứng — kết quả là 4.)</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Consider an array a with n elements. The code snippet above is executed to calculate the value s. Which of the following options correctly describes the value of s after executing the code snippet?<pre><code>int a[MAX_SIZE];\nint s = 0;\nfor (int i = 0; i &lt; n; i++)\n    s = s +  a[i] * a[i];</code></pre>|||Xét một mảng a có n phần tử. Đoạn mã trên được chạy để tính giá trị s. Phương án nào mô tả ĐÚNG giá trị của s sau khi chạy đoạn mã?<pre><code>int a[MAX_SIZE];\nint s = 0;\nfor (int i = 0; i &lt; n; i++)\n    s = s +  a[i] * a[i];</code></pre>",
          "options": [
            {
              "text": "The sum of squares of all elements in the array a.|||Tổng bình phương của tất cả phần tử trong mảng a."
            },
            {
          ...
          "explanation": "<div class=\"ml-en\">Each iteration adds <code>a[i] * a[i]</code> — the element multiplied by ITSELF, i.e. its square — to the running total s. There is no check for even/odd, so s accumulates the sum of squares of every element, not a filtered subset.</div><div class=\"ml-vi\">Mỗi vòng lặp cộng <code>a[i] * a[i]</code> — phần tử nhân với CHÍNH NÓ, tức bình phương của nó — vào tổng chạy s. Không có kiểm tra chẵn/lẻ nào, nên s tích luỹ tổng bình phương của MỌI phần tử, không phải một tập con đã lọc.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What happens when you try to access the 11th element of an array with a size of 10 in the C language?|||Điều gì xảy ra khi bạn cố truy cập phần tử thứ 11 của một mảng có kích thước 10 trong ngôn ngữ C?",
          "options": [
            {
              "text": "The program compiles and runs without any errors, but the behavior is undefined|||Chương trình biên dịch và chạy không lỗi, nhưng hành vi không xác định"
            },
            {
              "text": "The program compiles and runs without any errors, and the 11th element is accessed|||Chương trình biên dịch và chạy không lỗi, và phần tử thứ 11 được truy cập bình thường"
            },
            {
              "text": "The program fails to compile due to a compilation error|||Chương trình không biên dịch được do lỗi biên dịch"
            },
            {
              "text": "The program compiles and runs without any errors, and the 11th element is automatically added to the array|||Chương trình biên dịch và chạy không lỗi, và phần tử thứ 11 tự động được thêm vào mảng"
            }
          ],
          "correctIndexes": [
            0
          ],
          "explanation": "<div class=\"ml-en\">C performs NO bounds checking on array access. <code>a[10]</code> on a 10-element array (valid indices 0-9) simply computes an address one element past the end and reads/writes whatever memory happens to be there — this is classic undefined behaviour: it might \"work\" by accident, corrupt other data, or crash, and the compiler gives no error or warning for it.</div><div class=\"ml-vi\">C KHÔNG kiểm tra giới hạn khi truy cập mảng. <code>a[10]</code> trên mảng 10 phần tử (chỉ số hợp lệ 0-9) đơn giản tính ra địa chỉ nằm ngay sau phần tử cuối và đọc/ghi vào bất kỳ vùng nhớ nào đang ở đó — đây là hành vi không xác định kinh điển: có thể \"chạy được\" một cách tình cờ, làm hỏng dữ liệu khác, hoặc crash, và trình biên dịch không hề báo lỗi hay cảnh báo gì.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          ...
          "explanation": "<div class=\"ml-en\">The function expects <code>int *b</code> (an address) as its second parameter, but the call <code>swap(x, y)</code> passes <code>y</code>, a plain <code>int</code> — not its address <code>&amp;y</code>. A conforming compiler rejects this type mismatch: \"incompatible integer to pointer conversion\". (Verified: the exact code fails to compile; forcing it through as a warning instead crashes at runtime, confirming <code>&amp;y</code> was required.)</div><div class=\"ml-vi\">Hàm này cần <code>int *b</code> (một địa chỉ) làm tham số thứ hai, nhưng lời gọi <code>swap(x, y)</code> lại truyền <code>y</code>, một <code>int</code> thường — không phải địa chỉ <code>&amp;y</code> của nó. Trình biên dịch chuẩn từ chối kiểu không khớp này: \"incompatible integer to pointer conversion\". (Đã xác nhận: đúng đoạn mã này không biên dịch được; ép cho qua dưới dạng cảnh báo thì crash lúc chạy, xác nhận cần phải truyền <code>&amp;y</code>.)</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is not a loop in C programming language?|||Cấu trúc nào KHÔNG phải một vòng lặp trong ngôn ngữ C?",
          "options": [
            {
              "text": "While|||While"
            },
          ...
          "explanation": "<div class=\"ml-en\">C has exactly three loop constructs: <code>while</code>, <code>do-while</code> and <code>for</code>. \"Repeat until\" is a loop found in languages like Pascal — C has no such keyword at all.</div><div class=\"ml-vi\">C chỉ có đúng ba cấu trúc lặp: <code>while</code>, <code>do-while</code> và <code>for</code>. \"Repeat until\" là vòng lặp có ở các ngôn ngữ như Pascal — C hoàn toàn không có từ khoá này.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
        ...
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What will be the output of the following program?<pre><code>void func(int *p){\n    int j =11;\n    *p = j;\n}\nint main() {\n    int i = 1;\n    int *p = &amp;i;\n    func(p);\n    printf(&quot;%d,%d&quot;, i, *p);\n    getchar();\n    return 0;\n}</code></pre>|||Chương trình sau in ra gì?<pre><code>void func(int *p){\n    int j =11;\n    *p = j;\n}\nint main() {\n    int i = 1;\n    int *p = &amp;i;\n    func(p);\n    printf(&quot;%d,%d&quot;, i, *p);\n    getchar();\n    return 0;\n}</code></pre>",
          "options": [
            {
              "text": "11,11|||11,11"
            },
          ...
          "explanation": "<div class=\"ml-en\">p holds the address of i, so <code>*p = j;</code> inside func writes 11 directly into i's memory — this is a REAL change visible after the function returns, because a pointer (address) was passed, not a copy of the value. Both <code>i</code> and <code>*p</code> refer to the same memory, so both print 11. (Verified by running the code.)</div><div class=\"ml-vi\">p giữ địa chỉ của i, nên <code>*p = j;</code> bên trong func ghi thẳng 11 vào vùng nhớ của i — đây là thay đổi THẬT SỰ và còn thấy được sau khi hàm kết thúc, vì một con trỏ (địa chỉ) được truyền chứ không phải bản sao giá trị. Cả <code>i</code> và <code>*p</code> đều trỏ tới cùng vùng nhớ nên đều in ra 11. (Đã chạy thử kiểm chứng.)</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Which sorting algorithm repeatedly steps through the list, compares adjacent elements, and swaps them if they are in the wrong order?|||Thuật toán sắp xếp nào lặp đi lặp lại việc duyệt qua danh sách, so sánh CÁC PHẦN TỬ LIỀN KỀ và hoán đổi chúng nếu sai thứ tự?",
          "options": [
            {
          ...
          "explanation": "<div class=\"ml-en\">This is the textbook definition of BUBBLE SORT: each pass compares each pair of NEIGHBOURING elements and swaps them if out of order, letting the largest \"bubble up\" to the end. Selection sort instead picks the MINIMUM of the remaining elements each pass (no adjacent-swap pattern), and \"binary sort\" is not a standard sorting algorithm name.</div><div class=\"ml-vi\">Đây là định nghĩa kinh điển của BUBBLE SORT: mỗi lượt so sánh từng CẶP PHẦN TỬ LIỀN KỀ và hoán đổi nếu sai thứ tự, khiến phần tử lớn nhất dần \"nổi\" về cuối. Selection sort thì chọn phần tử NHỎ NHẤT trong các phần tử còn lại mỗi lượt (không có kiểu hoán đổi liền kề), còn \"binary sort\" không phải tên thuật toán sắp xếp chuẩn.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Consider the following array of integers: {8, 1, 2, 7, 9}. What will be the state of the array after one pass of selection sort?|||Cho mảng số nguyên: {8, 1, 2, 7, 9}. Trạng thái mảng sau MỘT lượt (pass) selection sort là gì?",
          "options": [
            {
              "text": "{1, 8, 2, 7, 9}|||{1, 8, 2, 7, 9}"
            },
          ...
          "explanation": "<div class=\"ml-en\">One pass of selection sort scans the WHOLE array to find the smallest element (1, at index 1) and swaps it into the first position. Only that single swap happens in one pass: 8 and 1 trade places, giving {1, 8, 2, 7, 9}. All later positions are untouched until the next pass.</div><div class=\"ml-vi\">Một lượt selection sort quét TOÀN BỘ mảng để tìm phần tử nhỏ nhất (1, ở chỉ số 1) rồi hoán đổi nó vào vị trí đầu tiên. Chỉ một phép hoán đổi duy nhất xảy ra trong một lượt: 8 và 1 đổi chỗ, cho ra {1, 8, 2, 7, 9}. Các vị trí sau chưa hề bị đụng tới cho tới lượt kế tiếp.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "The fopen function is used to open a file. Is the following true or false?<br>\"r\" - tells fopen you will be reading data from the file. If no such file exists, fopen will fail.|||Hàm fopen dùng để mở tệp. Phát biểu sau ĐÚNG hay SAI?<br>\"r\" - báo cho fopen biết bạn sẽ ĐỌC dữ liệu từ tệp. Nếu tệp không tồn tại, fopen sẽ thất bại.",
          "options": [
            {
              "text": "True|||Đúng"
            },
          ...
          "explanation": "<div class=\"ml-en\">Mode <code>&quot;r&quot;</code> opens an EXISTING file for reading only — it never creates a new file. If the named file is not found, <code>fopen</code> returns <code>NULL</code> (failure), which is exactly why every real program must check the return value before using the FILE pointer. (Contrast with <code>&quot;w&quot;</code>, which creates the file if it does not exist.)</div><div class=\"ml-vi\">Chế độ <code>&quot;r&quot;</code> mở một tệp ĐÃ TỒN TẠI chỉ để đọc — nó không bao giờ tạo tệp mới. Nếu không tìm thấy tệp, <code>fopen</code> trả về <code>NULL</code> (thất bại), đó chính là lý do mọi chương trình thật sự đều phải kiểm tra giá trị trả về trước khi dùng con trỏ FILE. (Khác với <code>&quot;w&quot;</code>, chế độ này sẽ TẠO tệp nếu chưa có.)</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What functions to input strings? (Choose 2 answers)|||Những hàm nào dùng để NHẬP chuỗi? (Chọn 2 đáp án)",
          "options": [
            {
              "text": "scanf()|||scanf()"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>scanf(&quot;%s&quot;, ...)</code> and <code>gets()</code> both READ a string from standard input. <code>putc()</code> and <code>puts()</code> go the other way — they WRITE a character and a string respectively — and <code>getc()</code> reads only a SINGLE character, not a whole string.</div><div class=\"ml-vi\"><code>scanf(&quot;%s&quot;, ...)</code> và <code>gets()</code> đều ĐỌC một chuỗi từ đầu vào chuẩn. <code>putc()</code> và <code>puts()</code> làm chiều ngược lại — GHI một ký tự và một chuỗi — còn <code>getc()</code> chỉ đọc MỘT ký tự đơn, không phải cả chuỗi.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "How can you initialize a character array in C?|||Làm sao khởi tạo một mảng ký tự trong C?",
          "options": [
            {
              "text": "<code>char arr[] = &quot;Hello&quot;;</code>|||<code>char arr[] = &quot;Hello&quot;;</code>"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>char arr[] = &quot;Hello&quot;;</code> is the standard, valid C syntax — it auto-sizes the array and appends the null terminator. The other options are invalid: C requires the lowercase keyword <code>char</code> (not <code>Char</code>, and never with <code>[]</code> between the type and the name — that is Java/C#-style syntax); array element lists must use curly braces <code>{ }</code>, not square brackets <code>[ ]</code>.</div><div class=\"ml-vi\"><code>char arr[] = &quot;Hello&quot;;</code> là cú pháp C hợp lệ, chuẩn — nó tự tính kích thước mảng và tự thêm ký tự kết thúc null. Các phương án còn lại đều sai: C yêu cầu từ khoá viết thường <code>char</code> (không phải <code>Char</code>, và không bao giờ có <code>[]</code> giữa kiểu và tên — đó là cú pháp kiểu Java/C#); danh sách phần tử mảng phải dùng ngoặc nhọn <code>{ }</code>, không phải ngoặc vuông <code>[ ]</code>.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "Which function writes the character value of the argument c to the output stream referenced by fp?|||Hàm nào ghi giá trị ký tự của đối số c vào luồng xuất được tham chiếu bởi fp?",
          "options": [
            {
              "text": "<code>int fputc( int c, FILE *fp );</code>|||<code>int fputc( int c, FILE *fp );</code>"
            },
            {
              "text": "<code>int fputs( const char *s, FILE *fp );</code>|||<code>int fputs( const char *s, FILE *fp );</code>"
            },
            {
              "text": "<code>int fgetc( FILE * fp );</code>|||<code>int fgetc( FILE * fp );</code>"
            },
            {
              "text": "<code>char *fgets( char *buf, int n, FILE *fp );</code>|||<code>char *fgets( char *buf, int n, FILE *fp );</code>"
            }
          ],
          "correctIndexes": [
            0
          ],
          "explanation": "<div class=\"ml-en\"><code>fputc</code> writes exactly ONE character (its first argument <code>c</code>) to the stream <code>fp</code> — the description in the question is its literal definition. <code>fputs</code> writes a whole STRING, while <code>fgetc</code>/<code>fgets</code> go the opposite direction — they READ from the stream instead of writing to it.</div><div class=\"ml-vi\"><code>fputc</code> ghi đúng MỘT ký tự (đối số đầu <code>c</code>) vào luồng <code>fp</code> — mô tả trong câu hỏi chính là định nghĩa của nó. <code>fputs</code> ghi cả một CHUỖI, còn <code>fgetc</code>/<code>fgets</code> làm chiều ngược lại — chúng ĐỌC từ luồng thay vì ghi vào.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What function is used to find the length of a string in C?|||Hàm nào dùng để tìm ĐỘ DÀI của một chuỗi trong C?",
          "options": [
            {
              "text": "strlen()|||strlen()"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>strlen(s)</code> counts the characters in s up to (but not including) the terminating <code>&#x27;\\0&#x27;</code>. The other three names do not exist in the standard C library (note: <code>strnlen</code> does exist in POSIX/C11 Annex K with a bounded-length variant, but it is not the plain, universally available answer here).</div><div class=\"ml-vi\"><code>strlen(s)</code> đếm số ký tự trong s cho tới (không tính) ký tự kết thúc <code>&#x27;\\0&#x27;</code>. Ba tên còn lại không tồn tại trong thư viện chuẩn C (lưu ý: <code>strnlen</code> có tồn tại trong POSIX/C11 Annex K như một biến thể giới hạn độ dài, nhưng không phải đáp án phổ biến, có sẵn ở mọi nơi cho câu hỏi này).</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
        ...
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is a proper method of opening a file for writing as binary file?|||Cách nào ĐÚNG để mở một tệp để GHI dưới dạng tệp nhị phân (binary)?",
          "options": [
            {
              "text": "<code>FILE *f = fwrite( &quot;test.bin&quot;, &quot;b&quot; );</code>|||<code>FILE *f = fwrite( &quot;test.bin&quot;, &quot;b&quot; );</code>"
            },
            {
              "text": "<code>FILE *f = fopenb( &quot;test.bin&quot;, &quot;w&quot; );</code>|||<code>FILE *f = fopenb( &quot;test.bin&quot;, &quot;w&quot; );</code>"
            },
            {
              "text": "<code>FILE *f = fopen( &quot;test.bin&quot;, &quot;wb&quot; );</code>|||<code>FILE *f = fopen( &quot;test.bin&quot;, &quot;wb&quot; );</code>"
            },
            {
              "text": "<code>FILE *f = fwriteb( &quot;test.bin&quot; );</code>|||<code>FILE *f = fwriteb( &quot;test.bin&quot; );</code>"
            },
            {
              "text": "<code>FILE *f = fopen( &quot;test.bin&quot;, &quot;bw&quot; );</code>|||<code>FILE *f = fopen( &quot;test.bin&quot;, &quot;bw&quot; );</code>"
            }
          ],
          "correctIndexes": [
            2
          ],
          "explanation": "<div class=\"ml-en\">Only <code>fopen</code> opens files — <code>fwrite</code> writes data to an ALREADY-open stream (wrong signature entirely), and <code>fopenb</code>/<code>fwriteb</code> are not real C functions. The standard mode string for \"write, binary\" is written with the base mode first: <code>&quot;wb&quot;</code> — that is the exact form defined by the C standard; <code>&quot;bw&quot;</code> reverses the canonical order and is not one of the standard's recognised mode strings.</div><div class=\"ml-vi\">Chỉ <code>fopen</code> mới mở tệp — <code>fwrite</code> ghi dữ liệu vào một luồng ĐÃ mở sẵn (chữ ký hàm hoàn toàn khác), còn <code>fopenb</code>/<code>fwriteb</code> không phải hàm C có thật. Chuỗi chế độ chuẩn cho \"ghi, nhị phân\" viết với ký tự chế độ gốc trước: <code>&quot;wb&quot;</code> — đúng dạng chuẩn C quy định; <code>&quot;bw&quot;</code> đảo ngược thứ tự chuẩn và không phải một trong các chuỗi chế độ được chuẩn công nhận.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What will be the maximum size of a double variable?|||Kích thước của một biến double là bao nhiêu?",
          "options": [
            {
              "text": "8 bytes|||8 byte"
            },
          ...
          "explanation": "<div class=\"ml-en\"><code>double</code> is the double-precision floating-point type, universally implemented as 8 bytes (64 bits: 1 sign + 11 exponent + 52 mantissa bits under IEEE 754). <code>float</code> is 4 bytes and <code>long double</code> can be larger (often 16 on some platforms), but plain double is 8.</div><div class=\"ml-vi\"><code>double</code> là kiểu số thực dấu phẩy động độ chính xác kép, hầu như mọi nơi triển khai là 8 byte (64 bit: 1 bit dấu + 11 bit mũ + 52 bit định trị theo IEEE 754). <code>float</code> là 4 byte và <code>long double</code> có thể lớn hơn (thường 16 byte ở một số nền tảng), nhưng double thường là 8 byte.</div>"
        },
        {
          "kind": "MCQ",
          "points": 1,
          "prompt": "What is the result of the expression sizeof(int) in Windows?|||Kết quả của biểu thức sizeof(int) trên Windows là bao nhiêu?",
          "options": [
            {
              "text": "4|||4"
            },
