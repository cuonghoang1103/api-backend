    ...
    >
      ...
      <div className="ct-sidebar-footer">
        ...
        <button
          ...
          aria-label={dich(MODE_LABEL[mode])}
          title={`${dich(MODE_LABEL[mode])}  (⌘B)`}
        >
          {mode === 'full' ? (
          ...
          <span className="ct-nav-label">{dich('Thu gọn')}</span>
        </button>
      </div>
    </nav>
  );
