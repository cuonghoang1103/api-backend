...
import { ArrowLeft, ExternalLink, Github, Scale, Tag, User2, FileWarning } from 'lucide-react';
import {
  SOURCE_TREE,
  detailHref,
  getTypeBySlug,
    ...
    <>
      ...
      <header className="mb-5 rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] p-5">
        ...
        {record.files && record.files.length > 0 && (
          <details className="mt-4 rounded-lg border border-[var(--border-color)] bg-[var(--bg-surface)] p-3">
        ...
        <div className="min-w-0">
          {content ? (
            ...
            <section className="rounded-xl border border-dashed border-[var(--border-color)] px-6 py-10 text-center">
              <FileWarning size={24} className="mx-auto text-[var(--text-muted)] opacity-50" />
              ...
              <p className="mx-auto mt-1.5 max-w-md text-[13px] leading-relaxed text-[var(--text-muted)]">
                {filePath
                  ...
                  : 'Loại này không có một tệp nguồn riêng — nó là gói tổng hợp. Xem repo của tác giả để biết chi tiết.'}
              </p>
              {filePath && (
                <a
                  href={`${SOURCE_TREE}/${filePath}`}
