...
/**
 * VoiceComments — viewer comments on a Voice post. Read is public; posting/
 * liking requires login. Flat list with one level of replies. Likes toggle
 * optimistically. Users can edit/delete their own comments (admins may delete
 * any for moderation). Mirrors the Tech Trends ArticleComments component.
 */
...
import { voiceApi, type VoiceComment } from '@/lib/api';
...
function name(a: VoiceComment['author']): string {
...
export default function VoiceComments({ postId }: { postId: number }) {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const r = await voiceApi.listComments(postId);
  ...
  const post = async (content: string, parentId: number | null, after?: () => void) => {
    ...
    setPosting(true);
    try {
      await voiceApi.addComment(postId, content.trim(), parentId);
      after?.();
      await load();
    } catch (err) {
...
function CommentItem({
  ...
  const toggleLike = async () => {
    ...
    try {
      const r = await voiceApi.likeComment(comment.id);
  ...
  const saveEdit = async () => {
    ...
    try {
      await voiceApi.editComment(comment.id, editText.trim());
      setEditing(false);
      onChanged();
    } catch (err) {
  ...
  const remove = async () => {
    ...
    try {
      await voiceApi.deleteComment(comment.id);
      onChanged();
    } catch (err) {
