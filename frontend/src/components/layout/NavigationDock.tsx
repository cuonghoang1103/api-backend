...
// ── iOS cinematic spring easing ────────────────────────────────────
const sectionVariants: Variants = {
  hidden: { opacity: 0, x: -12 },
  visible: (i: number) => ({
    opacity: 1,
    x: 0,
    transition: { delay: i * 0.07, duration: 0.25, ease: APPLE_EASE },
  }),
};

const itemVariants: Variants = {
  default: { backgroundColor: 'rgba(0,0,0,0)', scale: 1 },
  hover: {
    scale: 0.97,
  ...
  tap: {
    ...
    transition: { duration: 0.08, ease: APPLE_EASE },
  },
  active: {
    scale: 1,
...
export default function NavigationDock({ isOpen, onToggle }: NavigationDockProps) {
  ...
  return (
    <>
      {/* ── Full-screen backdrop: blurs + dims entire page, disables page interaction ── */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            key="dock-backdrop"
      ...
      {/* ── Sidebar drawer: fixed overlay, slides in/out with translateX ── */}
      <AnimatePresence initial={false}>
        {isOpen && (
          <motion.nav
            key="dock-panel"
            initial={{ x: -DOCK_WIDTH - 24, opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            exit={{ x: -DOCK_WIDTH - 24, opacity: 0 }}
          ...
          >
            ...
            >
              {/* Header */}
              <div className="px-4 pt-5 pb-3 flex items-center justify-between shrink-0 border-b border-white/[0.05]">
