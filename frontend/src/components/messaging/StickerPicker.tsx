...
export default function StickerPicker({
  ...
  const body = (
    ...
    >
      <div className="flex items-center justify-between border-b border-theme-light px-3 py-2">
        <span className="text-xs font-semibold text-text-secondary">Nhãn dán</span>
        <button onClick={onClose} className="rounded-lg p-1 text-text-muted hover:bg-[var(--bg-surface-hover)] hover:text-text-primary">
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {stickers.map((s) => (
              <button
                ...
                className="flex aspect-square items-center justify-center rounded-lg p-1 transition-colors hover:bg-[var(--bg-surface-hover)]"
              >
      ...
      {/* Pack tabs */}
      {packs.length > 0 && (
        <div className="flex items-center gap-1 overflow-x-auto border-t border-theme-light p-1.5">
          {packs.map((p) => (
            <button
              key={p.id}
              type="button"
              ...
              className={`flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-lg transition-colors ${
                activePack === p.id ? 'bg-cyan-500/20 ring-1 ring-cyan-500/40' : 'hover:bg-[var(--bg-surface-hover)]'
              }`}
            >
              {p.coverUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
