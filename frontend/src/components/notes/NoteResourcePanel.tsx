  ...
  return (
    <div className="space-y-5 text-sm">
      ...
      <section>
        <div className="mb-2 flex items-center justify-between">
          ...
          <button
            ...
            disabled={uploading}
            className="flex items-center gap-1 rounded-md px-2 py-1 text-[11px] text-teal-300 hover:bg-teal-500/10 disabled:opacity-50 min-h-[32px]"
          >
        ...
        {attachments.length === 0 ? (
          <p className="px-1 text-[12px] text-slate-600">Chưa có tệp nào.</p>
        ) : (
          <ul className="space-y-1">
            {attachments.map((a) => {
              const Icon = fileIcon(a.fileType, a.fileName);
              return (
                <li key={a.id} className="group flex items-center gap-2 rounded-lg border border-white/[0.05] bg-white/[0.02] px-2.5 py-2 min-h-[44px]">
                  <Icon className="h-4 w-4 shrink-0 text-teal-300/80" />
                  <a href={a.fileUrl} target="_blank" rel="noopener noreferrer" className="min-w-0 flex-1 truncate text-[13px] text-slate-200 hover:text-teal-200" title={a.fileName}>
                    {a.fileName}
                  </a>
                  <span className="shrink-0 text-[10px] tabular-nums text-slate-500">{fmtSize(a.fileSizeBytes)}</span>
                  <button onClick={async () => { await notesApi.deleteAttachment(a.id); onChanged(); }} className="flex h-7 w-7 shrink-0 items-center justify-center rounded text-slate-600 opacity-0 transition-opacity hover:text-red-400 group-hover:opacity-100" aria-label="Xoá tệp">
      ...
      <section>
        ...
        {/* Add link */}
        <div className="mb-2 space-y-1.5 rounded-lg border border-white/[0.05] bg-white/[0.02] p-2">
          <input
            value={linkUrl}
            onChange={(e) => setLinkUrl(e.target.value)}
            placeholder="Dán URL (YouTube tự nhúng được)"
            className="w-full rounded-md bg-slate-800/60 px-2 py-1.5 text-[13px] text-slate-100 placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-teal-500/40"
          />
          <div className="flex gap-1.5">
            <input
              ...
              className="min-w-0 flex-1 rounded-md bg-slate-800/60 px-2 py-1.5 text-[13px] text-slate-100 placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-teal-500/40"
            />
            <button onClick={submitLink} disabled={adding || !linkUrl.trim()} className="flex shrink-0 items-center gap-1 rounded-md bg-teal-500/15 px-3 text-[12px] text-teal-200 hover:bg-teal-500/25 disabled:opacity-40 min-h-[36px]">
        ...
        {links.length === 0 ? (
          <p className="px-1 text-[12px] text-slate-600">Chưa có liên kết nào.</p>
        ) : (
          <ul className="space-y-2">
            {links.map((l) => {
              const yt = l.type === 'YOUTUBE' ? youtubeId(l.url) : null;
              return (
                <li key={l.id} className="group rounded-lg border border-white/[0.05] bg-white/[0.02] p-2">
                  <div className="flex items-center gap-2">
                    ...
                    <a href={l.url} target="_blank" rel="noopener noreferrer" className="min-w-0 flex-1 truncate text-[13px] text-slate-200 hover:text-teal-200" title={l.url}>
                      {l.label || hostOf(l.url)}
                    </a>
                    <a href={l.url} target="_blank" rel="noopener noreferrer" className="shrink-0 text-slate-500 hover:text-teal-300" aria-label="Mở liên kết"><ExternalLink className="h-3.5 w-3.5" /></a>
                    <button onClick={async () => { await notesApi.deleteLink(l.id); onChanged(); }} className="flex h-7 w-7 shrink-0 items-center justify-center rounded text-slate-600 opacity-0 transition-opacity hover:text-red-400 group-hover:opacity-100" aria-label="Xoá liên kết">
                      <Trash2 className="h-3.5 w-3.5" />
                    </button>
                  </div>
                  {yt && (
