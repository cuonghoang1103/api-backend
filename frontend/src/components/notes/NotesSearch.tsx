      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed left-1/2 top-[8vh] z-[81] w-[92vw] max-w-2xl -translate-x-1/2 overflow-hidden rounded-2xl border border-slate-200 dark:border-white/[0.08] bg-white dark:bg-[#0e1218] shadow-2xl"
          >
            {/* Search input */}
            <div className="flex items-center gap-2 border-b border-slate-200 dark:border-white/[0.06] px-3.5 py-3">
              <Search className="h-4 w-4 shrink-0 text-slate-500 dark:text-slate-500" />
              <input
                ref={inputRef}
                value={q}
            ...
            {/* Filters */}
            <div className="flex flex-wrap gap-1.5 border-b border-slate-200 dark:border-white/[0.06] px-3 py-2">
              <select value={subjectId ?? ''} onChange={(e) => setSubjectId(e.target.value ? Number(e.target.value) : undefined)} className="rounded-md bg-slate-100 dark:bg-slate-800/60 px-2 py-1 text-[12px] text-slate-800 dark:text-slate-200 focus:outline-none">
                <option value="">Mọi môn học</option>
                {subjects.map((s) => <option key={s.id} value={s.id}>{s.emoji ? `${s.emoji} ` : ''}{s.name}</option>)}
              </select>
              {tags.length > 0 && (
                <select value={tag ?? ''} onChange={(e) => setTag(e.target.value || undefined)} className="rounded-md bg-slate-100 dark:bg-slate-800/60 px-2 py-1 text-[12px] text-slate-800 dark:text-slate-200 focus:outline-none">
                  <option value="">Mọi thẻ</option>
                  {tags.map((t) => <option key={t} value={t}>#{t}</option>)}
                </select>
              )}
              {(subjectId || tag) && <button onClick={() => { setSubjectId(undefined); setTag(undefined); }} className="rounded-md px-2 py-1 text-[12px] text-slate-500 dark:text-slate-500 hover:text-slate-800 dark:hover:text-slate-300">Xoá lọc</button>}
            </div>

            {/* Results */}
            <div className="max-h-[55vh] overflow-y-auto p-2">
              {results.length === 0 ? (
                <p className="px-3 py-8 text-center text-[13px] text-slate-500 dark:text-slate-500 dark:text-slate-600">{q || subjectId || tag ? 'Không tìm thấy ghi chú phù hợp.' : 'Gõ để tìm trong toàn bộ ghi chú.'}</p>
              ) : (
                results.map((r) => {
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="truncate text-[13.5px] font-medium text-slate-900 dark:text-slate-100">{r.title || 'Không có tiêu đề'}</span>
                          {s && <span className="shrink-0 text-[10px] text-slate-500 dark:text-slate-500">{s.emoji ? `${s.emoji} ` : ''}{s.name}</span>}
                        </div>
                        {r.snippet && <p className="mt-0.5 line-clamp-2 text-[12px] text-slate-500 dark:text-slate-500">{r.snippet}</p>}
                      </div>
                    </button>
                  );
                })
