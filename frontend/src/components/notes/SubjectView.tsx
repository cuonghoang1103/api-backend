    ...
    <div className="mx-auto w-full max-w-[760px] px-4 sm:px-6 py-6">
      ...
      <section className="mb-7">
        <div className="mb-2 flex items-center justify-between">
          ...
          <button onClick={() => onAddNote(subject.id, null)} className="flex items-center gap-1 rounded-md px-2 py-1 text-[11px] text-teal-600 dark:text-teal-300 hover:bg-teal-100 dark:hover:bg-teal-500/10 min-h-[32px]">
        ...
        {directNotes.length === 0 && chapters.length === 0 ? (
          ...
          <ul className="space-y-1">
            {directNotes.map((n) => (
              <li key={n.id}>
                <button onClick={() => onSelectNote(n.id)} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left hover:bg-slate-100 dark:hover:bg-white/[0.04] min-h-[40px]">
            ...
            {chapters.map((ch) => (
              <li key={`ch-${ch.id}`} className="pt-1">
                ...
                <ul className="space-y-1">
                  {ch.notes.map((n) => (
                    <li key={n.id}>
                      <button onClick={() => onSelectNote(n.id)} className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 pl-4 text-left hover:bg-slate-100 dark:hover:bg-white/[0.04] min-h-[40px]">
                        <FileText className="h-3.5 w-3.5 shrink-0 text-slate-500 dark:text-slate-500" />
                        <span className="truncate text-[13px] text-slate-800 dark:text-slate-200">{n.title || 'Không có tiêu đề'}</span>
                      </button>
                    </li>
    ...
    <section className="mb-8">
      <div className="mb-2 flex items-center justify-between">
        ...
        <button
          onClick={create}
          disabled={creating}
          className="flex min-h-8 items-center gap-1 rounded-md px-2 py-1 text-[11px] text-teal-600 hover:bg-teal-100 disabled:opacity-50 dark:hover:bg-teal-500/10 dark:text-teal-300"
        >
