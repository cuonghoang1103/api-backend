...
const BASE = `${process.env.HOME}/Downloads/Đề thi các môn học ở trường/Kì 2/MAD101/FE`;
const SCRATCH = '/private/tmp/claude-501/-Users-admin-Downloads-api-backend/66c9798d-db0c-466c-8bba-c9cbb4f84581/scratchpad/mad101-old-fe-img';

const DECKS = {
  1: 'Đề 1 MAD101 - SP26 - RE',
};
...
for (const [n, folder] of Object.entries(DECKS)) {
  ...
  // group by leading question number (handles "06a"/"06b" style multi-part files, and a "q01"-style prefix)
  const groups = {};
  ...
  console.log(`=== MAD101 old FE Đề ${n} (${folder}) — ${Object.keys(groups).length} questions, ${files.length} files ===`);
  for (const [qn, group] of Object.entries(groups)) {
    ...
    if (group.length === 1) {
      ...
      // multi-part question (e.g. 06a/06b): stack vertically into one image
  ...
  const outJson = `content/exams/_work/mad101-old-fe-d${n}-image-urls.json`;
  try {
    execFileSync('node', ['scripts/upload-exam-images.mjs', '--dir', dDir, '--prefix', `MAD101/D${n}`, '--out', outJson], { stdio: 'inherit' });
  } catch (e) {
