/**
 * build-csd201-pe22.mjs — sinh content/exams/CSD201-PE22.mjs.
 *
 * Nguồn thật: csd201_t1_111288.rar (Đề "T1") — MỘT project (Q1),
 * cùng scenario "Hotel"/dataList+requestQueue như PE Đề 21 nhưng lớp
 * chính tên MyStore và f4 khác (đếm phòng trống thay vì tổng doanh
 * thu). 4 câu 2.5đ mỗi câu.
 *
 * LỜI GIẢI ĐÃ VERIFY THẬT: javac + java Main, cả 4 lựa chọn với
 * data.txt đi kèm — khớp BYTE-FOR-BYTE 100% với ví dụ trong đề.
 *
 * Cùng phát hiện như PE Đề 21: data.txt có phòng 002 (size=0) và 1
 * yêu cầu (size=0) bị lọc bỏ do vi phạm ràng buộc "size/price phải
 * > 0" — verify khớp đúng danh sách rút gọn của đề (đề này không có
 * phòng VIP như Đề 21).
 *
 * Seed: node scripts/academy-seed-exam.mjs --file ./content/exams/CSD201-PE22.mjs --apply
 */
import fs from 'node:fs';
import path from 'node:path';

const OUT = path.resolve(import.meta.dirname, '../content/exams/CSD201-PE22.mjs');
const ATTACHMENT_URL = 'https://media.cuongthai.com/files/exam-attachments/CSD201/PE22-Given.zip';

const B = (en, vi) => `${en}|||${vi}`;
const ML = (en, vi) => `<div class="ml-en">${en}</div><div class="ml-vi">${vi}</div>`;

const instructions = ML(
  `<p><strong>CSD201 PE INSTRUCTIONS</strong> (transcribed from the real exam paper, "T1").</p>
   <ol>
     <li>Software: NetBeans IDE 8.x, Java JDK 1.8. Download the given materials above.</li>
     <li>Do not use accented Vietnamese when writing comments in the programs.</li>
  ...
  `<p><strong>HƯỚNG DẪN PE CSD201</strong> (dịch nguyên văn từ đề thi thật, "T1").</p>
   <ol>
...
const SCENARIO = B(
  `<p><strong>Scenario: Hotel Management.</strong></p>
   <p><strong>Class Structure:</strong> <code>Room</code> (code, status — 0 empty / 1 occupied, size — must be &gt; 0, price — must be &gt; 0) · <code>Node</code> — an info object (Room) plus a next pointer · <code>dataList</code> — a singly linked list managing all Room objects in the hotel · <code>requestQueue</code> — a queue (implemented as a linked list) whose nodes' info contains only size (number of people renting) and price (max amount the customer can pay); size &gt; 0 and price &gt; 0 · <code>MyStore</code> — the main class, holding a dataList (rooms) and a requestQueue (pending orders).</p>`,
  `<p><strong>Bối cảnh: Quản lý Khách sạn.</strong></p>
   <p><strong>Cấu trúc lớp:</strong> <code>Room</code> (code, status — 0 trống / 1 đang có khách, size — phải &gt; 0, price — phải &gt; 0) · <code>Node</code> — đối tượng info (Room) và con trỏ next · <code>dataList</code> — danh sách liên kết đơn quản lý toàn bộ phòng · <code>requestQueue</code> — hàng đợi (cài bằng danh sách liên kết) mà info của node chỉ chứa size (số người thuê) và price (số tiền tối đa khách trả được); size &gt; 0 và price &gt; 0 · <code>MyStore</code> — lớp chính, chứa 1 dataList (các phòng) và 1 requestQueue (các đơn chờ).</p>`,
);

const SOLVED = '/tmp/csd201-pe22-src/src/MyStore.java';
const ORIG = '/tmp/csd201-pe22-orig/CSD201_T1_111288/PaperNo_1/Q1/src/MyStore.java';
...
const q1 = {
  ...
  prompt: SCENARIO + B(
    `<p><strong>a. f1(): 2.5 marks – Load data</strong></p>
     <p>To complete f1(), students need to carry out two specific tasks: implement the function <code>addLast()</code> in the dataList class and the function <code>enQueue()</code> (similar to addLast) in the requestQueue class.</p>`,
  ...
  explanation: B(
    `<p>Verified by compiling against the real given project (Room.java/Node.java/Main.java/Lib.java unchanged) and running choice 1 — output matches the paper's expected example byte-for-byte. The bundled data.txt lists room "002" with size=0 and one request with size=0 — both violate the documented invariant ("size must be &gt; 0", "price must be &gt; 0"); <code>addLast()</code> and <code>enQueue()</code> both skip such invalid entries, matching the paper's shorter lists exactly.</p>`,
    `<p>Đã kiểm bằng cách biên dịch cùng project given thật (Room.java/Node.java/Main.java/Lib.java giữ nguyên) và chạy lựa chọn 1 — output khớp byte-for-byte với ví dụ trong đề. data.txt đi kèm liệt kê phòng "002" size=0 và 1 yêu cầu size=0 — cả hai vi phạm ràng buộc đã ghi ("size phải &gt; 0", "price phải &gt; 0"); cả <code>addLast()</code> và <code>enQueue()</code> đều bỏ qua các bản ghi vi phạm này, khớp đúng danh sách ngắn hơn của đề.</p>`
  ),
  rubric: [
...
const q3 = {
  ...
  prompt: SCENARIO + B(
    `<p><strong>c. f3(): 2.5 marks – Serve all requests in the requestQueue</strong></p>
     <p>Perform the pair of operations <code>deQueue()</code> and <code>rent()</code> for all elements in the requestQueue.</p>`,
...
const q4 = {
  kind: 'CODE', points: 2.5, language: 'java',
  prompt: SCENARIO + B(
    `<p><strong>d. f4(): 2.5 marks – Count the available Rooms after serve all requests in the requestQueue</strong></p>
     <p>You should: (1) perform the pair of operations deQueue() and rent() for all elements in the requestQueue, then (2) count the available Rooms with status = 0.</p>`,
    `<p><strong>d. f4(): 2.5 điểm – Đếm số phòng còn trống sau khi phục vụ hết yêu cầu</strong></p>
     <p>Cần: (1) áp dụng deQueue()+rent() cho mọi phần tử trong requestQueue, rồi (2) đếm số phòng còn status = 0.</p>`
  ),
  starterCode: starter,
  sampleSolution: solved,
  expectedOutput: `Data List: (001,0,10,200) (003,0,3,70) (004,0,4,100) (005,0,3,70)\nRequest  : (1,100) (12,500) (4,50) (4,400)\nData List: (001,0,10,200) (003,0,3,70) (004,1,4,100) (005,1,3,70)\nRequest  : Empty\nAvailable Room(s): 2`,
  explanation: B(
    `<p>Verified by compiling against the real given project and running choice 4 — output matches the paper's expected example byte-for-byte. Same serve-all logic as f3 (004 and 005 end up rented), then counts rooms with <code>status==0</code>: 001 and 003 → 2.</p>`,
    `<p>Đã kiểm bằng biên dịch project given thật và chạy lựa chọn 4 — khớp byte-for-byte với ví dụ trong đề. Cùng logic phục vụ hết như f3 (004 và 005 được thuê), rồi đếm phòng có <code>status==0</code>: 001 và 003 → 2.</p>`
  ),
  rubric: [
    ...
    { id: 'count_available', criterion: B('Correctly counts rooms with status == 0.', 'Đếm đúng phòng có status == 0.'), weight: 1, maxScore: 1 },
  ],
};
...
const spec = {
  ...
  exams: [{
    ...
    peType: 'CODE',
    code: 'PE22-T1',
    title: 'PE Đề 22 — Practical Exam (T1)|||PE Đề 22 — Thi thực hành (T1)',
    ...
    attachmentName: 'CSD201-PE22-Given.zip',
    instructions,
    isPublished: true,
