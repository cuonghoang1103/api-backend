...
// ⛔⛔ ĐỪNG chạy lại script này rồi commit thẳng kết quả.
// SOLVED/ORIG dưới đây trỏ vào /tmp — thư mục tạm, KHÔNG nằm trong repo và
// KHÔNG còn nội dung đã dùng lúc dựng deck. Bản deck đang commit chứa hai bản
// sửa tay mà script này KHÔNG sinh ra: (1) `if (head == null) tail = null;`
// nằm TRONG vòng lặp removeFirst, (2) biến `Bird c` (script sinh ra `Bird b`).
// Chạy lại sẽ âm thầm trả mã về bản CŨ chưa sửa. Đo thật 07/09/2026: sinh lại
// làm lệch 9 trường mã ở PE35 và 10 ở PE36.
// Muốn sinh lại thì trước hết phải khôi phục đúng nguồn Java vào /tmp, hoặc
// giữ lại các trường mã từ bản commit (xem cách làm ở commit sửa lỗi |||).
