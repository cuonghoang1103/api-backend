/**
 * build-pmg201c-pe12.mjs — sinh content/exams/PMG201c-PE12.mjs.
 *
 * Nguồn thật: "PMG201c - FA25 - PE2" ("PMG201c - Practical Exam 2 (Fall
 * 2025)", kênh tin tức số cho cộng đồng đại học). Không có solution.
 * Không có CPM/EVM số học — Request 4 chỉ yêu cầu XÁC ĐỊNH quan hệ trình
 * tự (FS/SS/SF/FF) giữa hoạt động, không tính ES/EF/LS/LF hay đường găng.
 * Trọng số gốc: Req1=20%, Req2=20%, Req3=30%, Req4=30% → 2/2/3/3 (10đ).
 *
 * Seed: node scripts/academy-seed-exam.mjs --file ./content/exams/PMG201c-PE12.mjs --apply
 */
import fs from 'node:fs';
import path from 'node:path';

const OUT = path.resolve(import.meta.dirname, '../content/exams/PMG201c-PE12.mjs');

const B = (en, vi) => `${en}|||${vi}`;
const ML = (en, vi) => `<div class="ml-en">${en}</div><div class="ml-vi">${vi}</div>`;

const projectContext = B(
  `<div class="pe-system"><b>Context:</b><p>Local media outlets do not cover events that matter to the university community. In this project, you must establish a digital news channel (e.g., a weekly podcast or video series) focused exclusively on this niche. The project requires producing and distributing at least one high-quality piece of content per week and reaching a milestone of <b>500 unique subscribers/followers</b>. The continuous production cycle must be maintained for a <b>three-month period</b>. The total budget for promotional boosting and equipment is strictly <b>$500</b>, with the budget constraint that all software used for editing and production must be free or open-source. For quality, the content must maintain a high level of journalistic integrity, defined by receiving <b>zero verifiable complaints of factual inaccuracy</b> and ensuring that <b>95% of viewers/listeners complete the content</b> (low drop-off rate).</p></div>`,
  `<div class="pe-system"><b>Bối cảnh:</b><p>Truyền thông địa phương không đưa tin về những sự kiện quan trọng với cộng đồng đại học. Ở dự án này, bạn phải xây một kênh tin tức số (ví dụ: podcast hoặc series video hằng tuần) chuyên biệt cho mảng này. Dự án yêu cầu sản xuất và phát hành ít nhất 1 nội dung chất lượng cao mỗi tuần và đạt mốc <b>500 người theo dõi/đăng ký duy nhất</b>. Chu kỳ sản xuất liên tục phải duy trì trong <b>3 tháng</b>. Ngân sách cho quảng bá và thiết bị bị giới hạn nghiêm ngặt ở <b>500 USD</b>, với ràng buộc mọi phần mềm biên tập/sản xuất phải miễn phí hoặc mã nguồn mở. Về chất lượng, nội dung phải giữ tính chính trực báo chí cao, định nghĩa bằng <b>zero khiếu nại xác minh được về sai lệch sự thật</b> và đảm bảo <b>95% người xem/nghe hoàn thành nội dung</b> (tỉ lệ bỏ giữa chừng thấp).</p></div>`,
);

const instructions = ML(
  `<p><strong>PMG201c – Practical Examination 2 (Fall 2025) — university-community digital news channel</strong>. This is a written project-management practical exam. There is no code to write; each answer is graded by an AI grader against the rubric shown per question.</p>` + projectContext,
  `<p><strong>PMG201c – Thi thực hành 2 (Fall 2025) — kênh tin tức số cộng đồng đại học</strong>. Đây là bài thi thực hành quản lý dự án dạng viết. Không có mã nguồn cần viết; mỗi câu trả lời được AI chấm theo tiêu chí ghi ở từng câu.</p>` + projectContext,
);
...
const spec = {
  ...
  exams: [{
    ...
    peType: 'WRITE',
    code: 'PE12',
    title: 'PMG201c – Practical Exam 2 (Fall 2025), University-Community News Channel|||PMG201c – Thi thực hành 2 (Fall 2025), Kênh tin tức cộng đồng đại học',
