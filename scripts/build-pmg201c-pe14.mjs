/**
 * build-pmg201c-pe14.mjs — sinh content/exams/PMG201c-PE14.mjs.
 *
 * Nguồn thật: "PMG201c - SU 2024 - PE2" ("PMG201c – Final PE #2 (Summer
 * 2024)"), đề MỞ — tự chọn dự án. Có scripts.txt (đúng y yêu cầu paper.pdf,
 * không thêm gì) + 3 file solution-N.txt là bài làm mẫu sinh viên (không
 * phải solution chính thức, văn phong bị công cụ đổi từ đồng nghĩa tự
 * động làm lệch — "understudies"=students, "extend"=project). Không copy
 * — tự dựng bối cảnh riêng: "FU Textbook Exchange" (chợ trao đổi sách
 * giáo trình cũ giữa sinh viên FU), khác các dự án đã dùng ở PE7/PE8/PE13.
 *
 * Trọng số gốc: 5 yêu cầu × 20% đều nhau → 2 điểm/câu (10đ).
 *
 * Seed: node scripts/academy-seed-exam.mjs --file ./content/exams/PMG201c-PE14.mjs --apply
 */
import fs from 'node:fs';
import path from 'node:path';

const OUT = path.resolve(import.meta.dirname, '../content/exams/PMG201c-PE14.mjs');

const B = (en, vi) => `${en}|||${vi}`;
const ML = (en, vi) => `<div class="ml-en">${en}</div><div class="ml-vi">${vi}</div>`;

const projectContext = B(
  `<div class="pe-system"><b>Context (open-ended — this paper lets you pick your own project; the assumed project used in these sample answers is stated below):</b><p>Consider a project that you are working on, have worked on before, or one that is currently running around you. Pick a single project within your field of study at FU (for example, a software project for SE students). <br/><b>Assumed project for these sample answers: "FU Textbook Exchange" (TBX)</b> — a web marketplace where FU students list, browse, and arrange the exchange or sale of used textbooks among themselves.</p></div>`,
  `<div class="pe-system"><b>Bối cảnh (đề MỞ — bạn tự chọn dự án; dự án giả định dùng trong các câu trả lời mẫu dưới đây được nêu rõ):</b><p>Nghĩ về 1 dự án bạn đang làm, đã làm, hoặc đang diễn ra quanh bạn. Chọn 1 dự án trong ngành học tại FU (ví dụ: dự án phần mềm cho sinh viên SE). <br/><b>Dự án giả định dùng cho các câu trả lời mẫu: "FU Textbook Exchange" (TBX)</b> — chợ web nơi sinh viên FU đăng, tìm kiếm, và thu xếp trao đổi/bán sách giáo trình cũ cho nhau.</p></div>`,
);

const instructions = ML(
  `<p><strong>PMG201c – Final PE #2 (Summer 2024)</strong>. This is a written, open-ended project-management practical exam — you pick your own project. There is no code to write; each answer is graded by an AI grader against the rubric shown per question.</p>` + projectContext,
  `<p><strong>PMG201c – Final PE #2 (Summer 2024)</strong>. Đây là bài thi thực hành quản lý dự án dạng viết, đề MỞ — bạn tự chọn dự án. Không có mã nguồn cần viết; mỗi câu trả lời được AI chấm theo tiêu chí ghi ở từng câu.</p>` + projectContext,
);

const q1 = {
  kind: 'WRITE', points: 2,
  prompt: B(
    `<p><strong>Request 1 (20%):</strong> provide the project name (title) and give a detailed description (answering what, why, how, etc.) in which you mention clearly its key characteristics: purpose/objectives, timeframe, customer/sponsor, project milestones, success criteria, and other project constraints (cost, resources, etc.).</p>`,
    `<p><strong>Yêu cầu 1 (20%):</strong> nêu tên dự án và mô tả chi tiết (cái gì, tại sao, như thế nào), nêu rõ đặc điểm chính: mục đích/mục tiêu, thời gian, khách hàng/nhà tài trợ, mốc dự án, tiêu chí thành công, ràng buộc khác (chi phí, nguồn lực...).</p>`,
  ),
  sampleSolution: B(
    `<p><strong>Project title:</strong> FU Textbook Exchange (TBX).</p>
     ...
     <p><strong>Other constraints:</strong> Cost — capped at $35,000. Resources — 1 project manager, 3 developers, 1 tester, 1 UI/UX designer. Technology — must integrate FU's SSO system for identity verification (a hard external dependency, not optional).</p>`,
    `<p><strong>Tên dự án:</strong> FU Textbook Exchange (TBX).</p>
  ...
  rubric: [
    { id: 'title_description', criterion: B('Gives a clear title and a description that genuinely answers what/why/how.', 'Nêu tên rõ ràng và mô tả thực sự trả lời cái gì/tại sao/như thế nào.'), weight: 1, maxScore: 0.5 },
    { id: 'all_characteristics', criterion: B('Covers all 6 required characteristics: purpose/objectives, timeframe, customer/sponsor, milestones, success criteria, other constraints.', 'Bao quát đủ 6 đặc điểm yêu cầu: mục đích/mục tiêu, thời gian, khách hàng/nhà tài trợ, mốc, tiêu chí thành công, ràng buộc khác.'), weight: 1, maxScore: 1.2 },
    { id: 'internal_consistency', criterion: B('Milestones, timeframe, and constraints are internally consistent with the stated project.', 'Mốc, thời gian, và ràng buộc nhất quán với dự án đã nêu.'), weight: 1, maxScore: 0.3 },
  ],
};
...
const q2 = {
  kind: 'WRITE', points: 2,
  prompt: B(
    `<p><strong>Request 2 (20%):</strong> create a Responsibility Assignment Matrix (RAM) with 10 work packages or activities that can be assigned to the team, and define the specific team members or roles who will complete each piece of work (R-Responsible, A-Accountable, C-Consult, or I-Inform).</p>`,
    `<p><strong>Yêu cầu 2 (20%):</strong> xây bảng Phân công trách nhiệm (RAM) với 10 gói công việc/hoạt động gán cho nhóm, xác định vai trò/thành viên hoàn thành từng việc (R-Responsible, A-Accountable, C-Consult, I-Inform).</p>`,
  ),
  sampleSolution: B(
    `<table><tr><th>Work Package/Activity</th><th>Project Manager</th><th>Developers</th><th>Tester</th><th>UI/UX Designer</th><th>Database Admin</th></tr>
     ...
     <p><i>R=Responsible, A=Accountable, C=Consult, I=Inform. Every row has exactly one A, and no cell is left blank/ambiguous.</i></p>`,
    `<table><tr><th>Gói công việc/Hoạt động</th><th>Project Manager</th><th>Lập trình viên</th><th>Tester</th><th>UI/UX Designer</th><th>Database Admin</th></tr>
  ...
  rubric: [
    { id: 'ten_work_packages', criterion: B('Lists at least 10 distinct, realistic work packages/activities.', 'Liệt kê ít nhất 10 gói công việc/hoạt động khác nhau, thực tế.'), weight: 1, maxScore: 1 },
    { id: 'raci_assignment', criterion: B('Every work package has a full R/A/C/I assignment across team members/roles, with exactly one Accountable per row.', 'Mỗi gói công việc có đủ gán R/A/C/I cho các vai trò, đúng 1 người Accountable mỗi dòng.'), weight: 1, maxScore: 1.4 },
    { id: 'realistic_roles', criterion: B('The roles/team members used match the resources stated in Request 1.', 'Vai trò/thành viên dùng khớp nguồn lực đã nêu ở Yêu cầu 1.'), weight: 1, maxScore: 0.2 },
  ],
};
...
const q3 = {
  kind: 'WRITE', points: 2,
  prompt: B(
    `<p><strong>Request 3 (20%):</strong> define at least 5 quality assurance (QA) or quality control (QC) activities for the project. For each activity, classify it as QA or QC and clearly describe its purpose, when to perform it, and how to perform it.</p>`,
    `<p><strong>Yêu cầu 3 (20%):</strong> xác định ít nhất 5 hoạt động đảm bảo chất lượng (QA) hoặc kiểm soát chất lượng (QC). Mỗi hoạt động phân loại QA/QC và mô tả rõ mục đích, khi nào làm, cách làm.</p>`,
  ),
  sampleSolution: B(
    `<p><strong>1. Code review (QA):</strong> purpose — ensure code quality and adherence to the team's coding standards before it reaches testing. When — during development, before each pull request is merged into the main branch. How — a peer developer reviews the diff against a checklist (naming conventions, error handling, SSO-security patterns) and approves or requests changes.</p>
     ...
     <p><strong>5. User acceptance testing (QC):</strong> purpose — verify the system genuinely meets real student needs before launch. When — after integration testing passes, just before the Deployment milestone. How — a small group of real FU students uses the platform in a controlled pilot, following guided scenarios (list a book, search for a book, message a seller), with their feedback logged.</p>`,
    `<p><strong>1. Code review (QA):</strong> mục đích — đảm bảo chất lượng mã và tuân thủ chuẩn code của nhóm trước khi vào kiểm thử. Khi nào — trong lúc phát triển, trước mỗi pull request được gộp vào nhánh chính. Cách làm — 1 lập trình viên khác review diff theo checklist (quy ước đặt tên, xử lý lỗi, mẫu bảo mật SSO) rồi duyệt hoặc yêu cầu sửa.</p>
  ...
  rubric: [
    { id: 'five_activities', criterion: B('Defines at least 5 distinct QA/QC activities relevant to a software project.', 'Xác định ít nhất 5 hoạt động QA/QC khác nhau, gắn với dự án phần mềm.'), weight: 1, maxScore: 0.7 },
    { id: 'correct_classification', criterion: B('Each activity is correctly classified as QA (process-focused, preventive) or QC (product-focused, inspection/testing).', 'Mỗi hoạt động phân loại đúng QA (tập trung quy trình, phòng ngừa) hay QC (tập trung sản phẩm, kiểm tra/test).'), weight: 1, maxScore: 0.7 },
    { id: 'purpose_when_how', criterion: B('Each activity states purpose, timing, and method clearly and specifically to this project.', 'Mỗi hoạt động nêu rõ mục đích, thời điểm, cách làm, cụ thể gắn với dự án này.'), weight: 1, maxScore: 0.6 },
  ],
};
...
const q4 = {
  kind: 'WRITE', points: 2,
  prompt: B(
    `<p><strong>Request 4 (20%):</strong> identify at least 4 project phases/iterations and 2 main deliverables in each phase/iteration. Provide the description for each identified deliverable about its content/purpose.</p>`,
    `<p><strong>Yêu cầu 4 (20%):</strong> xác định ít nhất 4 giai đoạn/iteration dự án và 2 sản phẩm bàn giao chính mỗi giai đoạn. Mô tả nội dung/mục đích từng sản phẩm bàn giao.</p>`,
  ),
  sampleSolution: B(
    `<p><strong>Phase 1 — Initiation:</strong></p>
     ...
     <li>Deliverable 2: Project Closure Report — final report summarizing what was delivered, lessons learned, and the post-launch support plan.</li></ul>`,
    `<p><strong>Giai đoạn 1 — Khởi động:</strong></p>
  ...
  rubric: [
    { id: 'four_phases', criterion: B('Identifies at least 4 distinct project phases/iterations.', 'Xác định ít nhất 4 giai đoạn/iteration dự án khác nhau.'), weight: 1, maxScore: 0.6 },
    { id: 'two_deliverables_each', criterion: B('Each phase has at least 2 main deliverables.', 'Mỗi giai đoạn có ít nhất 2 sản phẩm bàn giao chính.'), weight: 1, maxScore: 0.7 },
    { id: 'deliverable_descriptions', criterion: B('Each deliverable has a clear description of its content/purpose, specific to this project rather than generic.', 'Mỗi sản phẩm bàn giao có mô tả nội dung/mục đích rõ ràng, cụ thể gắn dự án này, không chung chung.'), weight: 1, maxScore: 0.7 },
  ],
};
...
const q5 = {
  kind: 'WRITE', points: 2,
  prompt: B(
    `<p><strong>Request 5 (20%):</strong> provide at least 5 main cost items together with a description and an estimation for each cost item needed to complete the project. The estimation must be expressed in effort (man-days, man-hours).</p>`,
    `<p><strong>Yêu cầu 5 (20%):</strong> nêu ít nhất 5 khoản chi phí chính kèm mô tả và ước tính công sức (man-day, man-hour) để hoàn thành dự án.</p>`,
  ),
  sampleSolution: B(
    `<p><strong>1. Development effort:</strong> time spent by the 3 developers writing and unit-testing the listing/search, messaging, and SSO-integration modules. Estimation: 220 man-days.</p>
     ...
     <p><strong>6. Deployment &amp; post-launch support:</strong> time spent deploying to production and handling early user-reported issues in the first month after launch. Estimation: 15 man-days.</p>`,
    `<p><strong>1. Công sức phát triển:</strong> thời gian 3 lập trình viên viết và unit-test mô-đun đăng/tìm kiếm, nhắn tin, tích hợp SSO. Ước tính: 220 man-day.</p>
  ...
  rubric: [
    { id: 'five_items', criterion: B('Provides at least 5 distinct, realistic main cost items covering the whole project (not just development).', 'Nêu ít nhất 5 khoản chi phí chính khác nhau, thực tế, bao quát cả dự án (không chỉ phát triển).'), weight: 1, maxScore: 1 },
    { id: 'description_estimation', criterion: B('Each item has a specific description and an effort estimation in man-days/man-hours.', 'Mỗi khoản có mô tả cụ thể và ước tính công sức bằng man-day/man-hour.'), weight: 1, maxScore: 1 },
  ],
};
