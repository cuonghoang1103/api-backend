/**
 * build-pmg201c-pe2.mjs — sinh content/exams/PMG201c-PE2.mjs.
 *
 * Nguồn thật: "PMG201c - FA 2024 - PE2" (chỉ có paper.pdf, KHÔNG có file
 * solution nào đi kèm lần này — khác PE1). Toàn bộ Request 4 (CPM) và
 * Request 5 (EVM) đã TỰ TÍNH TAY ĐỘC LẬP từ đầu (không có gì để đối chiếu).
 *
 * ⚠️⚠️ Request 4 có 1 CÁI BẪY CPM quan trọng, dễ bị bỏ sót: bảng precedence
 * cho ra đường găng Start→D→H→I→End = 19 tuần, NHƯNG đường thứ 2 dài nhất
 * Start→D→E→F→G→End = 17 tuần (float chỉ 2). Nếu đề bài yêu cầu "rút ngắn
 * lịch trình 3 tuần" mà CHỈ cắt đường găng gốc xuống 16 (19-3), đường thứ 2
 * (17 tuần) sẽ TỰ ĐỘNG trở thành đường găng MỚI vượt quá mục tiêu 16 tuần —
 * phải xử lý CẢ HAI đường cùng lúc (hoặc chọn cắt đúng hoạt động D, hoạt
 * động DUY NHẤT chung cho MỌI đường đi, để giảm đều tất cả cùng lúc). Đã tự
 * kiểm bằng cách tính lại forward+backward pass đầy đủ, xác nhận cả 4 đường
 * đi và đúng float từng activity trước khi viết lời giải.
 *
 * Request 1-3 (mô tả dự án/loại tổ chức/rủi ro): tự dựng bối cảnh dự án
 * (Hệ thống Quản lý Bệnh nhân Bệnh viện) nhất quán xuyên suốt cả 3 câu,
 * không có đáp án duy nhất — chấm theo rubric định tính.
 *
 * Seed: node scripts/academy-seed-exam.mjs --file ./content/exams/PMG201c-PE2.mjs --apply
 */
import fs from 'node:fs';
import path from 'node:path';

const OUT = path.resolve(import.meta.dirname, '../content/exams/PMG201c-PE2.mjs');

const B = (en, vi) => `${en}|||${vi}`;
const ML = (en, vi) => `<div class="ml-en">${en}</div><div class="ml-vi">${vi}</div>`;

const instructions = ML(
  `<p><strong>PMG201c – Final PE #2 (Fall 2024)</strong>. This is a written project-management practical exam — 5 requests, each worth 20% (2 points). There is no code to write; each answer is graded by an AI grader against the rubric shown per question. Write complete, well-reasoned answers — partial or vague answers lose rubric points even if the general idea is right.</p>`,
  `<p><strong>PMG201c – PE cuối kỳ #2 (Fall 2024)</strong>. Đây là bài thi thực hành quản lý dự án dạng viết — 5 yêu cầu, mỗi yêu cầu 20% (2 điểm). Không có mã nguồn cần viết; mỗi câu trả lời được AI chấm theo tiêu chí ghi ở từng câu. Viết câu trả lời đầy đủ, lập luận rõ ràng — trả lời chung chung/thiếu ý sẽ mất điểm rubric dù ý chính đúng hướng.</p>`,
);
...
const q1 = {
  kind: 'WRITE', points: 2,
  prompt: B(
    `<p><strong>Request 1 (20%):</strong> provide the project name and give a detailed description of the project, clearly covering its key characteristics: purpose/objectives (unique), customer/sponsor (clearly defined), success criteria, and other project constraints (cost, resources, or quality).</p>`,
    `<p><strong>Yêu cầu 1 (20%):</strong> đặt tên dự án và mô tả chi tiết, nêu rõ các đặc điểm chính: mục đích/mục tiêu (duy nhất), khách hàng/nhà tài trợ (xác định rõ ràng), tiêu chí thành công, và các ràng buộc khác (chi phí, nguồn lực, hoặc chất lượng).</p>`,
  ),
  sampleSolution: B(
    `<p><strong>Project Name:</strong> Hospital Patient Management System (HPMS)</p>
     ...
     <p><strong>Other constraints:</strong> Cost — budget capped at $500,000. Resources — team of 1 project manager, 4 developers, 2 QA engineers, 1 business analyst, and a part-time hospital IT liaison. Quality — the system must comply with healthcare data-privacy regulations and guarantee 99.9% uptime after launch.</p>`,
    `<p><strong>Tên dự án:</strong> Hệ thống Quản lý Bệnh nhân Bệnh viện (HPMS)</p>
  ...
  rubric: [
    { id: 'purpose', criterion: B('States a clear, unique purpose/objective.', 'Nêu rõ mục đích/mục tiêu duy nhất.'), weight: 1, maxScore: 0.5 },
    { id: 'sponsor', criterion: B('Clearly identifies a specific customer/sponsor.', 'Xác định rõ ràng 1 khách hàng/nhà tài trợ cụ thể.'), weight: 1, maxScore: 0.4 },
    { id: 'success_criteria', criterion: B('States concrete, measurable success criteria.', 'Nêu tiêu chí thành công cụ thể, đo lường được.'), weight: 1, maxScore: 0.6 },
...
const q2 = {
  kind: 'WRITE', points: 2,
  prompt: B(
    `<p><strong>Request 2 (20%):</strong> what type of organization do you think the project is using, and what type of organization do you think should apply or should have applied, along with the reasons for this? Provide relevant reasons for your answers.</p>`,
    `<p><strong>Yêu cầu 2 (20%):</strong> bạn nghĩ dự án đang dùng loại cấu trúc tổ chức nào, và loại nào nên áp dụng (hoặc lẽ ra nên áp dụng), kèm lý do? Nêu lý do phù hợp cho câu trả lời.</p>`,
  ),
  sampleSolution: B(
    `<p><strong>Current/assumed structure:</strong> a weak/functional matrix — the project team members (developers, QA, BA) most likely still report primarily to their functional department heads, with the project manager having limited formal authority, only coordinating across departments.</p>
     <p><strong>Recommended structure:</strong> a <b>strong matrix, or ideally a projectized structure</b> for the duration of the project. Reasons: (1) the project directly affects patient safety and involves sensitive healthcare data, so it needs fast, unambiguous decision-making — a PM with real authority over the dedicated team avoids the delays typical of functional silos; (2) the 6-month deadline is tight and requires full-time dedicated staff rather than people splitting time across multiple functional obligations; (3) the cross-functional nature (developers + hospital domain experts + compliance) benefits from a single point of accountability (the PM) rather than needing sign-off from multiple functional managers on every decision.</p>`,
    `<p><strong>Cấu trúc hiện tại/giả định:</strong> ma trận yếu/theo chức năng — các thành viên nhóm (lập trình viên, QA, BA) nhiều khả năng vẫn báo cáo chính cho trưởng bộ phận chức năng của họ, quản lý dự án có thẩm quyền hạn chế, chỉ điều phối liên phòng ban.</p>
  ...
  rubric: [
    { id: 'current_type', criterion: B('States a plausible current/assumed organization type with reasoning.', 'Nêu loại cấu trúc tổ chức hiện tại/giả định hợp lý kèm lý do.'), weight: 1, maxScore: 0.6 },
    { id: 'recommended_type', criterion: B('Recommends a specific organization type (functional/weak matrix/balanced matrix/strong matrix/projectized).', 'Đề xuất đúng 1 loại cấu trúc cụ thể (chức năng/ma trận yếu/ma trận cân bằng/ma trận mạnh/dự án hoá).'), weight: 1, maxScore: 0.6 },
    { id: 'reasoning', criterion: B('Gives specific, project-relevant reasons for the recommendation (tied to this project\'s actual characteristics, not generic).', 'Nêu lý do cụ thể, gắn với đặc điểm THẬT của dự án này (không chung chung).'), weight: 1, maxScore: 0.8 },
  ],
};
...
const q3 = {
  ...
  prompt: B(
    ...
    `<p><strong>Yêu cầu 3 (20%):</strong> xác định ít nhất 3 rủi ro đáng kể mà dự án có thể gặp. Với mỗi rủi ro, xác định khả năng xảy ra (probability) và mức ảnh hưởng (impact) nếu xảy ra. Tự tạo hệ thống chấm điểm cho probability và impact — có thể dùng Cao-Trung bình-Thấp; giá trị số; hoặc phần trăm. Định nghĩa phản ứng (response) cho từng rủi ro.</p>`,
  ),
  sampleSolution: B(
    `<p><strong>Scoring system:</strong> Probability and Impact each scored 1 (Low) – 2 (Medium) – 3 (High); Risk Score = Probability × Impact (range 1-9; ≥6 = high priority, 3-4 = medium, 1-2 = low).</p>
  ...
  rubric: [
    { id: 'three_risks', criterion: B('Identifies at least 3 distinct, plausible, project-relevant risks.', 'Xác định đủ ít nhất 3 rủi ro khác nhau, hợp lý, gắn với dự án.'), weight: 1, maxScore: 0.6 },
    { id: 'own_scoring', criterion: B('Defines its own explicit probability/impact scoring system (as requested) and applies it consistently to every risk.', 'Tự định nghĩa rõ hệ thống chấm điểm probability/impact (theo yêu cầu) và áp dụng nhất quán cho mọi rủi ro.'), weight: 1, maxScore: 0.7 },
    { id: 'response', criterion: B('Gives a specific, actionable response/mitigation for each risk.', 'Nêu phản ứng/giảm thiểu cụ thể, khả thi cho từng rủi ro.'), weight: 1, maxScore: 0.7 },
  ],
};
...
const q4 = {
  kind: 'WRITE', points: 2,
  prompt: B(
    `<p><strong>Request 4 (20%):</strong> you have defined and estimated a project schedule with the following activities:</p>${cpmTable}
     ...
     <li>Specify at least 3 solutions (with detailed explanation) to shorten this schedule by 3 weeks.</li></ul>`,
    `<p><strong>Yêu cầu 4 (20%):</strong> bạn đã xác định và ước lượng lịch trình dự án với các hoạt động sau:</p>${cpmTable}
  ...
  sampleSolution: B(
    `<p><strong>All paths (Start to End):</strong></p>
     ...
     <p><strong>Solution 3 — Fast-track by overlapping H and I, and overlapping E and F:</strong> instead of waiting for H to fully finish, start the first ~3 weeks of I's work once H is well underway (overlap), cutting the effective H→I duration by 3 weeks without adding resources. Since this alone doesn't touch the second-longest path, also overlap E and F by at least 1 week on that path. Trade-off: higher coordination/rework risk since downstream work starts before its input is fully finalized.</p>`,
  ...
  explanation: B(
    `<p><b>Verified independently by hand</b> (full forward + backward CPM pass from the raw precedence table, no source solution existed for this deck to check against) — 4 distinct paths, critical path Start-D-H-I-End at 19 weeks, float per activity all recomputed and cross-checked against the path durations.</p>`,
    `<p><b>Đã tự tính tay độc lập</b> (đầy đủ CPM xuôi+ngược từ bảng precedence gốc, đề này không có file solution nào để đối chiếu) — 4 đường đi khác nhau, đường găng Start-D-H-I-End ở 19 tuần, float từng hoạt động đều tính lại và đối chiếu chéo với thời lượng từng đường.</p>`,
  ),
  rubric: [
    { id: 'all_paths', criterion: B('Correctly lists all 4 distinct start-to-end paths with correct individual durations.', 'Liệt kê đúng đủ 4 đường đi khác nhau với thời lượng từng đường đúng.'), weight: 1, maxScore: 0.5 },
    { id: 'critical_path', criterion: B('Correctly identifies the critical path (Start-D-H-I-End) and total duration (19 weeks).', 'Xác định đúng đường găng (Start-D-H-I-End) và tổng thời lượng (19 tuần).'), weight: 1, maxScore: 0.4 },
    { id: 'near_critical_awareness', criterion: B('Recognizes that shortening only the original critical path by 3 weeks is not sufficient, because the second-longest path (17 weeks) would become the new bottleneck unless it is also addressed.', 'Nhận ra chỉ rút ngắn đường găng gốc 3 tuần là chưa đủ, vì đường dài thứ 2 (17 tuần) sẽ thành đường găng mới nếu không xử lý cùng.'), weight: 1, maxScore: 0.6 },
    { id: 'three_valid_solutions', criterion: B('Proposes at least 3 distinct, technically sound schedule-compression solutions (crashing and/or fast-tracking) that genuinely achieve a 3-week reduction.', 'Đề xuất đủ ít nhất 3 giải pháp rút ngắn lịch trình khác nhau, kỹ thuật hợp lý (crashing và/hoặc fast-tracking), thực sự đạt rút ngắn 3 tuần.'), weight: 1, maxScore: 0.5 },
  ],
};
...
const q5 = {
  kind: 'WRITE', points: 2,
  prompt: B(
    `<p><strong>Request 5 (20%):</strong> assuming your project was reported with the following status: AC = $120,000; PV = $125,000; CV = -$10,000. Calculate and provide your comments (current status, forecast and recommended adjustment) on the schedule and cost performance of the project.</p>`,
    `<p><strong>Yêu cầu 5 (20%):</strong> giả sử dự án được báo cáo với tình trạng: AC = 120,000$; PV = 125,000$; CV = -10,000$. Tính toán và nhận xét (tình trạng hiện tại, dự báo, đề xuất điều chỉnh) về hiệu suất tiến độ và chi phí của dự án.</p>`,
  ),
  sampleSolution: B(
    `<p><strong>Step 1 — derive EV from CV:</strong> CV = EV − AC → EV = CV + AC = −10,000 + 120,000 = <b>$110,000</b>.</p>
  ...
  explanation: B(
    `<p>Verified independently: since CV = EV − AC is given directly (−10,000) along with AC (120,000), EV must be derived first (EV = CV + AC = 110,000) before SV/SPI/CPI can be computed — a step easy to miss if a student tries to use the standard formulas directly without noticing EV isn't given. All formulas check out: SV=−15,000, SPI=0.88, CPI≈0.92.</p>`,
    `<p>Đã tự verify độc lập: vì đề cho CV = EV − AC trực tiếp (−10,000) cùng AC (120,000), phải suy ra EV trước (EV = CV + AC = 110,000) mới tính được SV/SPI/CPI — bước này dễ bị bỏ sót nếu học viên áp công thức chuẩn ngay mà không để ý EV không được cho sẵn. Mọi công thức đều khớp: SV=−15,000, SPI=0.88, CPI≈0.92.</p>`,
  ),
  rubric: [
    { id: 'derive_ev', criterion: B('Correctly derives EV = $110,000 from the given CV and AC (CV = EV - AC).', 'Suy ra đúng EV = 110,000$ từ CV và AC đã cho (CV = EV - AC).'), weight: 1, maxScore: 0.6 },
    { id: 'sv_spi_cpi', criterion: B('Correctly calculates SV (-$15,000), SPI (0.88), and CPI (≈0.92).', 'Tính đúng SV (-15,000$), SPI (0.88), và CPI (≈0.92).'), weight: 1, maxScore: 0.6 },
    { id: 'commentary', criterion: B('Correctly identifies the project is BOTH behind schedule AND over budget, with a sensible, specific recommendation.', 'Nhận ra đúng dự án VỪA trễ tiến độ VỪA vượt ngân sách, kèm đề xuất cụ thể hợp lý.'), weight: 1, maxScore: 0.8 },
  ],
};
