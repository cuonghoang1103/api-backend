/**
 * build-pmg201c-pe7.mjs — sinh content/exams/PMG201c-PE7.mjs.
 *
 * Nguồn thật: "PMG201c - PE1 - SP 2024" ("PMG201c – Final Practice
 * Examination #1"). KHÔNG có file solution. Request 1-3 là đề MỞ (tự chọn
 * 1 dự án bất kỳ, đề gợi ý ví dụ "software project for SE") — tự dựng bối
 * cảnh 1 dự án phần mềm (Hệ thống Đăng ký Học phần Sinh viên) nhất quán
 * xuyên suốt cả 3 câu.
 *
 * Request 4 (CPM) và Request 5 (EVM) đã TỰ TÍNH TAY ĐỘC LẬP từ đầu (không
 * có gì để đối chiếu): đường găng A-B-D-G-I=19 tuần; SPI=0.6, CPI=0.75,
 * EAC=$12,000, dự báo tiến độ 20 tháng.
 *
 * Seed: node scripts/academy-seed-exam.mjs --file ./content/exams/PMG201c-PE7.mjs --apply
 */
import fs from 'node:fs';
import path from 'node:path';

const OUT = path.resolve(import.meta.dirname, '../content/exams/PMG201c-PE7.mjs');

const B = (en, vi) => `${en}|||${vi}`;
const ML = (en, vi) => `<div class="ml-en">${en}</div><div class="ml-vi">${vi}</div>`;

const instructions = ML(
  `<p><strong>PMG201c – Final Practice Examination #1 (Spring 2024)</strong>. This is a written project-management practical exam. There is no code to write; each answer is graded by an AI grader against the rubric shown per question.</p>
...
const q1 = {
  kind: 'WRITE', points: 2,
  prompt: B(
    `<p><strong>Request 1:</strong> describe the selected project in detail, clearly presenting its key characteristics: project title, project purpose/objectives (unique), project timeframe (temporary), customer/sponsor (clearly defined), project milestones (time constraints), and other project constraints (cost, resources).</p>`,
    `<p><strong>Yêu cầu 1:</strong> mô tả chi tiết dự án đã chọn, nêu rõ đặc điểm chính: tên dự án, mục đích/mục tiêu (duy nhất), khung thời gian (tạm thời), khách hàng/nhà tài trợ (rõ ràng), mốc dự án (ràng buộc thời gian), và các ràng buộc khác (chi phí, nguồn lực).</p>`,
  ),
  sampleSolution: B(
    `<p><strong>Project Title:</strong> Student Course Registration System (SCRS)</p>
     ...
     <p><strong>Other constraints:</strong> Cost — capped at $60,000. Resources — 1 PM, 3 developers, 1 QA, 1 business analyst liaising with Academic Affairs. Quality — must handle peak concurrent load during the registration rush (thousands of students registering within the first hour).</p>`,
    `<p><strong>Tên dự án:</strong> Hệ thống Đăng ký Học phần Sinh viên (SCRS)</p>
  ...
  rubric: [
    { id: 'purpose_timeframe', criterion: B('States a clear unique purpose and a specific temporary timeframe.', 'Nêu rõ mục đích duy nhất và khung thời gian tạm thời cụ thể.'), weight: 1, maxScore: 0.6 },
    { id: 'sponsor', criterion: B('Clearly identifies a specific customer/sponsor.', 'Xác định rõ khách hàng/nhà tài trợ cụ thể.'), weight: 1, maxScore: 0.4 },
    { id: 'milestones_constraints', criterion: B('States concrete milestones and at least cost/resource constraints.', 'Nêu mốc cụ thể và ít nhất ràng buộc chi phí/nguồn lực.'), weight: 1, maxScore: 1 },
  ],
};
...
const q2 = {
  ...
  prompt: B(
    `<p><strong>Request 2:</strong> state what type of organization you think the project is using, and what type of organization you think it should or should have used, along with the reasons for this. Provide relevant reasons for your answers.</p>`,
    `<p><strong>Yêu cầu 2:</strong> nêu loại cấu trúc tổ chức bạn nghĩ dự án đang dùng, và loại nào lẽ ra nên dùng, kèm lý do?</p>`,
  ),
  sampleSolution: B(
    `<p><strong>Current/assumed structure:</strong> a balanced matrix — the university IT department likely runs multiple concurrent projects, so developers/QA split time across projects while a PM coordinates, but functional managers retain significant authority over staffing.</p>
     <p><strong>Recommended structure:</strong> a <b>strong matrix</b>. Reasons: (1) the hard deadline (before the registration window opens) demands a PM with real authority to prioritize this project's needs over other IT-department work when conflicts arise; (2) the project needs sustained, focused attention from a small dedicated core team (not people constantly pulled onto other university IT tickets) to hit peak-load quality requirements; (3) unlike a fully projectized structure, a strong matrix still lets specialists (e.g. a security reviewer) be shared with other university IT projects when not needed full-time on SCRS, which fits a university IT department's typical resource constraints better than standing up a fully separate team.</p>`,
    `<p><strong>Cấu trúc hiện tại/giả định:</strong> ma trận cân bằng — phòng IT trường thường chạy nhiều dự án cùng lúc, nên lập trình viên/QA chia thời gian cho nhiều dự án, PM điều phối nhưng trưởng bộ phận chức năng vẫn có thẩm quyền đáng kể về nhân sự.</p>
  ...
  rubric: [
    { id: 'current_and_recommended', criterion: B('States both a plausible current structure and a specific recommended type.', 'Nêu cả cấu trúc hiện tại hợp lý lẫn loại đề xuất cụ thể.'), weight: 1, maxScore: 0.7 },
    { id: 'reasoning', criterion: B('Gives specific, project-relevant reasons tied to the actual project characteristics (deadline, team focus, resource sharing).', 'Nêu lý do cụ thể gắn với đặc điểm thật của dự án (hạn định, tập trung nhóm, chia sẻ nguồn lực).'), weight: 1, maxScore: 0.8 },
  ],
};
...
const q3 = {
  kind: 'WRITE', points: 1.5,
  prompt: B(
    `<p><strong>Request 3:</strong> identify at least 3 significant risks that you believe the project faces. For each risk, estimate how likely it is to occur (the probability) and the impact if it does occur. Create your own scoring system for probability and impact. Define a response for each of these risks.</p>`,
    `<p><strong>Yêu cầu 3:</strong> xác định ít nhất 3 rủi ro đáng kể. Với mỗi rủi ro, ước lượng khả năng xảy ra và tác động nếu xảy ra. Tự tạo hệ thống chấm điểm. Định nghĩa phản ứng cho từng rủi ro.</p>`,
  ),
  sampleSolution: B(
    `<p><strong>Scoring system:</strong> Probability and Impact each scored 1 (Low) – 2 (Medium) – 3 (High); Risk Score = Probability × Impact.</p>
  ...
  rubric: [
    { id: 'three_risks', criterion: B('Identifies at least 3 distinct, plausible risks specific to this project.', 'Xác định đủ ít nhất 3 rủi ro khác nhau, hợp lý, gắn với dự án.'), weight: 1, maxScore: 0.5 },
    { id: 'own_scoring_system', criterion: B('Defines its own explicit probability/impact scoring system and applies it consistently.', 'Tự định nghĩa hệ thống chấm điểm probability/impact rõ ràng, áp dụng nhất quán.'), weight: 1, maxScore: 0.5 },
    { id: 'response', criterion: B('Gives a specific, actionable response for each risk.', 'Nêu phản ứng cụ thể, khả thi cho từng rủi ro.'), weight: 1, maxScore: 0.5 },
  ],
};
...
const q4 = {
  ...
  prompt: B(
    `<p><strong>Request 4:</strong> assume that you have defined and estimated the project schedule with the following high-level activities:</p>${cpmTable}
     ...
     <p>b) In the 8th week, while executing activity B, you find that you need to speed up the project schedule to recover a 3-week delay. Define 2 solutions, with relevant explanation and assumptions (if needed), to achieve this recovery (how it would help and difficulties/impacts to the project).</p>`,
  ...
  sampleSolution: B(
    `<p><strong>a) Critical path analysis:</strong></p>
  ...
  explanation: B(
    `<p><b>Verified independently by hand</b> (full forward + backward CPM pass, no source solution existed) — 4 distinct paths, critical path A-B-D-G-I at 19 weeks.</p>`,
    `<p><b>Đã tự tính tay độc lập</b> (đầy đủ CPM xuôi+ngược, đề này không có solution để đối chiếu) — 4 đường đi khác nhau, đường găng A-B-D-G-I ở 19 tuần.</p>`,
  ),
  rubric: [
    { id: 'critical_path', criterion: B('Correctly determines the minimum duration (19 weeks) via critical path analysis.', 'Xác định đúng thời lượng tối thiểu (19 tuần) qua phân tích đường găng.'), weight: 1, maxScore: 1 },
    { id: 'two_solutions', criterion: B('Proposes 2 distinct, technically sound recovery solutions targeting critical-path activities, each with benefit and difficulty/impact discussed.', 'Đề xuất đủ 2 giải pháp phục hồi khác nhau, nhắm đúng hoạt động đường găng, mỗi cái có bàn về lợi ích và khó khăn/ảnh hưởng.'), weight: 1, maxScore: 1.5 },
  ],
};
...
const q5 = {
  ...
  sampleSolution: B(
    ...
    `<p><strong>Dữ liệu cho:</strong> EV = $3,000; AC = $4,000; PV = $5,000; BAC = $9,000; thời lượng kế hoạch = 12 tháng.</p>
  ...
  explanation: B(
    `<p>Verified independently by hand from the given data (no source solution existed) — all standard EVM formulas applied: SPI=0.6, CPI=0.75, EAC=$12,000, schedule forecast=20 months.</p>`,
    `<p>Đã tự tính tay độc lập từ dữ liệu đề cho (không có solution để đối chiếu) — áp dụng đúng công thức EVM chuẩn: SPI=0.6, CPI=0.75, EAC=$12,000, dự báo tiến độ=20 tháng.</p>`,
  ),
  rubric: [
    { id: 'spi_cpi', criterion: B('Correctly calculates SPI (0.6) and CPI (0.75).', 'Tính đúng SPI (0.6) và CPI (0.75).'), weight: 1, maxScore: 0.6 },
    { id: 'sv_cv', criterion: B('Correctly calculates SV (-$2,000) and CV (-$1,000).', 'Tính đúng SV (-$2,000) và CV (-$1,000).'), weight: 1, maxScore: 0.5 },
    { id: 'eac_schedule_forecast', criterion: B('Correctly calculates EAC ($12,000) and the schedule forecast (20 months).', 'Tính đúng EAC ($12,000) và dự báo tiến độ (20 tháng).'), weight: 1, maxScore: 0.7 },
    { id: 'commentary', criterion: B('Correctly identifies the project as both over budget and behind schedule, with a sensible recommendation.', 'Nhận ra đúng dự án vừa vượt ngân sách vừa trễ tiến độ, kèm đề xuất hợp lý.'), weight: 1, maxScore: 0.7 },
  ],
};
