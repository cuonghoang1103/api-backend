/**
 * build-pro192-pe11.mjs — sinh content/exams/PRO192-PE11.mjs.
 *
 ...
 *
 * Nguồn đã verify: scratchpad pro192-pe/de11/q{0..3} — mỗi thư mục compile
 ...
 *
 * Seed: node scripts/academy-seed-exam.mjs --file ./content/exams/PRO192-PE11.mjs --apply
 */
import fs from 'node:fs';
import path from 'node:path';

const SRC = '/private/tmp/claude-501/-Users-admin-Downloads-api-backend/7a3dab05-4d89-4139-97aa-6d1578ef74f8/scratchpad/pro192-pe/de11';
const OUT = path.resolve(import.meta.dirname, '../content/exams/PRO192-PE11.mjs');
...
const ATTACHMENT_URL = 'https://media.cuongthai.com/files/exam-attachments/PRO192/PE11-Given.zip';
...
<ul>
  ...
  <li><code>getName(): String</code> — returns the string s, by removing the last character of the name.</li>
  <li><code>getPrice(): int</code> — return price.</li>
  <li><code>setPrice(price: int): void</code> — update price = new price.</li>
</ul>
...
<ul>
  ...
  <li><code>getName(): String</code> — trả về chuỗi s, đã bỏ ký tự cuối cùng của name.</li>
  <li><code>getPrice(): int</code> — trả về price.</li>
  <li><code>setPrice(price: int): void</code> — cập nhật price = giá trị mới.</li>
</ul>
<p><em>Không định dạng lại kết quả.</em></p>`;

const q1ExplainEn = `<p>Straightforward encapsulation. The only trap is <code>getName()</code>: it does NOT return the raw <code>name</code> field — it returns <code>name</code> with its LAST character removed (<code>name.substring(0, name.length() - 1)</code>), e.g. "vietnam" → "vietna". Everything else is a plain getter/setter/constructor pair.</p>
...
OUTPUT:
12</pre>`;
const q1ExplainVi = `<p>Đóng gói (encapsulation) đơn giản. Bẫy duy nhất nằm ở <code>getName()</code>: KHÔNG trả về nguyên field <code>name</code>, mà trả về <code>name</code> đã bỏ ký tự CUỐI CÙNG (<code>name.substring(0, name.length() - 1)</code>), ví dụ "vietnam" → "vietna". Các hàm còn lại là getter/setter/constructor thông thường.</p>
...
12`,
  ...
  rubric: [
    ...
    { id: 'getName', criterion: B('getName() correctly removes the last character (not the raw field).', 'getName() bỏ đúng ký tự cuối (không trả nguyên field).'), weight: 2, maxScore: 1 },
...
<ul>
  ...
  <li><code>setData(): void</code> — replace the last alphabetic character in type with the last digit of price.</li>
  <li><code>getValue(): String</code> — if the weight contains at least two even digits, return the concatenation of type and the number of even digits in the weight. Otherwise, return the concatenation of type and the total number of digits in the weight.</li>
</ul>`;
...
<ul>
  ...
  <li><code>setData(): void</code> — thay ký tự chữ cái cuối cùng trong type bằng chữ số cuối cùng của price.</li>
  <li><code>getValue(): String</code> — nếu weight chứa ít nhất 2 chữ số chẵn, trả về type nối với số lượng chữ số chẵn trong weight. Ngược lại, trả về type nối với tổng số chữ số trong weight.</li>
</ul>`;

const q2ExplainEn = `<p><strong>setData()</strong>: "last alphabetic character in type" is found by scanning from the end of the string for the first letter (<code>Character.isLetter</code>); "last digit of price" is <code>Math.abs(price) % 10</code>. Replace that one character in place, e.g. type="standard", price=21 → last digit '1' replaces the trailing 'd' → "standar1".</p>
<p><strong>getValue()</strong>: count how many of weight's digits (as a string, ignoring sign) are even (0,2,4,6,8). If ≥ 2, return <code>type + evenCount</code>; otherwise return <code>type + totalDigitCount</code>. Verified against BOTH given samples: weight=24 → digits {2,4} both even → 2 even digits ≥ 2 → "standard2". weight=135 → digits {1,3,5} none even → 0 even digits &lt; 2 → fall back to total digit count 3 → "standard3".</p>
...
Enter type: standard / price: 123 / weight: 24 → TC3
OUTPUT:
standard2

Enter type: standard / price: 12 / weight: 135 → TC3
OUTPUT:
standard3</pre>`;
const q2ExplainVi = `<p><strong>setData()</strong>: "ký tự chữ cái cuối cùng trong type" tìm bằng cách quét từ cuối chuỗi tới chữ cái đầu tiên gặp được (<code>Character.isLetter</code>); "chữ số cuối cùng của price" là <code>Math.abs(price) % 10</code>. Thay đúng 1 ký tự đó, ví dụ type="standard", price=21 → chữ số cuối '1' thay cho 'd' cuối → "standar1".</p>
<p><strong>getValue()</strong>: đếm trong các chữ số của weight (chuyển thành chuỗi, bỏ dấu âm) có bao nhiêu chữ số chẵn (0,2,4,6,8). Nếu ≥ 2, trả về <code>type + số_chữ_số_chẵn</code>; ngược lại trả về <code>type + tổng_số_chữ_số</code>. Đã đối chiếu với CẢ HAI mẫu trong đề: weight=24 → chữ số {2,4} đều chẵn → 2 chữ số chẵn ≥ 2 → "standard2". weight=135 → chữ số {1,3,5} không chữ số nào chẵn → 0 &lt; 2 → dùng tổng số chữ số = 3 → "standard3".</p>
...
Sample run 3 (TC 3 — getValue, type=standard price=123 weight=24):
OUTPUT:
standard2

Sample run 4 (TC 3 — getValue, type=standard price=12 weight=135):
OUTPUT:
standard3`,
  ...
  rubric: [
    ...
    { id: 'setData', criterion: B('setData() replaces exactly the last alphabetic character with the last digit of price.', 'setData() thay đúng ký tự chữ cái cuối bằng chữ số cuối của price.'), weight: 2, maxScore: 1 },
    { id: 'getValue', criterion: B('getValue() correctly branches on "at least two even digits" vs total digit count.', 'getValue() rẽ nhánh đúng theo "ít nhất 2 chữ số chẵn" và tổng số chữ số.'), weight: 2, maxScore: 1 },
  ],
};
...
const spec = {
  ...
  exams: [
    {
      kind: 'PE', peType: 'CODE',
      code: 'PE11-SU26-B1-P5',
      title: B('PRO192 — Practical Exam Đề 11 (SU26, Block 1, Paper 5)', 'PRO192 — Đề thi thực hành số 11 (SU26, Block 1, Paper 5)'),
      description: B(
...
fs.writeFileSync(
  OUT,
  `// AUTO-GENERATED by scripts/build-pro192-pe11.mjs — do not hand-edit, re-run the builder instead.\nexport default ${JSON.stringify(spec, null, 2)};\n`,
  'utf8',
);
