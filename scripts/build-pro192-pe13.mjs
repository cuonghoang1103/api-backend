/**
 * build-pro192-pe13.mjs — sinh content/exams/PRO192-PE13.mjs.
 *
 ...
 *
 * Nguồn đã verify: scratchpad pro192-pe/de13/q{0..3} — mỗi thư mục compile
 ...
 *
 * Seed: node scripts/academy-seed-exam.mjs --file ./content/exams/PRO192-PE13.mjs --apply
 */
import fs from 'node:fs';
import path from 'node:path';

const SRC = '/private/tmp/claude-501/-Users-admin-Downloads-api-backend/7a3dab05-4d89-4139-97aa-6d1578ef74f8/scratchpad/pro192-pe/de13';
const OUT = path.resolve(import.meta.dirname, '../content/exams/PRO192-PE13.mjs');
...
const ATTACHMENT_URL = 'https://media.cuongthai.com/files/exam-attachments/PRO192/PE13-Given.zip';
...
<ul>
  <li><strong>f1</strong>: Return the number of words that contain exactly two different digits and at least one uppercase letter. (word = a string without space(s))</li>
  <li><strong>f2</strong>: Return a new string consisting of all words whose number of alphabetic characters is equal to the number of non-alphabetic characters. Keep the original order of appearance. Words in the result must be separated by a single space and the returned string must not contain leading or trailing spaces. Return an empty string if no word satisfies the condition. (word = a string without space(s))</li>
</ul>`;
...
<ul>
  <li><strong>f1</strong>: Trả về số lượng từ chứa đúng 2 chữ số KHÁC NHAU và có ít nhất 1 chữ cái HOA. (từ = chuỗi không chứa khoảng trắng)</li>
  <li><strong>f2</strong>: Trả về chuỗi mới gồm các từ có số ký tự chữ cái BẰNG số ký tự không phải chữ cái. Giữ nguyên thứ tự xuất hiện ban đầu. Các từ trong kết quả cách nhau đúng 1 khoảng trắng, chuỗi trả về không có khoảng trắng ở đầu/cuối. Trả về chuỗi rỗng nếu không có từ nào thoả điều kiện. (từ = chuỗi không chứa khoảng trắng)</li>
</ul>`;

const q4ExplainEn = `<p><strong>f1</strong>: collect the DISTINCT digit characters of each word into a <code>Set&lt;Character&gt;</code> — "exactly two different digits" means <code>set.size() == 2</code> (a word like "B121" has digits {1,2}, still size 2, even though '1' repeats), AND at least one UPPERCASE letter must be present (the opposite case of Đề 11/12's MyString, which required lowercase — don't reuse that class unchanged).</p>
<p><strong>f2</strong>: for each word, count alphabetic vs non-alphabetic characters; keep the word only when the two counts are EQUAL (also the opposite of Đề 11/12, which kept words where the counts DIFFERED), join survivors with a single space (no leading/trailing space — a <code>StringBuilder</code> that only inserts a separator when non-empty handles this naturally). The given sample text reads "is equal to" — easy to misread as "is not equal to" since that's the wording in the other papers; the sample run settles it either way.</p>
<p>Verified against both given samples by compiling <code>MyString.java</code> with the real given <code>Main.class</code>:</p>
<pre>f1("A12b B121 c34D 111X") → 3
  A12b: digits {1,2} size 2, has uppercase 'A' → counts
  B121: digits {1,2} size 2, has uppercase 'B' → counts
  c34D: digits {3,4} size 2, has uppercase 'D' → counts
  111X: digits {1} size 1 → excluded

f2("ab12 A#B# xyz") → "ab12 A#B#"
  ab12: 2 letters, 2 non-letters → equal → kept
  A#B#: 2 letters, 2 non-letters → equal → kept
  xyz:  3 letters, 0 non-letters → not equal → excluded</pre>`;
const q4ExplainVi = `<p><strong>f1</strong>: gom các ký tự CHỮ SỐ KHÁC NHAU của mỗi từ vào <code>Set&lt;Character&gt;</code> — "đúng 2 chữ số khác nhau" nghĩa là <code>set.size() == 2</code> (từ như "B121" có chữ số {1,2}, vẫn size 2 dù '1' lặp lại), VÀ phải có ít nhất 1 chữ cái HOA (ngược lại với MyString ở Đề 11/12 vốn đòi chữ thường — đừng copy nguyên lớp cũ).</p>
<p><strong>f2</strong>: với mỗi từ, đếm số ký tự chữ cái và không phải chữ cái; chỉ giữ từ khi 2 số này BẰNG NHAU (cũng ngược lại với Đề 11/12, vốn giữ từ khi 2 số KHÁC nhau), nối các từ giữ lại bằng đúng 1 khoảng trắng (không có khoảng trắng đầu/cuối — dùng <code>StringBuilder</code> chỉ chèn dấu cách khi đã có nội dung sẽ tự nhiên đạt yêu cầu này). Đề gốc ghi rõ "is equal to" — rất dễ đọc nhầm thành "is not equal to" vì đó là câu chữ ở các đề khác; chạy mẫu là cách chốt chắc chắn nhất.</p>
<p>Đã đối chiếu với cả 2 mẫu trong đề bằng cách biên dịch <code>MyString.java</code> cùng <code>Main.class</code> given thật:</p>
<pre>f1("A12b B121 c34D 111X") → 3
  A12b: chữ số {1,2} size 2, có chữ HOA 'A' → tính
  B121: chữ số {1,2} size 2, có chữ HOA 'B' → tính
  c34D: chữ số {3,4} size 2, có chữ HOA 'D' → tính
  111X: chữ số {1} size 1 → loại

f2("ab12 A#B# xyz") → "ab12 A#B#"
  ab12: 2 chữ cái, 2 không-chữ-cái → bằng nhau → giữ lại
  A#B#: 2 chữ cái, 2 không-chữ-cái → bằng nhau → giữ lại
  xyz:  3 chữ cái, 0 không-chữ-cái → không bằng → loại</pre>`;
...
ab12 A#B#`,
  ...
  rubric: [
    { id: 'f1', criterion: B('f1() requires exactly 2 DISTINCT digits (as a set) AND at least one UPPERCASE letter.', 'f1() đòi đúng 2 chữ số KHÁC NHAU (theo set) VÀ ít nhất 1 chữ HOA.'), weight: 1, maxScore: 1 },
    { id: 'f2', criterion: B('f2() keeps words where letter count EQUALS non-letter count, joins with single spaces, no leading/trailing space.', 'f2() giữ từ khi số chữ cái BẰNG số không-chữ-cái, nối bằng 1 khoảng trắng, không dư đầu/cuối.'), weight: 1, maxScore: 1 },
  ],
};
...
const spec = {
  ...
  exams: [
    {
      kind: 'PE', peType: 'CODE',
      code: 'PE13-SU26-B1-P3',
      title: B('PRO192 — Practical Exam Đề 13 (SU26, Block 1, Paper 3)', 'PRO192 — Đề thi thực hành số 13 (SU26, Block 1, Paper 3)'),
      description: B(
...
fs.writeFileSync(
  OUT,
  `// AUTO-GENERATED by scripts/build-pro192-pe13.mjs — do not hand-edit, re-run the builder instead.\nexport default ${JSON.stringify(spec, null, 2)};\n`,
  'utf8',
);
