/**
 * build-swd392-pe6.mjs — sinh content/exams/SWD392-PE6.mjs.
 *
 * Nguồn thật: "SWD392 - SU25 - PE - RE" ("Student Management
 * Administration Portal" tại công ty giả định "Cyber AI FPTx"). Không
 * có solution. 5 câu hỏi độc lập, mỗi câu đã có điểm riêng trong đề gốc
 * (3+3+2+1+1=10đ), không cần tách thêm.
 *
 * Q1 dùng Association Class (GradeReport) — Mermaid classDiagram KHÔNG
 * hỗ trợ đường nối đứt nét UML thật cho association class, nên mô
 * phỏng bằng stereotype «AssociationClass» + FK 2 chiều, đã ghi rõ
 * trong giải thích. Q2 chọn Sequence Diagram (không phải Communication)
 * vì rõ ràng hơn cho luồng theo thời gian, có gắn stereotype
 * «boundary»/«control»/«entity» đúng yêu cầu đề. Q5 là Observer pattern
 * (Course=subject, sinh viên chờ=observer).
 *
 * Seed: node scripts/academy-seed-exam.mjs --file ./content/exams/SWD392-PE6.mjs --apply
 */
import fs from 'node:fs';
import path from 'node:path';

const OUT = path.resolve(import.meta.dirname, '../content/exams/SWD392-PE6.mjs');
...
const instructions = ML(
  `<p><strong>SWD392 – Practical Exam (Summer 2025, Retake) — Student Management Administration Portal, "Cyber AI FPTx"</strong>. You work as a software engineer continuing to develop features for this portal. UML tools (offline mode) may be used; the diagrams here are rendered as Mermaid directly in the exam viewer. This is a written practical exam — each answer is graded by an AI grader against the rubric shown per question.</p>`,
  `<p><strong>SWD392 – Thi thực hành (Summer 2025, Retake) — Student Management Administration Portal, "Cyber AI FPTx"</strong>. Bạn là kỹ sư phần mềm tiếp tục phát triển tính năng cho cổng thông tin này. Có thể dùng công cụ UML (chế độ offline); sơ đồ ở đây hiển thị dạng Mermaid trực tiếp trong trình xem đề. Đây là bài thi thực hành dạng viết — mỗi câu trả lời được AI chấm theo tiêu chí ghi ở từng câu.</p>`,
);

const q1 = {
  kind: 'WRITE', points: 3,
  prompt: B(
    `<p><strong>Question 1 (3.0 points):</strong> Business Scenario: the system must capture how students, courses, and their grades relate to one another. A Student may enroll in many Courses, and each Course may have many enrolled Students. Whenever a Student finishes a Course, a GradeReport is created. This GradeReport is associated with exactly one Student and one Course, and it holds the final grade (e.g., '5.0', '6.0', '7.0', '8.0'). A GradeReport cannot exist unless both a Student and a Course are present. Task: create a Class Diagram that accurately represents this scenario — must include Student, Course, and GradeReport; give careful thought to the nature of the Student-Course relationship; GradeReport must be correctly modeled as an <b>Association Class</b>, with its connections and attributes displayed.</p>`,
    `<p><strong>Câu 1 (3.0 điểm):</strong> Tình huống: hệ thống phải thể hiện quan hệ sinh viên, môn học, và điểm số. 1 Student có thể đăng ký nhiều Course, mỗi Course có thể có nhiều Student đăng ký. Khi Student hoàn tất 1 Course, 1 GradeReport được tạo. GradeReport gắn với đúng 1 Student và 1 Course, giữ điểm cuối (VD '5.0', '6.0', '7.0', '8.0'). GradeReport không thể tồn tại nếu thiếu 1 trong 2. Nhiệm vụ: tạo Class Diagram thể hiện đúng tình huống — phải có Student, Course, GradeReport; cân nhắc kỹ bản chất quan hệ Student-Course; GradeReport phải mô hình đúng là <b>Association Class</b>, có đủ kết nối và thuộc tính.</p>`,
  ),
  sampleSolution: B(
    MMD(`classDiagram
    class Student {
...
const q2 = {
  ...
  sampleSolution: B(
    ...
    `<p><strong>Sơ đồ chọn: Sequence Diagram</strong> — phù hợp hơn ở đây vì tình huống được mô tả rõ là luồng theo thứ tự thời gian từng bước (controller phải kiểm điều kiện TRƯỚC KHI tạo đăng ký); Sequence diagram thể hiện rõ ràng thứ tự thời gian nghiêm ngặt đó và nhánh điều kiện (đủ/không đủ) qua trục dọc thời gian và alt fragment, trong khi Communication diagram (nhấn mạnh liên kết/cấu trúc đối tượng hơn thứ tự thời gian nghiêm ngặt) sẽ trình bày kém rõ ràng hơn.</p>` +
    MMD(`sequenceDiagram
    actor Student
    participant View as "CourseRegistrationView «boundary»"
    participant Ctrl as "RegistrationController «control»"
  ...
  explanation: B(
    `<p>PrerequisiteService is labeled «control» rather than a nonstandard stereotype: the 3 canonical analysis-level stereotypes are boundary/control/entity, and a "service" object performing business-rule logic (not raw data access, not UI) fits «control» — the same category as RegistrationController, just a delegate control object the main controller invokes.</p>`,
    `<p>PrerequisiteService gắn «control» thay vì 1 stereotype không chuẩn: 3 stereotype mức phân tích chuẩn là boundary/control/entity, và 1 object "service" xử lý logic nghiệp vụ (không phải truy cập dữ liệu thô, không phải UI) khớp «control» — cùng nhóm với RegistrationController, chỉ là control object phụ mà controller chính gọi tới.</p>`,
  ),
  rubric: [
    { id: 'appropriate_diagram_chosen', criterion: B('Chooses Sequence Diagram (or gives a coherent, defensible justification if Communication is chosen) and explains the choice.', 'Chọn Sequence Diagram (hoặc lý giải mạch lạc, hợp lý nếu chọn Communication) và giải thích lựa chọn.'), weight: 1, maxScore: 0.4 },
    { id: 'correct_stereotypes', criterion: B('All 4 objects (CourseRegistrationView, RegistrationController, PrerequisiteService, EnrollmentRepository) are labeled with the correct stereotype: boundary, control, control, and entity respectively.', 'Cả 4 object (CourseRegistrationView, RegistrationController, PrerequisiteService, EnrollmentRepository) gắn đúng stereotype: boundary, control, control, entity tương ứng.'), weight: 1, maxScore: 1 },
    { id: 'correct_time_ordered_flow', criterion: B('Shows the correct step-by-step order: view forwards to controller, controller checks eligibility via the service BEFORE creating the enrollment via the repository, with a conditional branch for the ineligible case.', 'Thể hiện đúng thứ tự từng bước: view chuyển cho controller, controller kiểm điều kiện qua service TRƯỚC KHI tạo đăng ký qua repository, có nhánh điều kiện cho trường hợp không đủ điều kiện.'), weight: 1, maxScore: 1.6 },
  ],
};
...
const q3 = {
  ...
  explanation: B(
    `<p>Every transition is triggered by the same event (<code>gpaRecalculated</code>, happening once per semester) but the guard condition and CURRENT state together determine the outcome: being already in AcademicWarning and still failing the threshold escalates to Probation (not a repeat of Warning), while being in GoodStanding and newly failing enters Warning for the first time — the state machine encodes "how many consecutive semesters below threshold" purely through which state the transition originates from, without needing an explicit counter attribute.</p>`,
    `<p>Mọi chuyển trạng thái đều do cùng 1 sự kiện (<code>gpaRecalculated</code>, xảy ra 1 lần/học kỳ) nhưng điều kiện bảo vệ CỘNG trạng thái HIỆN TẠI cùng quyết định kết quả: đã ở AcademicWarning mà vẫn dưới ngưỡng thì lên Probation (không lặp lại Warning), còn đang GoodStanding mà mới dưới ngưỡng lần đầu thì vào Warning lần đầu — máy trạng thái mã hoá "bao nhiêu kỳ liên tiếp dưới ngưỡng" hoàn toàn qua việc chuyển tiếp bắt đầu từ trạng thái nào, không cần thuộc tính đếm tường minh.</p>`,
  ),
  rubric: [
    { id: 'all_four_states', criterion: B('Includes all 4 required states: GoodStanding, AcademicWarning, AcademicProbation, Dismissed, with Dismissed as a final state.', 'Có đủ 4 trạng thái yêu cầu: GoodStanding, AcademicWarning, AcademicProbation, Dismissed, Dismissed là trạng thái cuối.'), weight: 1, maxScore: 0.6 },
    { id: 'events_and_guards_labeled', criterion: B('Every transition is labeled with the triggering event (gpaRecalculated) and the correct guard condition ([GPA < 5.0] or [GPA >= 5.0]).', 'Mọi chuyển trạng thái gắn đúng sự kiện kích hoạt (gpaRecalculated) và điều kiện bảo vệ đúng ([GPA < 5.0] hoặc [GPA >= 5.0]).'), weight: 1, maxScore: 0.8 },
    { id: 'escalation_logic_correct', criterion: B('Correctly models the escalation logic: failing while already in Warning leads to Probation (not back to Warning), and failing while already in Probation leads to Dismissed — the "still below threshold in the following semester" progression.', 'Mô hình đúng logic leo thang: trượt khi đã ở Warning thì lên Probation (không lặp Warning), trượt khi đã ở Probation thì tới Dismissed — đúng tiến trình "vẫn dưới ngưỡng ở kỳ sau".'), weight: 1, maxScore: 0.6 },
  ],
};
...
const q4 = {
  kind: 'WRITE', points: 1,
  prompt: B(
    `<p><strong>Question 4 (1.0 point):</strong> the Student Management Portal is a typical web-based application. Its key requirements are a clear separation of concerns, easy maintainability, and the ability for different teams (UI designers, backend developers, database administrators) to work in parallel. The system should also be organized so that new developers can understand it quickly and get onboarded easily. Task: suggest a well-known, appropriate software architecture that best fits a traditional web application with these requirements. Name the architecture you select and explain why its structure is a good choice for this project. List one key advantage and one key disadvantage.</p>`,
    `<p><strong>Câu 4 (1.0 điểm):</strong> Student Management Portal là ứng dụng web điển hình. Yêu cầu chính: tách biệt rõ mối quan tâm, dễ bảo trì, các nhóm khác nhau (thiết kế UI, backend, quản trị database) làm việc song song được. Hệ thống cũng cần tổ chức để dev mới hiểu nhanh và onboard dễ dàng. Nhiệm vụ: đề xuất 1 kiến trúc phần mềm nổi tiếng, phù hợp cho ứng dụng web truyền thống với các yêu cầu này. Nêu tên kiến trúc chọn và giải thích tại sao cấu trúc của nó là lựa chọn tốt cho dự án. Liệt kê 1 ưu điểm chính và 1 nhược điểm chính.</p>`,
  ),
  sampleSolution: B(
    `<p><strong>Architecture selected: Layered (N-tier) Architecture</strong> — presentation layer (UI), business logic layer (services/controllers), and data access layer (repositories/database), each with a clearly defined boundary.</p>
  ...
  rubric: [
    { id: 'named_architecture_appropriate', criterion: B('Names a well-known architecture genuinely suited to a traditional web app with the stated requirements (Layered/N-tier is the natural fit; other well-justified choices like MVC can also be accepted).', 'Nêu tên 1 kiến trúc nổi tiếng thực sự phù hợp ứng dụng web truyền thống với yêu cầu đề nêu (Layered/N-tier là khớp tự nhiên nhất; lựa chọn khác có lý giải tốt như MVC cũng chấp nhận được).'), weight: 1, maxScore: 0.4 },
    { id: 'explanation_ties_to_requirements', criterion: B('Explains why the chosen structure specifically supports separation of concerns, parallel team work, and fast onboarding — not a generic definition disconnected from the stated requirements.', 'Giải thích tại sao cấu trúc chọn cụ thể hỗ trợ tách mối quan tâm, làm việc song song theo nhóm, và onboard nhanh — không phải định nghĩa chung chung tách rời yêu cầu đề nêu.'), weight: 1, maxScore: 0.3 },
    { id: 'advantage_and_disadvantage', criterion: B('Lists one genuine key advantage and one genuine key disadvantage of the chosen architecture.', 'Nêu đủ 1 ưu điểm chính và 1 nhược điểm chính thật của kiến trúc chọn.'), weight: 1, maxScore: 0.3 },
  ],
};
...
const q5 = {
  ...
  rubric: [
    { id: 'correct_pattern_named', criterion: B('Correctly identifies Observer as the pattern.', 'Xác định đúng Observer là pattern.'), weight: 1, maxScore: 0.5 },
    { id: 'why_explanation', criterion: B('Explains why Observer fits specifically because it decouples the subject (Course) from needing to know the identity/contact details of its dependents (waiting-list students), addressing the scenario\'s explicit constraint.', 'Giải thích tại sao Observer phù hợp cụ thể vì nó tách subject (Course) khỏi việc cần biết danh tính/chi tiết liên hệ của các đối tượng phụ thuộc (sinh viên chờ), đúng ràng buộc rõ của tình huống.'), weight: 1, maxScore: 0.5 },
  ],
};
