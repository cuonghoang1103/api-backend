/**
 * dk-ghep-chuong.mjs — kiểm một chương khoá "Docker" trước khi ghép (chép từ git-ghep-chuong.mjs).
 *   node scripts/dk-ghep-chuong.mjs <file chương .mjs> [--render <thư mục ảnh đã render>] [--cdn] [--moi]
 *
 * (--render mặc định: $DK_RENDER hoặc /tmp/dk-render — nơi _render-slides.mjs --out đã ghi dk-NN/NNN.webp)
 * (--moi: chương MỚI, chưa có trong git HEAD ⇒ bỏ qua phép so với bản cũ)
 *
 * 1. import được; content là chuỗi; title ≤ 180 ký tự và có |||; slug dạng dk-N-M-…
 * 2. bài đầu = dk-N-0-slides DOCUMENT; có đúng MỘT bài QUIZ ở cuối, 10 câu, câu nào cũng có explanation,
...
const RENDER = rI >= 0 ? process.argv[rI + 1] : (process.env.DK_RENDER || '/tmp/dk-render');
let bad = 0, warn = 0;
...
const i0 = ls.findIndex((l) => !/^dk-0-\d+-bat-dau/.test(l.slug || ''));
if (i0 > 2) err('Mục 0: tối đa 2 bài bat-dau trước bài slide');
const m0 = ls[i0]?.slug?.match(/^dk-(\d+)-0-slides$/);
if (!m0 || ls[i0].type !== 'DOCUMENT') err('bài đầu phải là dk-N-0-slides, type DOCUMENT');
const N = m0 ? m0[1] : '?';
...
for (const l of ls) {
  ...
  if (!/^dk-\d+-\d+-[a-z0-9-]+$/.test(l.slug || '')) err(`${tag}: slug sai dạng`);
  else if (!l.slug.startsWith(`dk-${N}-`)) err(`${tag}: slug không thuộc chương ${N}`);
...
if (ls[i0]?.type === 'DOCUMENT') {
  const deck = `dk-${String(N).padStart(2, '0')}`;
...
/* 5. ảnh slide */
const refs = new Set();
for (const l of ls) for (const m of l.content.matchAll(/images\/academy\/DK\/(v\d+)\/(dk-\d\d)\/(\d{3})\.webp/g)) refs.add(m[0]);
let miss = 0;
for (const r of refs) {
