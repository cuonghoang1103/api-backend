/**
 * ga-ghep-chuong.mjs — kiểm một chương khoá "GitHub Actions" trước khi ghép (chép từ git-ghep-chuong.mjs).
 *   node scripts/ga-ghep-chuong.mjs <file chương .mjs> [--render <thư mục ảnh đã render>] [--cdn] [--moi]
 *
 * (--render mặc định: $GA_RENDER hoặc /tmp/ga-render — nơi _render-slides.mjs --out đã ghi ga-NN/NNN.webp)
 * (--moi: chương MỚI, chưa có trong git HEAD ⇒ bỏ qua phép so với bản cũ)
 *
 * 1. import được; content là chuỗi; title ≤ 180 ký tự và có |||; slug dạng ga-N-M-…
 * 2. bài đầu = ga-N-0-slides DOCUMENT; có đúng MỘT bài QUIZ ở cuối, 10 câu, câu nào cũng có explanation,
...
const RENDER = rI >= 0 ? process.argv[rI + 1] : (process.env.GA_RENDER || '/tmp/ga-render');
let bad = 0, warn = 0;
...
// Bài slide N.0 thường đứng ĐẦU chương. Ngoại lệ: Mục 0 có ≤2 bài "…-bat-dau-…" đứng trước; Chương 11 (ôn tổng) giữ
// bài cũ ga-11-1-chot đứng đầu để seeder neo chương bằng slug cũ khi ĐỔI TIÊU ĐỀ chương (xem _HOP-DONG.md mục 4c).
const i0 = ls.findIndex((l) => /^ga-\d+-0-slides$/.test(l.slug || ''));
if (i0 < 0) err('không thấy bài ga-N-0-slides');
else if (i0 > 2) err('bài slide phải nằm trong 3 bài đầu chương');
else ls.slice(0, i0).forEach((l) => { if (!/^ga-0-\d+-bat-dau/.test(l.slug) && l.slug !== 'ga-11-1-chot') err(`${l.slug}: bài đứng trước bài slide không hợp lệ`); });
const m0 = ls[i0]?.slug?.match(/^ga-(\d+)-0-slides$/);
if (m0 && ls[i0].type !== 'DOCUMENT') err('bài ga-N-0-slides phải là type DOCUMENT');
const N = m0 ? m0[1] : '?';
...
for (const l of ls) {
  ...
  if (!/^ga-\d+-\d+-[a-z0-9-]+$/.test(l.slug || '')) err(`${tag}: slug sai dạng`);
  else if (!l.slug.startsWith(`ga-${N}-`)) err(`${tag}: slug không thuộc chương ${N}`);
...
if (!MOI) {
  ...
  if (old) {
    if (old.title !== mod.title && old.lessons.some((o) => o.slug === ls[0]?.slug)) wn(`tiêu đề chương đổi nhưng bài đầu (${ls[0].slug}) là bài cũ ⇒ seeder neo bằng slug, an toàn`);
    else if (old.title !== mod.title) err(`tiêu đề chương ĐỔI so với HEAD — seeder sẽ tạo CHƯƠNG MỚI:\n      cũ: ${old.title}\n      mới: ${mod.title}`);
...
if (ls[i0]?.type === 'DOCUMENT') {
  const deck = `ga-${String(N).padStart(2, '0')}`;
...
/* 5. ảnh slide */
const refs = new Set();
for (const l of ls) for (const m of l.content.matchAll(/images\/academy\/GA\/(v\d+)\/(ga-\d\d)\/(\d{3})\.webp/g)) refs.add(m[0]);
let miss = 0;
for (const r of refs) {
