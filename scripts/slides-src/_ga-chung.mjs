/**
 * _ga-chung.mjs — giao diện + thư viện hình vẽ cho các deck của khoá "GitHub Actions"
 * (deck ga-00 … ga-15). CHÉP từ _dk-chung.mjs (khoá Docker) rồi đổi màu nhấn sang xanh GitHub Actions #2088ff,
 * thêm pipe() (sơ đồ job/step) và bìa hình pipeline. Tên màu 'dk' vẫn dùng được (= màu nhấn), thêm 'ga'.
 *
...
export const D = {
  ...
  dk: '#2088ff', ga: '#2088ff', blu: '#58a6ff', tea: '#2dd4bf', grn: '#3fb950', amb: '#ffc233', ora: '#ff8a3d', vio: '#bc8cff', pnk: '#f778ba', red: '#ff5c6c',
};
...
.slide{background:
 radial-gradient(900px 480px at 100% 0%,rgba(32,136,255,.16),transparent 60%),
...
.slide.cover{background:
 radial-gradient(700px 420px at 78% 30%,rgba(32,136,255,.34),transparent 62%),
...
.c-cov .chip{background:rgba(32,136,255,.16);border-color:rgba(32,136,255,.6)}
.c-cov .chip i{background:${D.dk};box-shadow:0 0 12px ${D.dk};border-radius:2px;transform:none}
.c-mm .ctr{background:linear-gradient(135deg,${D.dk},#1f6feb);box-shadow:0 10px 40px rgba(32,136,255,.35)}
...
export const cover = ({ t, sub, chap, meta = 'GitHub Actions · cuongthai.com' }) => ({
  kind: 'cover', t, sub,
  body: `<div class="c-cov">${chap ? `<span class="chip"><i></i>${chap}</span>` : ''}` +
    `${miniStack()}<span class="meta">${meta}</span></div>`,
});

/** Hình trang trí trên bìa: một pipeline — trigger → 2 job song song → deploy. */
const miniStack = () => {
  const n = (x, y, c) => `<circle cx="${x}" cy="${y}" r="11" fill="#0b1322" stroke="${c}" stroke-width="4"/><path d="M${x - 5} ${y} l3.5 3.5 l6.5 -7" stroke="${c}" stroke-width="2.6" fill="none"/>`;
  return `<svg viewBox="0 0 330 70" width="330" height="70">` +
    `<path d="M40 35 C80 35 80 14 120 14 H210 C250 14 250 35 290 35 M40 35 C80 35 80 56 120 56 H210 C250 56 250 35 290 35" stroke="${D.dk}" stroke-width="3.5" fill="none" opacity=".75"/>` +
    n(40, 35, D.blu) + n(120, 14, D.grn) + n(210, 14, D.grn) + n(120, 56, D.tea) + n(210, 56, D.tea) + n(290, 35, D.vio) + `</svg>`;
};
...
export const sv = (w, h, inner) => `<svg viewBox="0 0 ${w} ${h}" width="${w}" height="${h}" style="display:block;margin:0 auto"><defs>` +
  ['dk', 'ga', 'blu', 'tea', 'grn', 'amb', 'ora', 'vio', 'pnk', 'red', 'mu', 'dim'].map((k) => `<marker id="m-${k}" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M0 0 L10 5 L0 10 z" fill="${D[k]}"/></marker>`).join('') +
  `</defs>${inner}</svg>`;
