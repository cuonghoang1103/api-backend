...
export async function adminGenerate(
  ...
  let raw = '';
  try {
    const res = await llmComplete({ step: 'generation', purpose: 'language_bulk', feature: 'bulk_gen', system, messages: [{ role: 'user', content: user }], maxTokens, maxRetries: 1, timeoutMs: 90_000, userId });
    raw = res.text;
  } catch {
