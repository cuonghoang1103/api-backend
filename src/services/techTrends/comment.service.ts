/**
 * Tech Trends — reader comments service.
 *
...
export async function listComments(articleId: number, viewerId?: number | null) {
  const rows = await prisma.techTrendComment.findMany({
    where: { articleId },
  ...
  if (viewerId) {
    const likes = await prisma.techTrendCommentLike.findMany({
...
export async function createComment(
  ...
  const article = await prisma.techTrendArticle.findUnique({ where: { id: articleId }, select: { status: true } });
  ...
  if (parentId) {
    const parent = await prisma.techTrendComment.findUnique({ where: { id: parentId }, select: { id: true, articleId: true, parentId: true } });
    if (!parent || parent.articleId !== articleId) throw new AppError('Bình luận gốc không tồn tại', 400, 'PARENT_NOT_FOUND');
    effectiveParent = parent.parentId ?? parent.id;
  }

  const created = await prisma.techTrendComment.create({
    data: { articleId, userId, content: text, parentId: effectiveParent },
    include: { user: authorSelect },
  });
...
export async function editComment(commentId: number, userId: number, content: string): Promise<CommentDto> {
  ...
  const existing = await prisma.techTrendComment.findUnique({ where: { id: commentId }, select: { userId: true } });
  ...
  const updated = await prisma.techTrendComment.update({
    where: { id: commentId },
...
export async function deleteComment(commentId: number, userId: number): Promise<void> {
  ...
  await prisma.techTrendComment.delete({ where: { id: commentId } });
}

export async function toggleLike(commentId: number, userId: number): Promise<{ liked: boolean; likesCount: number }> {
  const comment = await prisma.techTrendComment.findUnique({ where: { id: commentId }, select: { id: true } });
  if (!comment) throw new AppError('Comment not found', 404, 'COMMENT_NOT_FOUND');

  const existing = await prisma.techTrendCommentLike.findUnique({
    where: { uk_tt_comment_like: { commentId, userId } },
    select: { id: true },
  });

  if (existing) {
    const [, updated] = await prisma.$transaction([
      prisma.techTrendCommentLike.delete({ where: { id: existing.id } }),
      prisma.techTrendComment.update({ where: { id: commentId }, data: { likesCount: { decrement: 1 } }, select: { likesCount: true } }),
    ]);
  ...
  try {
    const [, updated] = await prisma.$transaction([
      prisma.techTrendCommentLike.create({ data: { commentId, userId } }),
      prisma.techTrendComment.update({ where: { id: commentId }, data: { likesCount: { increment: 1 } }, select: { likesCount: true } }),
    ]);
    return { liked: true, likesCount: updated.likesCount };
  } catch {
    // Unique race: the like already exists — report current state.
    const c = await prisma.techTrendComment.findUnique({ where: { id: commentId }, select: { likesCount: true } });
